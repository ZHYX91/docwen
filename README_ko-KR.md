...
`DocWen.exe`를 더블클릭하여 그래픽 인터페이스를 실행합니다.
...
1.  **프로그램 실행**: `DocWen.exe`를 더블클릭합니다.
2.  **파일 가져오기**:
    -   방법 1: 파일을 창으로 드래그합니다.
...
-   **대화형 모드**: 파일을 전달하면 GUI와 유사한 메뉴 가이드를 표시합니다.
-   **Headless 모드**: `--action` 파라미터로 직접 실행하며 스크립트 호출에 적합합니다.
### 일반적인 예시
...
| 인자 | 설명 |
| :--- | :--- |
| `--action` | 작업 유형: `export_md`, `convert`, `validate`, `merge_pdfs`, `split_pdf` |
| `--target` | 대상 형식: `pdf`, `docx`, `xlsx`, `md` |
| `--template` | 템플릿 이름(예: `Template Name`) |
| `--extract-img` | 내보내기 시 이미지 추출 |
| `--ocr` | OCR 활성화 |
| `--batch` | 배치 처리 |
| `--yes` / `-y` | 확인 프롬프트 건너뛰기 |
| `--continue-on-error` | 오류가 있어도 다음 항목 계속 |
