...
def _make_doc_to_docx_converter(input_path: str, output_path: str, prog_id: str) -> str | None:
    """DOC→DOCX 和 WPS→DOCX COM 转换器"""
    return try_com_conversion(input_path, output_path, prog_id, 12, "word")


def _make_odt_to_docx_converter(input_path: str, output_path: str, prog_id: str) -> str | None:
    """ODT → DOCX COM 转换器"""
    return try_com_conversion(input_path, output_path, prog_id, 12, "word")


def _make_rtf_to_docx_converter(input_path: str, output_path: str, prog_id: str) -> str | None:
    """RTF → DOCX COM 转换器"""
    return try_com_conversion(input_path, output_path, prog_id, 12, "word")


def _make_docx_to_odt_converter(input_path: str, output_path: str, prog_id: str) -> str | None:
    """DOCX → ODT COM 转换器"""
    return try_com_conversion(input_path, output_path, prog_id, 23, "word")


def _make_docx_to_doc_converter(input_path: str, output_path: str, prog_id: str) -> str | None:
    """DOCX → DOC COM 转换器"""
    return try_com_conversion(input_path, output_path, prog_id, 0, "word")


def _make_docx_to_rtf_converter(input_path: str, output_path: str, prog_id: str) -> str | None:
    """DOCX → RTF COM 转换器"""
...
def office_to_docx(
    ...
    **kwargs,
    ...
    """
    logger.info(f"开始转换: {Path(input_path).name} → DOCX")
    ...
    if result:
        logger.info(f"✓ DOCX转换完成: {Path(result).name} (使用软件: {used_software})")
    else:
...
def rtf_to_docx(
    ...
    **kwargs,
    ...
    if result:
        logger.info(f"✓ DOCX 转换完成: {Path(result).name} (使用软件: {used_software})")
...
def odt_to_docx(
    ...
    **kwargs,
    ...
    converters = [("Microsoft Word", "Word.Application", _make_odt_to_docx_converter)]
...
def docx_to_odt(
    ...
    **kwargs,
    ...
    返回:
        成功时返回output_path，失败时返回None
    """
    logger.info(f"开始转换: {Path(input_path).name} → ODT")

    converters = [("Microsoft Word", "Word.Application", _make_docx_to_odt_converter)]
    ...
    if result:
        logger.info(f"✓ ODT 转换完成: {Path(result).name} (使用软件: {used_software})")
...
def docx_to_doc(
    ...
    **kwargs,
    ...
    返回:
        成功时返回output_path，失败时返回None
    """
    logger.info(f"开始转换: {Path(input_path).name} → DOC")
    ...
    if result:
        logger.info(f"✓ DOC 转换完成: {Path(result).name}")
...
def docx_to_rtf(
    ...
    **kwargs,
    ...
    返回:
        成功时返回output_path，失败时返回None
    """
    logger.info(f"开始转换: {Path(input_path).name} → RTF")
    ...
    if result:
        logger.info(f"✓ RTF 转换完成: {Path(result).name}")
