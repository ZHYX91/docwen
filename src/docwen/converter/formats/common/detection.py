...
def check_office_availability(target_format: str) -> tuple[bool, str]:
    ...
    if target_format in ["doc", "wps", "rtf"]:
        # 文档格式：检测Word类软件
        if _check_wps_word_available() or _check_office_word_available() or _check_libreoffice_available():
            result = (True, "")
        else:
            error_msg = (
        ...
        # 表格格式：检测Excel类软件
        if _check_wps_spreadsheets_available() or _check_office_excel_available() or _check_libreoffice_available():
            result = (True, "")
        else:
            error_msg = (
                f"无法转换为 {target_format.upper()} 格式\n\n"
        ...
        if _check_office_word_available() or _check_libreoffice_available():
            result = (True, "")
        else:
            error_msg = (
                f"无法转换为 {target_format.upper()} 格式\n\n"
        ...
        if _check_office_excel_available() or _check_libreoffice_available():
            result = (True, "")
        else:
            error_msg = (
                f"无法转换为 {target_format.upper()} 格式\n\n"
