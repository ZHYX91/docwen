...
def _get_fallback_title(yaml_data: dict, md_path: str | None = None) -> str:
    """
    获取标题的回退值
...
def _ensure_title_fallbacks(yaml_data: dict, md_path: str | None = None):
    """
    为所有语言版本的标题键设置回退值
...
def convert(
    ...
    *,
    ...
    try:
        logger.info("=" * 60)
        logger.info(f"开始MD转Excel: {Path(md_path).name}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 4. 在临时目录生成临时输出文件
            temp_output = str(Path(temp_dir) / "temp_output.xlsx")

            # 5. 在临时目录转换
            success, warnings = _convert_internal(
                yaml_data, md_body, template_path, temp_output, progress_callback, cancel_event
            )
            ...
            original_input_file = original_source_path if original_source_path else md_path
            saved_path, _ = save_output_with_fallback(temp_output, output_path, original_input_file=original_input_file)
            if not saved_path:
                return None
            ...
            if should_save_intermediate_files():
                ...
                for item, saved_path in saved_items:
                    logger.debug(f"保留中间文件: {Path(saved_path).name} ({item.kind})")
            ...
            if warnings:
                ...
                if progress_callback:
                    progress_callback(
                        t("conversion.progress.conversion_completed_with_readonly_warnings", count=len(warnings))
                    )
...
def _read_and_parse_md(temp_md_path: str, original_md_path: str | None = None, temp_dir: str | None = None) -> tuple:
    ...
    with Path(temp_md_path).open(encoding="utf-8") as f:
        content = f.read()

    from docwen.utils.markdown_utils import extract_yaml

    yaml_content, md_body = extract_yaml(content)
    ...
    try:
        ...
        def process_yaml_links(data, source_path):
            ...
            if isinstance(data, dict):
                ...
                return process_markdown_links(data, source_path, set(), depth=0, temp_dir=temp_dir)
            else:
                return data
        ...
        md_body = process_markdown_links(md_body, source_path_for_resolve, table_safe=True, temp_dir=temp_dir)
...
def _get_template_path(template_name: str) -> str:
    ...
    try:
        ...
        logger.error(f"获取模板路径失败: {e!s}")
        raise RuntimeError("无法获取模板路径") from e


def _convert_internal(
    yaml_data: dict, md_body: str, template_path: str, output_path: str, progress_callback, cancel_event
) -> tuple:
    """
    内部转换实现
...
def process_cell_value(raw_value):
    ...
    try:
        from docwen.utils.link_processing import TABLE_CELL_BR_TOKEN

        raw_value = raw_value.replace(TABLE_CELL_BR_TOKEN, "\n")
    except Exception:
        pass
    ...
    try:
        ...
        for table_index, table_data in enumerate(tables):
            try:
                ...
                logger.warning(f"跳过表格 {table_index + 1}: {e!s}")
                continue

    except Exception as e:
        logger.error(f"表格提取失败: {e!s}")
...
def is_cell_writable(ws, row, col):
    ...
    for merged_range in ws.merged_cells.ranges:
        ...
        if min_row <= row <= max_row and min_col <= col <= max_col:
            # 只有左上角单元格可写入
            return bool(row == min_row and col == min_col)
    return True
...
def is_cell_protected(ws, cell):
    ...
    try:
        ...
        logger.debug(f"检查单元格保护状态时出错: {e!s}")
    return False
...
def add_cell_comment(cell, comment_text, consumed_value=None):
    ...
    try:
        ...
        logger.warning(f"无法添加批注到单元格 {cell.coordinate}: {e!s}")
            ...
            while data_index < len(values_with_format):
                ...
                if is_cell_protected(ws, target_cell):
                    ...
                    try:
                        ...
                        filled_count += 1
                        logger.debug(
                            f"列填充: {position.sheet_name}[{current_row},{position.col}] -> {value} (格式: {cell_format})"
                        )
                        data_index += 1
                        current_row += 1
                        ...
                        # 处理其他只读情况
                        warning_msg = (
                            f"无法填充 {field_name} 到 {position.sheet_name}[{current_row},{position.col}]：单元格只读"
                        )
                        logger.warning(warning_msg)
                        if warning_msg not in warning_messages:
                            warning_messages.add(warning_msg)
            ...
            while data_index < len(values_with_format):
                ...
                if is_cell_protected(ws, target_cell):
                    ...
                    try:
                        ...
                        filled_count += 1
                        logger.debug(
                            f"行填充: {position.sheet_name}[{position.row},{current_col}] -> {value} (格式: {cell_format})"
                        )
                        data_index += 1
                        current_col += 1
                        ...
                        # 处理其他只读情况
                        warning_msg = (
                            f"无法填充 {field_name} 到 {position.sheet_name}[{position.row},{current_col}]：单元格只读"
                        )
                        logger.warning(warning_msg)
                        if warning_msg not in warning_messages:
                            warning_messages.add(warning_msg)
