...
from docwen.utils.text_utils import clean_text
...
_is_fraction = is_fraction


def is_numeric_string(value: object) -> bool:
    """
    检查字符串是否是数字格式（整数、小数、负数等）
    ...
    示例:
        >>> _is_numeric_string("123")
        True
        >>> _is_numeric_string("123.45")
        True
        >>> _is_numeric_string("-123")
        True
        >>> _is_numeric_string("abc")
        False
    """
...
def extract_template_placeholders(template_path: str) -> dict:
    ...
    logger.info(
        f"从模板中提取到占位符: YAML={len(yaml_placeholders)}, 列填充={len(column_placeholders)}, 行填充={len(row_placeholders)}"
    )
        ...
        if field_name in data:
            ...
            if value is None:
                ...
                if _is_fraction(value):
                    ...
                    # 字符串形式的数字（整数、小数、负数等）
                    # 计算有效数字位数（移除小数点、负号、正号）
                    digit_count = len(value.replace(".", "").replace("-", "").replace("+", ""))
                    if digit_count > 15 or value.isdigit():
                        # 超长数字或纯数字字符串，设置为文本格式防止精度丢失或科学计数法
                        processed_value = value
                        cell_format = numbers.FORMAT_TEXT
                    else:
                        # 短数字保持原样，让 Excel 自然处理
                        processed_value = value
                else:
                ...
                cleaned_items = [clean_text(str(item)) for item in value if item not in [None, "", "null", "None"]]
                processed_value = list_separator.join(cleaned_items)
            else:
                ...
                if cell.value and isinstance(cell.value, str):
                    for placeholder in template_placeholders:
                        if placeholder in cell.value:
                            ...
                            # 使用 cell.row 和 cell.column 获取准确的行列位置
                            position = PlaceholderPosition(
                                sheet_name=sheet_name, row=cell.row, col=cell.column, is_whole_cell=is_whole_cell
                            )
                            positions[placeholder].append(position)
                            logger.debug(
                                f"找到占位符位置: {placeholder} -> {sheet_name}[{cell.row},{cell.column}] (完整单元格: {is_whole_cell})"
                            )
            ...
            for cell in row:
                ...
                if is_single_placeholder:
                    ...
                    if placeholder in replacements:
                        ...
                        logger.debug(
                            f"完整替换单元格 {cell.coordinate}: {original_value[:50]} -> {str(value_to_replace)[:50]}"
                        )
                ...
                for image_payload in matches:
                    ...
                    try:
                        ...
                        if width is not None or height is not None:
                            img.width, img.height = _scale_image(img.width, img.height, width, height)
                        else:
                            max_width = 200
                        ...
                        if (
                            ...
                            or ws.row_dimensions[cell.row].height < row_height
                        ):
                        ...
                        inserted_count += 1
                        logger.info(f"成功插入图片 ({inserted_count}) 到 {cell.coordinate}: {Path(image_path).name}")

                    except Exception as e:
                        logger.error(f"插入图片失败: {image_path} | 错误: {e!s}")
                        # 删除占位符，避免后续清理时出错
                        cell.value = cell.value.replace(placeholder, "")
