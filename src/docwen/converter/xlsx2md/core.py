...
from .image_processor import extract_images_from_xlsx, replace_image_markers
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if (r, c) not in visited and _cell_has_value(cell_value):
                ...
                # 提取块并清理
                block = cast(pd.DataFrame, df.iloc[min_r : max_r + 1, min_c : max_c + 1].copy())
...
def _fill_merged_cells(ws: Any) -> None:
    """
...
def _worksheet_to_dataframe(ws: Any, sheet_images: list[dict[str, Any]] | None = None) -> pd.DataFrame:
    ...
    for merged_range in list(ws.merged_cells.ranges):
        range_min_row, range_min_col, range_max_row, range_max_col = (
            merged_range.min_row,
            merged_range.min_col,
            merged_range.max_row,
            merged_range.max_col,
        ...
        top_left_value = ws.cell(row=range_min_row, column=range_min_col).value
        for row in range(range_min_row, range_max_row + 1):
            for col in range(range_min_col, range_max_col + 1):
                if row == range_min_row and col == range_min_col:
                    continue
                merged_value_by_cell[(row, col)] = top_left_value
    ...
    for row_cells in ws.iter_rows(min_row=1, max_row=max_row, min_col=1, max_col=max_col):
        ...
        for cell in row_cells:
            value = cell.value
            if value is None:
                value = merged_value_by_cell.get((cell.row, cell.column))
    ...
    if sheet_images:
        ...
        if max_row_needed >= current_rows:
            ...
            # 创建空行并添加到DataFrame
            empty_rows = pd.DataFrame([[None] * max(current_cols, max_col_needed + 1) for _ in range(rows_to_add)])
            if current_cols > 0:
                empty_rows.columns = (
                    df.columns
                    if len(df.columns) == max(current_cols, max_col_needed + 1)
                    else list(range(max(current_cols, max_col_needed + 1)))
                )
        ...
        for img_info in sheet_images:
            ...
            if row is not None and col is not None and filename:
                ...
                if 0 <= df_row < len(df) and 0 <= df_col < len(df.columns):
                    ...
                    logger.warning(
                        f"图片位置超出DataFrame范围: 行{row}, 列{col}, DataFrame大小: {len(df)}x{len(df.columns)}"
                    )
...
def _read_csv_flexible(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        return pd.read_csv(file_path, header=None, keep_default_na=False)
    ...
    for encoding in candidates:
        ...
        try:
            if sep:
                return pd.read_csv(file_path, header=None, keep_default_na=False, encoding=encoding, sep=sep)
            return pd.read_csv(
                file_path, header=None, keep_default_na=False, encoding=encoding, sep=None, engine="python"
            )
        except Exception:
            continue
...
def convert_spreadsheet_to_md(
    ...
    file_stem = Path(path_for_naming).stem

    try:
        from docwen.utils.file_type_utils import detect_actual_file_format

        input_format = detect_actual_file_format(file_path)
    except Exception:
    ...
    logger.debug(f"输入文件实际格式: {input_format}")

    try:
        if extraction_mode is None:
            extraction_mode = config_manager.get_xlsx_to_md_image_extraction_mode()
        if ocr_placement_mode is None:
            ocr_placement_mode = config_manager.get_xlsx_to_md_ocr_placement_mode()
        extraction_mode_s: str = extraction_mode or "file"
        ocr_placement_mode_s: str = ocr_placement_mode or "image_md"
        ...
        if input_format == "csv":
            ...
            for _i, block in enumerate(blocks):
                ...
                if block.shape[0] > 0:
                    ...
                    data = block.iloc[1:]
                    md_content += data.to_markdown(index=False, headers=headers) or ""
                else:
                    md_content += (
                        block.to_markdown(
                            index=False, headers=[] if block.shape[1] > 0 and pd.isna(block.columns).all() else "keys"
                        )
                        or ""
                    )
                md_content += "\n\n"
            ...
            if (extract_image or extract_ocr) and output_folder:
                ...
                images_info = extract_images_from_xlsx(
                    wb, output_folder, path_for_naming, progress_callback=progress_callback
                )
                ...
                if not blocks:
                    ...
                    for _i, block in enumerate(blocks):
                        ...
                        if block.shape[0] > 0:
                            headers = block.iloc[0].tolist()
                            data = block.iloc[1:]
                            block_md = data.to_markdown(index=False, headers=headers) or ""
                        else:
                            block_md = (
                                block.to_markdown(
                                    index=False,
                                    headers=[] if block.shape[1] > 0 and pd.isna(block.columns).all() else "keys",
                                )
                                or ""
                            )
        ...
        logger.error(f"处理表格文件 {Path(path_for_naming).name} 失败: {e}", exc_info=True)
        raise
