...
class SmartConverter:
    ...
    def convert(
        ...
        preferred_software: str | None = None,
    ) -> str | None:
        """
        执行智能转换
        ...
        try:
            ...
            # 规划转换路径
            conversion_path = self._plan_conversion_path(source_format, target_format, category)

            if not conversion_path:
                logger.error(f"无法规划转换路径: {source_format} → {target_format}")
                raise RuntimeError(f"无法规划转换路径: {source_format} → {target_format}")
            ...
            raise RuntimeError(f"智能转换失败: {e}") from e

    def _plan_conversion_path(self, source_format: str, target_format: str, category: str) -> list[str]:
        """
        规划转换路径
    ...
    def _convert_single_step(
        ...
        if progress_callback:
            progress_callback(
                t(
                    "conversion.progress.converting_source_to_target",
        ...
        final_output_dir = output_dir or str(Path(input_path).parent)
        from docwen.utils.workspace_manager import finalize_output

        with tempfile.TemporaryDirectory() as temp_dir:
            result_item = self._execute_conversion(
                input_path, source_format, target_format, temp_dir, cancel_event, exclude_wps, preferred_software
            )
            if not result_item:
                return None

            if target_format == "csv":
                temp_folder = str(Path(result_item).parent)
                folder_name = Path(temp_folder).name
                target_folder = str(Path(final_output_dir) / folder_name)
                saved_folder, _location = finalize_output(
                    temp_folder,
                    target_folder,
                ...
                if not saved_folder:
                    return None
                return str(Path(saved_folder) / Path(result_item).name)

            target_path = str(Path(final_output_dir) / Path(result_item).name)
            saved_path, _location = finalize_output(
                result_item,
                target_path,
                original_input_file=input_path,
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for step_index, intermediate_format in enumerate(conversion_path, 1):
                ...
                if progress_callback:
                    progress_callback(
                        t(
                            "conversion.progress.step_conversion_progress",
                            step=step_index,
                            total=len(conversion_path),
                            source=current_format.upper(),
                            target=intermediate_format.upper(),
                        )
                    )
                ...
                logger.info(f"步骤 {step_index}/{len(conversion_path)} 完成: {Path(result_file).name}")
                ...
                if not is_last_step:
                    ...
                    intermediate_items.append(
                        IntermediateItem(
                            kind=f"conversion_step_{step_index}_{intermediate_format}",
                            path=result_file,
                        )
                    )
                    logger.debug(f"记录中间文件: {Path(result_file).name}")
            ...
            effective_output_dir = output_dir or str(Path(input_path).parent)

            from docwen.utils.workspace_manager import finalize_output, save_intermediate_items

            # 检查最后一步是否是CSV转换
            final_format = conversion_path[-1]
            if final_format == "csv":
                # CSV转换：移动整个子文件夹到output_dir
                csv_subfolder = str(Path(final_file).parent)  # 获取CSV文件所在的子文件夹
                subfolder_name = Path(csv_subfolder).name

                # 构建目标文件夹路径
                target_subfolder = str(Path(effective_output_dir) / subfolder_name)

                final_subfolder, _location = finalize_output(
                    csv_subfolder,
                    target_subfolder,
                    original_input_file=input_path,
                )
                if not final_subfolder:
                    logger.error(f"保存CSV文件夹失败: {target_subfolder}")
                    return None
                logger.info(f"CSV文件夹已保存到: {final_subfolder}")

                # 返回output_dir中的文件路径
                final_output_path = str(Path(final_subfolder) / Path(final_file).name)
                logger.debug(f"返回CSV文件路径: {final_output_path}")
                actual_output_dir = str(Path(final_subfolder).parent)
            else:
                # 非CSV转换：移动单个文件到output_dir
                final_output_path = str(Path(effective_output_dir) / Path(final_file).name)
                saved_path, _location = finalize_output(
                    final_file,
                    final_output_path,
                    original_input_file=input_path,
                ...
                if not saved_path:
                    logger.error(f"保存最终文件失败: {final_output_path}")
                    return None
                final_output_path = saved_path
                logger.info(f"最终文件已保存: {Path(final_output_path).name}")
                actual_output_dir = str(Path(final_output_path).parent)

            should_keep = self._should_keep_intermediates()
            if should_keep and intermediate_items:
                logger.info(f"保留 {len(intermediate_items)} 个中间文件到输出目录")
                saved_items = save_intermediate_items(intermediate_items, actual_output_dir, move=True)
                for _, saved_path in saved_items:
                    logger.info(f"保留中间文件: {Path(saved_path).name}")
            elif intermediate_items:
                logger.debug(f"清理 {len(intermediate_items)} 个中间文件")
    ...
    def _execute_conversion(
        ...
        from docwen.converter.formats.spreadsheet import ods_to_xlsx, office_to_xlsx, xlsx_to_ods, xlsx_to_xls
        from docwen.utils.path_utils import generate_output_path

        logger.debug(f"执行转换: {source_format} → {target_format}, 文件: {Path(input_path).name}")
        if output_dir is None:
            output_dir = str(Path(input_path).resolve().parent)
        ...
        try:
            ...
            if source_format in ["xls", "et"] and target_format == "xlsx":
                ...
                return xlsx_to_ods(input_path=input_path, output_path=output_path, cancel_event=cancel_event)
                ...
                return ods_to_xlsx(input_path=input_path, output_path=output_path, cancel_event=cancel_event)
                ...
                return xlsx_to_xls(input_path=input_path, output_path=output_path, cancel_event=cancel_event)
            ...
            # CSV 转换支持
                ...
                folder_basename = Path(base_output).stem
                logger.debug(f"CSV转换 - 子文件夹名: {folder_basename}")

                # 提取清理后的basename（不含扩展名、时间戳、描述）
                base_filename = Path(base_output).name
                ...
                # 创建子文件夹路径
                csv_folder = Path(output_dir) / folder_basename
                ...
                csv_files = xlsx_to_csv(
                    ...
                    output_dir=str(csv_folder),  # 输出到子文件夹
                ...
                return csv_to_xlsx(csv_path=input_path, output_path=output_path)
            ...
            # 文档格式转换
                ...
                return rtf_to_docx(input_path=input_path, output_path=output_path, cancel_event=cancel_event)
                ...
                return docx_to_odt(input_path=input_path, output_path=output_path, cancel_event=cancel_event)
                ...
                return odt_to_docx(input_path=input_path, output_path=output_path, cancel_event=cancel_event)
                ...
                return docx_to_doc(input_path=input_path, output_path=output_path, cancel_event=cancel_event)
                ...
                return docx_to_rtf(input_path=input_path, output_path=output_path, cancel_event=cancel_event)
