...
class SmartConverter:
    ...
    def _convert_single_step(
        ...
        if progress_callback:
            progress_callback(t('conversion.progress.converting_source_to_target', source=source_format.upper(), target=target_format.upper()))
    ...
    def _convert_multi_step(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for step_index, intermediate_format in enumerate(conversion_path, 1):
                ...
                if progress_callback:
                    progress_callback(t('conversion.progress.step_conversion_progress', step=step_index, total=len(conversion_path), source=current_format.upper(), target=intermediate_format.upper()))
