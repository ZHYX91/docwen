...
def extract_content_from_docx(
    docx_path: str, options, output_dir: str, cancel_event: threading.Event | None = None
) -> dict:
    """
    从DOCX文件中提取内容（文本、图片、OCR）
    ...
    try:
        ...
        raise ImportError("需要python-docx库: pip install python-docx") from e

    if not Path(docx_path).exists():
        raise FileNotFoundError(f"DOCX文件不存在: {docx_path}")

    logger.info(f"开始从DOCX提取内容: {Path(docx_path).name}")

    result = {"text": "", "images": [], "ocr_results": [], "method": "external_tool"}
    ...
    try:
        ...
        if options.has_images() or options.has_ocr():
            ...
            # 创建图片保存目录
            docx_basename = Path(docx_path).stem
            images_dir = Path(output_dir) / f"{docx_basename}_images"
                ...
                if "image" in rel.target_ref:
                    try:
                        ...
                        result["images"].append(
                            {"type": "docx_embedded", "path": str(image_path), "width": width, "height": height}
                        )
        ...
        if options.has_ocr():
            ...
            for idx, img_info in enumerate(result["images"], start=1):
                ...
                logger.debug(f"OCR识别 {idx}/{len(result['images'])}: {Path(image_path).name}")
            ...
            for img_info in result["images"]:
                try:
                    ...
                    if img_path.exists():
                        ...
                        logger.debug(f"删除图片: {img_path.name}")
                except Exception as e:
