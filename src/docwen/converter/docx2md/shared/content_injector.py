...
def inject_special_content(doc):
    ...
    try:
        ...
        logger.error(f"插入特殊内容失败: {e!s}", exc_info=True)
        return doc
...
def calculate_insert_position(content):
    ...
    try:
        ...
        elif source_type.startswith(("DrawingML", "VML", "AlternateContent")):
    ...
    try:
        ...
        for content in sorted_contents:
            ...
            if insert_position < 0:
                ...
                insert_operations.append(
                    {"position": insert_position, "text": text_content, "source_type": source_type, "content": content}
                )
            else:
        ...
        logger.error(f"插入内容到文档失败: {e!s}", exc_info=True)
        return doc
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            saved_path, _ = write_temp_file_then_finalize(
                temp_dir=temp_dir,
                target_path=extracted_output_path,
                original_input_file=original_docx_path,
                writer=doc.save,
            )
        if not saved_path:
            raise OSError("保存提取文档失败")
        logger.info(f"提取文档已保存: {saved_path}")

        return saved_path

    except Exception as e:
        logger.error(f"保存提取文档失败: {e!s}")
        return None
...
def inject_textboxes_only(doc):
    ...
    try:
        ...
        logger.error(f"插入文本框内容失败: {e!s}", exc_info=True)
        return doc
...
def process_document_with_special_content(doc, original_docx_path, output_dir, mode="gongwen"):
    ...
    try:
        ...
        logger.error(f"处理文档表格、文本框内容失败: {e!s}")
        return doc, None
