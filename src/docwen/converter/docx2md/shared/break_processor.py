...
def extract_paragraph_border_info(para) -> dict:
    ...
    try:
        ...
        needs_style_check = not any(
            result[direction] is not None for direction in ["top", "bottom", "left", "right", "between"]
        )
...
class BorderGroupTracker:
    ...
    def process_paragraph(self, para, config_manager) -> list:
        ...
        if curr_has_border and not self.prev_had_border:
            ...
            if is_valid_border(curr_border_info["top"]) and md_separator:
            ...
            if self.prev_border_info and is_valid_border(self.prev_border_info["between"]) and md_separator:
                separators.append(md_separator)
                logger.debug(f"边框组内部，输出 between 分隔线: {md_separator}")

        elif (
            not curr_has_border
            and self.prev_had_border
            and self.prev_border_info
            and md_separator
            and is_valid_border(self.prev_border_info["bottom"])
        ):
    ...
    def finalize(self, config_manager) -> str | None:
        """
        文档末尾处理，返回最后的分隔线（如有）
        ...
        if is_valid_border(self.prev_border_info["bottom"]) and md_separator:
    ...
    def __str__(self):
        """调试用字符串表示"""
        return f"BorderGroupTracker(prev_had_border={self.prev_had_border}, prev_border_info={self.prev_border_info})"
