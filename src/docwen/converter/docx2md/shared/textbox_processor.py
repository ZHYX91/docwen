...
from docwen.utils.docx_utils import NAMESPACES, extract_format_from_paragraph
...
def extract_textboxes_from_document(doc):
    ...
    try:
        ...
        alternate_content_textboxes = extract_alternate_content_textboxes(
            body, doc, processed_para_elements, processed_text_contents
        )
        ...
        logger.error(f"提取文本框内容失败: {e!s}", exc_info=True)
        return []
                ...
                if para_text:
                    ...
                    if processed_text_contents is not None:
                        ...
                        if text_key in processed_text_contents:
                            logger.debug(
                                f"跳过已处理的DrawingML文本内容: 文本框{textbox_idx + 1}段落{para_idx + 1} - '{para_text[:50]}...'"
                            )
                            continue
                    ...
                    logger.debug(
                        f"从DrawingML文本框提取段落: 文本框{textbox_idx + 1}段落{para_idx + 1} - '{para_text[:50]}...'"
                    )

        return textbox_contents

    except Exception as e:
        logger.error(f"提取DrawingML文本框内容失败: {e!s}")
        return []
            ...
            for para_idx, para_element in enumerate(paragraphs):
                ...
                if para_text:
                    ...
                    if processed_text_contents is not None:
                        ...
                        if text_key in processed_text_contents:
                            logger.debug(
                                f"跳过已处理的VML文本内容: 文本框{textbox_idx + 1}段落{para_idx + 1} - '{para_text[:50]}...'"
                            )
                            continue
                        processed_text_contents.add(text_key)
                    ...
                    textbox_contents.append(textbox_data)
                    logger.debug(
                        f"从VML文本框提取段落: 文本框{textbox_idx + 1}段落{para_idx + 1} - '{para_text[:50]}...'"
                    )

        return textbox_contents

    except Exception as e:
        logger.error(f"提取VML文本框内容失败: {e!s}")
        return []
...
def extract_alternate_content_textboxes(body, doc, processed_para_elements=None, processed_text_contents=None):
    ...
    try:
            ...
            for textbox in choice_textboxes:
                ...
                for para_idx, para_element in enumerate(paragraphs):
                    ...
                    if processed_para_elements is not None:
                        ...
                        if para_id in processed_para_elements:
                            logger.debug(
                                f"跳过已处理的AlternateContent Choice段落: 文本框{textbox_idx + 1}段落{para_idx + 1}"
                            )
                            continue
                        processed_para_elements.add(para_id)
                    ...
                    if para_text:
                        ...
                        if processed_text_contents is not None:
                            ...
                            if text_key in processed_text_contents:
                                logger.debug(
                                    f"跳过已处理的AlternateContent Choice文本内容: 文本框{textbox_idx + 1}段落{para_idx + 1} - '{para_text[:50]}...'"
                                )
                                continue
                            processed_text_contents.add(text_key)
                        ...
                        for run in paragraph.runs:
                            if run.text.strip():
                                runs_data.append({"text": run.text, "fonts": {}})
                        ...
                        textbox_contents.append(textbox_data)
                        logger.debug(
                            f"从AlternateContent Choice文本框提取段落: 文本框{textbox_idx + 1}段落{para_idx + 1} - '{para_text[:50]}...'"
                        )
        ...
        logger.error(f"提取AlternateContent文本框内容失败: {e!s}")
        return []
    ...
    try:
        ...
        if paragraph_element is not None:
            ...
            if paragraph_element in all_elements:
                ...
                logger.debug(
                    f"文本框{textbox_index + 1}位置计算: 类型={textbox_type}, 段落位置{para_pos}, 插入位置{insert_position}"
                )
        ...
        logger.warning(f"获取文本框位置失败: {e!s}")
        return {"type": "paragraph", "index": -1, "element": None, "textbox_type": textbox_type}
