...
def has_gray_shading(run, wps_enabled=True, word_enabled=True):
    ...
    try:
        ...
        # Word图案填充
        return bool(
            word_enabled and val and val.startswith("pct") and (fill is None or fill.upper() in ["FFFFFF", "AUTO"])
        )
    except Exception as e:
...
def apply_format_markers(run, text, syntax_config, wps_shading_enabled=True, word_shading_enabled=True):
    ...
    if run.font.superscript:
        text = f"^{text}^" if syntax_config.get("superscript") == "extended" else f"<sup>{text}</sup>"

    # 下标
    if run.font.subscript:
        text = f"~{text}~" if syntax_config.get("subscript") == "extended" else f"<sub>{text}</sub>"

    # 删除线
    if run.font.strike:
        text = f"~~{text}~~" if syntax_config.get("strikethrough") == "extended" else f"<del>{text}</del>"
    ...
    # 斜体
    if run.italic:
        text = f"*{text}*" if syntax_config.get("italic") == "asterisk" else f"_{text}_"

    # 粗体
    if run.bold:
        text = f"**{text}**" if syntax_config.get("bold") == "asterisk" else f"__{text}__"

    # 高亮
    if run.font.highlight_color:
        text = f"=={text}==" if syntax_config.get("highlight") == "extended" else f"<mark>{text}</mark>"

    return text


def convert_paragraph_to_markdown(
    para, preserve_formatting=True, syntax_config=None, wps_shading_enabled=True, word_shading_enabled=True
):
    """
...
def convert_paragraph_to_markdown_skip_prefix(
    para, skip_chars, preserve_formatting=True, syntax_config=None, wps_shading_enabled=True, word_shading_enabled=True
):
    """
    ...
    if skip_chars <= 0:
        return convert_paragraph_to_markdown(
            para, preserve_formatting, syntax_config, wps_shading_enabled, word_shading_enabled
        )
...
def convert_paragraph_to_markdown_with_styles(
    para,
    config_manager,
    preserve_formatting=True,
    syntax_config=None,
    wps_shading_enabled=None,
    word_shading_enabled=None,
    note_extractor=None,
):
    """
    ...
    # 3. 整段字符样式检测
    if config_manager.get_code_full_paragraph_as_block() and is_full_paragraph_code_style(
        para.runs, config_manager, wps_shading_enabled, word_shading_enabled
    ):
        logger.debug("整段字符样式检测：所有 Run 都是代码样式，输出为代码块")
        text = _process_text_with_notes_for_code_block(para, note_extractor)
        return text, "code_block", True
    ...
    if not preserve_formatting:
        ...
        for run in para.runs:
            ...
            if run_style_type is None and has_gray_shading(run, wps_shading_enabled, word_shading_enabled):
                run_style_type = "code"
    ...
    md_parts = []
    current_code_text = ""
    current_code_type: str | None = None
