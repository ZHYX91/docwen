...
def _get_list_type_from_numbering(para, num_id, level):
    ...
    try:
        ...
        # 获取 abstractNumId
        abstract_num_id_elem = num_elem.find(
...
LIST_PARAGRAPH_STYLE_NAMES = ["List Paragraph", "列表段落", "ListParagraph", "List Block", "列表块", "ListBlock"]
    ...
    for idx, para in enumerate(doc.paragraphs):
        ...
        if list_type and num_id:
            if num_id not in list_ranges:
                list_ranges[num_id] = {"first": idx, "last": idx, "levels": {}}
            else:
...
class ListContextManager:
    ...
    def __init__(self, list_ranges: dict, list_style_names: list | None = None):
        """
        初始化列表上下文管理器
    ...
    def get_context_for_position(self, pos_idx: int) -> tuple | None:
        """
    ...
    def get_context_for_para(self, para_idx: int, para) -> tuple | None:
        """
