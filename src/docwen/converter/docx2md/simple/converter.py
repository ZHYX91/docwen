...
from ..shared.list_processor import preprocess_list_ranges, ListCounterManager, ListContextManager
...
def convert_docx_to_md_simple(
    ...
    参数:
        ...
        options: dict - 转换选项字典，包含以下键（可选）：
            - doc_remove_numbering: bool - 是否清除原有序号
            - doc_add_numbering: bool - 是否添加新序号
            - doc_numbering_scheme: str - 序号方案名称
    ...
    try:
        ...
        # 获取父文件名
        parent_file_name = os.path.splitext(os.path.basename(original_file_path or docx_path))[0]
...
def _extract_title_metadata(doc, docx_path: str, original_file_path: str, cancel_event) -> tuple:
    """
    提取标题和副标题元数据
    ...
    if title_indices:
        ...
        filename = os.path.splitext(os.path.basename(path_for_title))[0]
...
def _generate_markdown_content_simple(
    ...
    if note_extractor.has_notes:
        logger.info(f"提取到脚注/尾注 | 脚注: {len(note_extractor.footnotes)} 个, 尾注: {len(note_extractor.endnotes)} 个")
        ...
        if idx in tables_by_position:
            for table_info in tables_by_position[idx]:
                table_md = _process_table(
                    table_info, images_info, output_folder, parent_file_name,
                    keep_images, enable_ocr, progress_callback, cancel_event,
                    list_context_manager, list_indent_spaces, idx
                )
...
def _process_table(
    ...
    table_md = convert_table_to_md_with_images(
        table, table_index, images_info, output_folder,
        parent_file_name, config_manager
    )
    
    # 替换表格中的图片标记
    table_md = replace_image_markers_in_table(
        table_md, images_info, keep_images, enable_ocr,
        output_folder, progress_callback, cancel_event
    )
