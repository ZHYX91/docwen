...
class ParagraphHandler:
    ...
    def __init__(
        ...
        logger.debug(
            f"ParagraphHandler 初始化完成: list_indent={list_indent_spaces}, preserve_formatting={preserve_formatting}"
        )
    ...
    def process_paragraph(
        ...
        # ==== 公式检测 ====
        result, current_image_index = self._try_handle_formula(para, idx, current_image_index)
        ...
        para_text_formatted, inline_style_type, _inline_style_value = self._get_formatted_text(para)
        ...
        if style_name and style_name.startswith("Heading"):
            ...
            # 列表或普通段落
            result = self._handle_list_or_normal(para, para_text_formatted, idx, has_page_break, page_break_separator)
            content_lines.extend(result)
    ...
    def _detect_section_break(self, para, idx: int) -> str | None:
        """检测分节符"""
    ...
    def _detect_page_break(self, para, idx: int) -> tuple[bool, str | None]:
        """检测分页符"""
        if not self.config_manager.is_horizontal_rule_enabled():
            return False, None
    ...
    def _try_handle_formula(self, para, idx: int, current_image_index: int) -> tuple[list[str] | None, int]:
        """尝试处理公式段落"""
        ...
        formula_md = process_paragraph_with_formulas(
            para, preserve_formatting=self.preserve_formatting, syntax_config=self.syntax_config
        )
        if not formula_md:
            return None, current_image_index
        ...
        if list_type:
            ...
            safe_level = list_level if isinstance(list_level, int) else 0
            indent = " " * self.list_indent_spaces * safe_level
            ...
            if list_type == "bullet":
                ...
                safe_num_id = num_id if isinstance(num_id, str) else ""
                number = self.list_counter_manager.increment(safe_num_id, safe_level)
                self.list_counter_manager.reset_deeper_levels(safe_num_id, safe_level)
    ...
    def _try_handle_style(self, para, idx: int) -> list[str] | None:
        """尝试处理样式段落（代码块/引用块）"""
        ...
        if style_type == "code_block":
            ...
            # 生成引用块
            quote_level = style_value if isinstance(style_value, int) else 1
            quote_prefix = ">" * quote_level + " "
            quote_text, _, _ = convert_paragraph_to_markdown_with_styles(
                para,
                self.config_manager,
                self.preserve_formatting,
                self.syntax_config,
                self.wps_shading_enabled,
                self.word_shading_enabled,
            )
    ...
    def _try_handle_shading(self, para, idx: int) -> list[str] | None:
        """尝试处理底纹段落（兼容处理）"""
        is_code_block = has_paragraph_gray_shading(para, self.wps_shading_enabled, self.word_shading_enabled)
    ...
    def _get_formatted_text(self, para) -> tuple[str, str | None, int | None]:
        """获取段落的格式化文本"""
        para_text_raw = para.text.strip()
    ...
    def _handle_empty_paragraph(
        ...
        total_images: int,
        has_page_break: bool,
        page_break_separator: str | None,
        pending_section_break: str | None,
    ) -> tuple[list[str], int]:
        """处理空段落"""
        content_lines = []
    ...
    def _handle_heading(
        self, para, para_text_raw: str, para_text_formatted: str, style_name: str, idx: int
    ) -> list[str]:
        """处理标题段落"""
        content_lines = []
    ...
    def _apply_numbering(self, para, para_text_raw: str, level: int) -> str:
        ...
        final_text = convert_paragraph_to_markdown_skip_prefix(
            para,
            skip_chars,
            self.preserve_heading_formatting,
            self.syntax_config,
            self.wps_shading_enabled,
            self.word_shading_enabled,
        )
    ...
    def _handle_list_or_normal(
        self, para, para_text_formatted: str, idx: int, has_page_break: bool, page_break_separator: str | None
    ) -> list[str]:
        """处理列表或普通段落"""
        content_lines = []
            ...
            if run.text:
                if self.preserve_formatting and self.syntax_config:
                    text = apply_format_markers(
                        run, run.text, self.syntax_config, self.wps_shading_enabled, self.word_shading_enabled
                    )
                else:
                    text = run.text
