...
class BaseValidator:
    ...
    def create_result(self, is_valid: bool, message: str = "", details: dict | None = None) -> dict:
        """
        创建标准化的验证结果
        ...
        """
        result = {"valid": is_valid, "message": message, "details": details or {}}
...
class ElementScorer(BaseValidator):
    ...
    # 创建完整匹配模式（添加^和$）
    DOCUMENT_NUMBER_FULL_PATTERNS = tuple(f"^{pattern}\\s*$" for pattern in DOCUMENT_NUMBER_PATTERNS)
    ...
    def __init__(self):
        ...
        self.ROUND2_ELEMENTS = ["issuing_authority_mark", "printing_authority"]

        self.ROUND3_ELEMENTS = [
            "title_following",
            "body",
        ...
        self.SCORING_RULES = {
            ...
            "printing_authority": [  # 印发机关
                {
                    "condition": self.follows_element("printing_date", max_empty_paras=float("inf"), reverse=True),
                    "score": 40,
                },
    ...
    def reset_context(self):
        ...
        # 使用更简洁的字典推导式初始化所有位置
        self.element_positions = dict.fromkeys(self.ELEMENT_TYPES, -1)
    ...
    def follows_element(self, preceding_element, max_empty_paras=None, reverse=False):
        ...
        def checker(run, context):
            ...
            if max_empty_paras is None:
                logger.debug(
                    f"位置关系检查通过（不限距离）: {preceding_element}({preceding_pos}) <-> 当前({current_pos})"
                )
                return True
    ...
    def is_after_last_known_element(self, run, context):
        ...
        logger.debug(
            f"检查是否在最后一个已知元素之后: 当前={context['para_index']}, 最后已知={self.last_known_element_index}, 结果={is_after}"
        )
    ...
    def is_too_close_to_recipient_or_title(self, run, context):
        ...
        logger.debug(
            f"距离检查: 当前段落{context['para_index']}, 参考位置{reference_position}, 距离{distance}, 太近{too_close}"
        )
        return too_close
    ...
    def is_document_number_format(self, run, context):
        ...
        for pattern in self.DOCUMENT_NUMBER_FULL_PATTERNS:
            if re.match(pattern, text):
                logger.debug("发文字号格式匹配 -> True")
                return True

        logger.debug("发文字号格式不匹配 -> False")
        return False

    def starts_with_urgency_keywords(self, run, context):
        """检查是否以紧急程度关键词开头"""
        text = run["text"].strip()
        patterns = [r"^特急\b", r"^加急\b", r"^特提\b", r"^平急\b", r"^特\s*急", r"^加\s*急", r"^特\s*提", r"^平\s*急"]
    ...
    def is_body_position(self, run, context):
        ...
        if (
            self.element_positions["issuing_authority_signature"] != -1
            and context["para_index"] >= self.element_positions["issuing_authority_signature"]
        ):
            return False

        return not (
            self.element_positions["issuing_authority_signature"] == -1
            and self.element_positions["issue_date"] != -1
            and context["para_index"] >= self.element_positions["issue_date"]
        )
