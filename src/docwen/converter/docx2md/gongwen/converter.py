...
def convert_docx_to_md_gongwen(
    ...
    try:
        ...
        modified_doc, extracted_path = process_document_with_special_content(doc, docx_path, temp_dir, mode="gongwen")
...
def _identify_elements(
    ...
    logger.info("初始化公文元素评分器")

    # 收集文档上下文信息
    context = {"total_paras": len(doc.paragraphs), "para_index": 0, "para_non_empty": {}, "doc": doc}
...
def _get_run_dict(para, para_text: str) -> dict:
    ...
    return {"text": para_text, "fonts": effective_fonts, "is_chinese": contains_chinese(para_text)}


def _extract_yaml_info(doc, paragraph_elements: dict, yaml_info: dict, skip_para_indices: list, title_groups: list):
    """
    从识别的段落元素中提取YAML信息
    ...
    for idx, best_element in paragraph_elements.items():
        ...
        if best_element == "combined_id":
            parts = re.split(r"[\s\x20\t]+", para_text, maxsplit=1)
            if len(parts) >= 2:
...
def _process_title_groups(doc, paragraph_elements: dict, yaml_info: dict, skip_para_indices: list, title_groups: list):
    """处理标题组"""
    ...
    if title_groups:
        ...
        full_title = "".join(
            doc.paragraphs[i].text.replace("\n", "").replace("\r", "").strip() for i in first_title_indices
        )
