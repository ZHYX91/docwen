...
def convert(
    ...
    *,
    ...
    try:
        logger.info("=" * 60)
        logger.info(f"开始MD转DOCX: {Path(md_path).name}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 3. 在临时目录生成临时输出文件
            temp_output = str(Path(temp_dir) / "temp_output.docx")
...
def _read_and_parse_md(temp_md_path: str, original_md_path: str | None = None, temp_dir: str | None = None) -> tuple:
    ...
    try:
        ...
        md_body = process_markdown_links(md_body, source_path_for_resolve, temp_dir=temp_dir)
...
def _convert_internal(
    ...
    try:
        ...
        logger.info(
            f"正文处理完成, 生成 {len(processed_body)} 个段落, 脚注: {len(footnotes)} 个, 尾注: {len(endnotes)} 个"
        )
        ...
        processed_template_path = ensure_styles(
            original_template_path, progress_callback, cleanup_callback=cleanup_temp_dir
        )
...
def _process_docx_template(
    ...
    try:
        ...
        temp_path, warnings = docx_processor.replace_placeholders(
            doc,
            yaml_data,
            body_data,
            template_name,
        ...
        moved_path = move_file_with_retry(temp_path, output_path)
        if not moved_path:
        ...
        logger.error(f"处理DOCX模板时发生错误: {e!s}", exc_info=True)
        return False
    finally:
