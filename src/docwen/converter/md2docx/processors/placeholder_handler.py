...
# 导入 i18n 模块用于获取国际化占位符
from docwen.i18n import t_all_locales
...
# 保持向后兼容的静态变量（延迟初始化）
SPECIAL_PLACEHOLDERS = None
SPECIAL_PLACEHOLDER_LIST = None


def _init_special_placeholders():
    """延迟初始化特殊占位符变量"""
    global SPECIAL_PLACEHOLDERS, SPECIAL_PLACEHOLDER_LIST
    if SPECIAL_PLACEHOLDERS is None:
        # 获取 body 占位符的所有变体
        body_variants = _get_all_placeholder_variants("body")
        # 创建兼容格式的字典（使用第一个变体作为值）
        SPECIAL_PLACEHOLDERS = {
            "正文": body_variants[0] if body_variants else "{{正文}}",
        }
        # 添加附件说明（目前未国际化）
        SPECIAL_PLACEHOLDERS["附件说明"] = "{{附件说明}}"
        SPECIAL_PLACEHOLDER_LIST = _get_special_placeholder_list()
        # 添加附件说明到列表
        SPECIAL_PLACEHOLDER_LIST.append("{{附件说明}}")
        logger.debug("特殊占位符初始化完成: %s", SPECIAL_PLACEHOLDER_LIST)
...
def _find_placeholder_type(ph_text: str) -> str:
    """
    根据占位符文本查找其类型名称
