...
from docwen.utils import docx_utils, text_utils
from docwen.utils.path_utils import ensure_dir_exists_or_raise, generate_named_output_path
...
def process_docx_file(docx_path, yaml_data):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # 处理所有XML文件
            xml_files = [str(p) for p in Path(tmpdir).rglob("*.xml")]
            ...
            for xml_path in xml_files:
                try:
                    ...
                    logger.error(f"处理XML文件失败: {xml_path}, 错误: {e!s}")
        ...
        logger.error(f"处理DOCX文件失败: {e!s}", exc_info=True)
        raise
...
def repack_docx(source_dir, output_path):
    ...
    try:
        # 确保输出目录存在
        ensure_dir_exists_or_raise(str(Path(output_path).parent))
        ...
        with zipfile.ZipFile(output_path, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            for file_path in source_dir_path.rglob("*"):
                if not file_path.is_file():
                    continue
                arcname = str(file_path.relative_to(source_dir_path))
                zipf.write(file_path, arcname)
                logger.debug(f"添加文件到ZIP: {arcname}")

        logger.info(f"成功重新打包DOCX文件: {output_path}")
    except Exception as e:
        logger.error(f"重新打包DOCX文件失败: {e!s}")
        raise
...
def process_xml_file(xml_path, yaml_data):
    ...
    try:
        # 读取XML内容
        with Path(xml_path).open(encoding="utf-8") as f:
            content = f.read()
        ...
        # 保存修改后的XML
        with Path(xml_path).open("w", encoding="utf-8") as f:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for xml_path in xml_files:
                try:
                    ...
                    logger.error(f"全局替换失败: {xml_path}, 错误: {e!s}")

            # 创建输出文件名
            base_name = Path(docx_path).stem
            output_name = f"{base_name}_global_replaced.docx"
            output_path = str(Path(docx_path).parent / output_name)
            ensure_dir_exists_or_raise(str(Path(output_path).parent))
