...
def get_heading_style_name(doc, level: int) -> str:
    ...
    try:
        for style in doc.styles:
            if style.type.value == 1 and style.style_id == target_style_id:  # 只检查段落样式
                logger.debug(f"找到标题样式(按ID): ID='{style.style_id}', Name='{style.name}'")
                return style.name
    except Exception as e:
        logger.warning(f"遍历样式查找ID时出错: {e!s}")
...
def get_code_block_style_name(doc) -> str:
    """
    获取代码块段落样式名称（支持国际化）
    ...
    style_name = _style("code_block")
    logger.debug(f"代码块样式名: '{style_name}'")
    return style_name


def get_quote_style_name(doc, level: int) -> str:
    """
    获取引用块段落样式名称（支持国际化）
    ...
    style_name = _style(style_key)
    logger.debug(f"引用样式名（{level}级）: '{style_name}'")
    return style_name


def get_formula_block_style_name(doc) -> str:
    """
    获取公式块段落样式名称（支持国际化）
    ...
    """
    # 直接返回当前语言的样式名（样式已在 injector.py 中注入）
    style_name = _style("formula_block")
...
def get_image_paragraph_style_name(doc=None) -> str:
    style_name = _style("image_paragraph")
