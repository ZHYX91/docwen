...
from docwen.translation import get_style_format, t, t_locale
...
class _InjectedStyleResolver:
    def get_injection_name(self, name: str) -> str:
        key = f"styles.{name}"
        return t(key, default=t_locale(key, "zh_CN", default=name))

    def get_style_format(self, style_key: str) -> dict:
        return get_style_format(style_key)
...
def _replace_base_style_refs(template: str, base_ids: dict) -> str:
    ...
    if base_ids.get("Normal") != "Normal":
        result = result.replace('w:val="Normal"', f'w:val="{base_ids["Normal"]}"')

    # 替换 basedOn 中的 DefaultParagraphFont 引用
    if base_ids.get("DefaultParagraphFont") != "DefaultParagraphFont":
        result = result.replace('w:val="DefaultParagraphFont"', f'w:val="{base_ids["DefaultParagraphFont"]}"')
...
def _inject_style_if_needed(
    ...
    if conflicting_style is not None:
        ...
        if conflicting_type != expected_type:
            ...
            logger.warning(
                f"发现冲突样式 '{target_name}'（类型={conflicting_type}，期望={expected_type}），将删除并重新注入"
            )
            ...
            if progress_callback:
                progress_callback(
                    t(
                        "conversion.progress.fixing_conflict_style",
                        name=target_name,
                        from_type=conflicting_type,
    ...
    if final_style_id != default_style_id:
        final_template = style_template.replace(f'w:styleId="{default_style_id}"', f'w:styleId="{final_style_id}"')
...
def ensure_styles(template_path, progress_callback=None, cleanup_callback=None):
    ...
    # ==== 获取国际化样式配置 ====
    # 根据当前语言设置获取本地化的样式名
    localized_configs = get_localized_style_configs(_InjectedStyleResolver())
    logger.debug("已获取国际化样式配置")
    ...
    for name, style_type, style_id, template in code_configs:
        ...
        injected, _, conflict = _inject_style_if_needed(
            root, existing_styles, existing_ids, name, style_type, style_id, adjusted_template, progress_callback
        )
        if injected:
            injected_count += 1
    ...
    if progress_callback and injected_count > 0:
        if conflict_count > 0:
            progress_callback(
                t("conversion.progress.styles_injected_with_conflicts", count=injected_count, conflicts=conflict_count)
            )
        else:
    ...
    try:
        ...
        with (
            zipfile.ZipFile(template_path, "r") as zf_in,
