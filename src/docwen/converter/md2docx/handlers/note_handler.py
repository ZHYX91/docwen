...
def _create_formatted_runs(p: etree._Element, text: str) -> None:
    ...
    for segment in segments:
        ...
        seg_formats = segment["formats"] or []
        ...
        if seg_formats:
            ...
            for fmt in seg_formats:
                ...
                if fmt == "italic" or fmt == "bold_italic":
                    etree.SubElement(rPr, f"{{{WORD_NS}}}i")
                if fmt == "strikethrough":
                    etree.SubElement(rPr, f"{{{WORD_NS}}}strike")
                if fmt == "underline":
        ...
        hint_east_asia = contains_east_asian(seg_text) if seg_text else False
        if hint_east_asia and "code" not in seg_formats:
            if rPr is None:
                rPr = etree.SubElement(r, f"{{{WORD_NS}}}rPr")
            rFonts = rPr.find(f"{{{WORD_NS}}}rFonts")
            if rFonts is None:
                rFonts = etree.SubElement(rPr, f"{{{WORD_NS}}}rFonts")
            rFonts.set(f"{{{WORD_NS}}}hint", "eastAsia")
...
def create_footnote_element(
    footnote_id: int, content: str, text_style_id: str = "FootnoteText", ref_style_id: str = "FootnoteReference"
) -> etree._Element:
    """
    创建脚注内容XML元素
...
def create_endnote_element(
    endnote_id: int, content: str, text_style_id: str = "EndnoteText", ref_style_id: str = "EndnoteReference"
) -> etree._Element:
    """
    创建尾注内容XML元素
...
class NoteContext:
    ...
    def __init__(self, footnotes: dict[str, str] | None = None, endnotes: dict[str, str] | None = None):
        """
        初始化脚注/尾注上下文
    ...
    def get_footnote_word_id(self, md_id: str) -> int | None:
        """
        ...
        if md_id not in self._footnote_id_map:
            ...
            footnote_elem = create_footnote_element(word_id, content, self.footnote_text_style, self.footnote_ref_style)
    ...
    def get_endnote_word_id(self, md_id: str) -> int | None:
        """
        ...
        if clean_id not in self._endnote_id_map:
            ...
            endnote_elem = create_endnote_element(word_id, content, self.endnote_text_style, self.endnote_ref_style)
    ...
    def create_footnote_ref_run(self, md_id: str) -> etree._Element | None:
        """
        创建脚注引用run
    ...
    def create_endnote_ref_run(self, md_id: str) -> etree._Element | None:
        """
        创建尾注引用run
...
def write_notes_to_docx(docx_path: str, note_ctx: "NoteContext") -> None:
    ...
    logger.info(
        f"写入脚注/尾注 | 脚注: {len(note_ctx.footnote_elements)} 个, 尾注: {len(note_ctx.endnote_elements)} 个"
    )

    try:
        # 创建临时文件
        temp_dir = tempfile.mkdtemp()
        temp_path = str(Path(temp_dir) / "temp_notes.docx")

        with zipfile.ZipFile(docx_path, "r") as zf_in, zipfile.ZipFile(temp_path, "w", zipfile.ZIP_DEFLATED) as zf_out:
            for item in zf_in.infolist():
                content = zf_in.read(item.filename)

                # 处理 footnotes.xml
                if item.filename == "word/footnotes.xml" and note_ctx.footnote_elements:
                    content = _append_notes_to_xml(content, note_ctx.footnote_elements, "footnote")
                    logger.debug(f"已向 footnotes.xml 添加 {len(note_ctx.footnote_elements)} 个脚注")

                # 处理 endnotes.xml
                elif item.filename == "word/endnotes.xml" and note_ctx.endnote_elements:
                    content = _append_notes_to_xml(content, note_ctx.endnote_elements, "endnote")
                    logger.debug(f"已向 endnotes.xml 添加 {len(note_ctx.endnote_elements)} 个尾注")
        ...
        replace_file_atomic(temp_path, docx_path)
...
def _append_notes_to_xml(xml_content: bytes, note_elements: list[etree._Element], note_type: str) -> bytes:
    """
...
def process_text_with_note_references(
    text: str, paragraph, fonts: dict, formatting_mode: str, doc, note_ctx: "NoteContext"
) -> None:
    """
    ...
    for match in references:
        ...
        if start > last_end:
            ...
            add_formatted_text_to_paragraph(paragraph, before_text, fonts, formatting_mode, doc=doc)
    ...
    if last_end < len(text):
        ...
        add_formatted_text_to_paragraph(paragraph, after_text, fonts, formatting_mode, doc=doc)
