...
from docwen.translation import t, t_locale

from .formula_handler import is_formula_supported, process_paragraph_formulas
...
def create_word_table(doc, table_data: dict, fonts: dict | None = None):
    """
    创建 Word 表格
    ...
    for col_idx, header in enumerate(headers):
        ...
        add_formatted_text_to_paragraph(paragraph, header_text, {}, table_header_formatting_mode, doc=doc)
        ...
        for col_idx, cell_value in enumerate(row_data):
            if col_idx < len(headers):  # 确保不超出列数
                ...
                if has_formula:
                    ...
                    add_formatted_text_to_paragraph(paragraph, cell_text, {}, formatting_mode, doc=doc)
                    logger.debug(
                        f"填充数据单元格 [{row_idx + 1},{col_idx}]: {cell_value[:30] if cell_value else ''}..."
                    )
...
def get_table_style_name() -> str:
    ...
    if builtin_key == "table_grid":
        style_name = _style("table_grid")
        logger.debug(f"使用网格表样式: {style_name}")
        return style_name
    else:
        style_name = _style("three_line_table")
...
def get_table_content_style_name() -> str:
    ...
    返回:
        str: 表格内容样式名称（国际化）
    """
    style_name = _style("table_content")
