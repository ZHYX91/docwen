...
def _inject_style_to_omml(omml_tree, style_id: str) -> None:
    ...
    def ensure_rPr_and_style(parent_elem):
        ...
        # 如果没有 w:rPr，创建一个
        if rPr is None:
            rPr = etree.Element(f"{{{w_ns}}}rPr", nsmap={"w": w_ns})
            # w:rPr 应该是第一个子元素
        ...
        rStyle = None
        for child in rPr:
            if child.tag == f"{{{w_ns}}}rStyle":
                rStyle = child
                break

        # 如果没有 w:rStyle，创建一个
        if rStyle is None:
            rStyle = etree.Element(f"{{{w_ns}}}rStyle", nsmap={"w": w_ns})
            # w:rStyle 应该是 w:rPr 的第一个子元素
            rPr.insert(0, rStyle)

        # 设置样式 ID
        rStyle.set(f"{{{w_ns}}}val", style_id)

    # 遍历所有 m:r 元素（公式文本 run）
    for m_r in omml_tree.iter(f"{{{m_ns}}}r"):
        ensure_rPr_and_style(m_r)

    # 遍历所有 m:ctrlPr 元素（控制属性）
    for ctrlPr in omml_tree.iter(f"{{{m_ns}}}ctrlPr"):
...
def process_paragraph_formulas(
    paragraph, para_text: str, fonts: dict | None = None, formatting_mode: str = "apply", doc=None, note_ctx=None
) -> bool:
    """
    ...
    def add_text_segment(text_segment):
        ...
        if note_ctx and note_ctx.has_notes and "[^" in text_segment:
            # 包含脚注/尾注引用，使用专门的处理函数
            process_text_with_note_references(text_segment, paragraph, fonts or {}, formatting_mode, doc, note_ctx)
        else:
            add_formatted_text_to_paragraph(paragraph, text_segment, fonts or {}, formatting_mode, doc=doc)
    ...
    try:
        ...
        for formula in formulas:
            ...
            # 插入公式
            insert_formula_to_paragraph(paragraph, formula["latex"], formula["is_inline"])
...
def restore_formulas_from_placeholders(text: str, formulas: list[dict]) -> str:
    ...
    for idx, formula in enumerate(formulas):
        ...
        latex_syntax = f"${formula['latex']}$" if formula["is_inline"] else f"$$\n{formula['latex']}\n$$"
