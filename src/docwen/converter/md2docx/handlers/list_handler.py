...
class ListFormatManager:
    ...
    def __init__(self, doc):
        ...
        logger.debug(
            f"ListFormatManager 初始化: next_abstractNumId={self._next_abstract_num_id}, next_numId={self._next_num_id}"
        )
    ...
    def build_format_tables(self):
        ...
        abstract_nums = []
        for abstract_num in self._numbering_root.findall(f".//{{{WORD_NS}}}abstractNum"):
        ...
        for abstract_num_id, abstract_num in abstract_nums:
            for lvl in abstract_num.findall(f".//{{{WORD_NS}}}lvl"):
                ...
                tentative = lvl.get(f"{{{WORD_NS}}}tentative")
                if tentative == "1":
                ...
                if is_ordered_feature and level not in self._ordered_formats:
                    ...
                    logger.debug(
                        f"收集最新有效有序格式: level={level}, from abstractNumId={abstract_num_id}, fmt={num_fmt}"
                    )
    ...
    def _extract_level_definition(self, lvl_elem) -> dict:
        ...
        # start
        start_elem = lvl_elem.find(f".//{{{WORD_NS}}}start")
        if start_elem is not None:
            level_def["start"] = start_elem.get(f"{{{WORD_NS}}}val", "1")

        # numFmt
        num_fmt_elem = lvl_elem.find(f".//{{{WORD_NS}}}numFmt")
        if num_fmt_elem is not None:
            level_def["numFmt"] = num_fmt_elem.get(f"{{{WORD_NS}}}val", "decimal")

        # lvlText
        lvl_text_elem = lvl_elem.find(f".//{{{WORD_NS}}}lvlText")
        if lvl_text_elem is not None:
            level_def["lvlText"] = lvl_text_elem.get(f"{{{WORD_NS}}}val", "")

        # lvlJc
        lvl_jc_elem = lvl_elem.find(f".//{{{WORD_NS}}}lvlJc")
        if lvl_jc_elem is not None:
            level_def["lvlJc"] = lvl_jc_elem.get(f"{{{WORD_NS}}}val", "left")

        # pPr/ind（缩进）
        pPr = lvl_elem.find(f".//{{{WORD_NS}}}pPr")
        if pPr is not None:
            ind = pPr.find(f".//{{{WORD_NS}}}ind")
            if ind is not None:
                level_def["ind_left"] = ind.get(f"{{{WORD_NS}}}left")
                level_def["ind_hanging"] = ind.get(f"{{{WORD_NS}}}hanging")

        # rPr（字符格式，用于 bullet 的字体）
        rPr = lvl_elem.find(f".//{{{WORD_NS}}}rPr")
        if rPr is not None:
            rFonts = rPr.find(f".//{{{WORD_NS}}}rFonts")
            if rFonts is not None:
                level_def["rFonts_ascii"] = rFonts.get(f"{{{WORD_NS}}}ascii")
                level_def["rFonts_hAnsi"] = rFonts.get(f"{{{WORD_NS}}}hAnsi")
    ...
    def _fill_missing_with_presets(self):
        ...
        if self._ordered_formats is None:
            self._ordered_formats = {}
        if self._unordered_formats is None:
            self._unordered_formats = {}
    ...
    def create_list_definition(self, level_types: dict[int, str]) -> str | None:
        """
        根据需求拼接并创建列表定义
        ...
        if self._numbering_root is None:
            ...
            return None
        if self._next_abstract_num_id is None:
            self._next_abstract_num_id = 0
        if self._next_num_id is None:
            self._next_num_id = 1
        if self._ordered_formats is None or self._unordered_formats is None:
            self.build_format_tables()
        assert self._ordered_formats is not None
        assert self._unordered_formats is not None

        # 拼接各级别格式
        levels = []
        for level in range(9):
            type_ = level_types.get(level, "ordered")
        ...
        # 2. 所有 <w:num> 元素
        first_num = self._numbering_root.find(f"{{{WORD_NS}}}num")
    ...
    def _create_abstract_num(self, abstract_num_id: int, levels: list[dict]):
        ...
        """
        abstract_num = etree.Element(f"{{{WORD_NS}}}abstractNum")
        abstract_num.set(f"{{{WORD_NS}}}abstractNumId", str(abstract_num_id))

        # nsid（必需，随机值）
        nsid = etree.SubElement(abstract_num, f"{{{WORD_NS}}}nsid")
        nsid.set(f"{{{WORD_NS}}}val", format(hash(str(abstract_num_id)) & 0xFFFFFFFF, "08X"))

        # multiLevelType
        multi_level = etree.SubElement(abstract_num, f"{{{WORD_NS}}}multiLevelType")
        multi_level.set(f"{{{WORD_NS}}}val", "hybridMultilevel")
    ...
    def _create_lvl(self, level: int, level_def: dict):
        ...
        """
        lvl = etree.Element(f"{{{WORD_NS}}}lvl")
        lvl.set(f"{{{WORD_NS}}}ilvl", str(level))

        # start
        start = etree.SubElement(lvl, f"{{{WORD_NS}}}start")
        start.set(f"{{{WORD_NS}}}val", level_def.get("start", "1"))

        # numFmt
        num_fmt = etree.SubElement(lvl, f"{{{WORD_NS}}}numFmt")
        num_fmt.set(f"{{{WORD_NS}}}val", level_def.get("numFmt", "decimal"))

        # lvlText
        lvl_text = etree.SubElement(lvl, f"{{{WORD_NS}}}lvlText")
        lvl_text.set(f"{{{WORD_NS}}}val", level_def.get("lvlText", f"%{level + 1}."))

        # lvlJc
        lvl_jc = etree.SubElement(lvl, f"{{{WORD_NS}}}lvlJc")
        lvl_jc.set(f"{{{WORD_NS}}}val", level_def.get("lvlJc", "left"))

        # pPr（段落属性，包含缩进）
        pPr = etree.SubElement(lvl, f"{{{WORD_NS}}}pPr")
        ind = etree.SubElement(pPr, f"{{{WORD_NS}}}ind")
        ind.set(f"{{{WORD_NS}}}left", level_def.get("ind_left", str(BASE_INDENT + INDENT_INCREMENT * level)))
        ind.set(f"{{{WORD_NS}}}hanging", level_def.get("ind_hanging", str(INDENT_INCREMENT)))

        # rPr（字符属性，仅用于 bullet）
        if level_def.get("numFmt") == "bullet":
            rPr = etree.SubElement(lvl, f"{{{WORD_NS}}}rPr")
            rFonts = etree.SubElement(rPr, f"{{{WORD_NS}}}rFonts")
            # 如果有字体设置则使用，否则不设置（使用 Unicode 字符）
            if level_def.get("rFonts_ascii"):
                rFonts.set(f"{{{WORD_NS}}}ascii", level_def["rFonts_ascii"])
                rFonts.set(f"{{{WORD_NS}}}hAnsi", level_def.get("rFonts_hAnsi", level_def["rFonts_ascii"]))
    ...
    def _create_num(self, num_id: int, abstract_num_id: int):
        ...
        """
        num = etree.Element(f"{{{WORD_NS}}}num")
        num.set(f"{{{WORD_NS}}}numId", str(num_id))

        abstract_num_id_elem = etree.SubElement(num, f"{{{WORD_NS}}}abstractNumId")
        abstract_num_id_elem.set(f"{{{WORD_NS}}}val", str(abstract_num_id))
    ...
    for item in list_items:
        ...
        if level in level_types:
            if level_types[level] != type_:
                ...
                logger.warning(f"列表级别 {level} 类型冲突: 已有 {level_types[level]}, 新遇 {type_}, 统一为 ordered")
