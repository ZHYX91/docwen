...
def _add_run_with_emoji_support(paragraph, text: str, base_fonts: dict | None = None):
    """
...
def apply_formats_to_run(
    run,
    formats: list,
    doc=None,
    code_font: str = "Consolas",
    code_bg_color: str = "D9D9D9",
...
def add_formatted_text_to_paragraph(
    paragraph, text: str, base_fonts: dict | None = None, mode: str = "apply", doc=None
):
    """
        ...
        for part_text, is_emoji in emoji_parts:
            ...
            if seg_formats and (not is_emoji or "code" not in seg_formats):
                apply_formats_to_run(
                    run, seg_formats, doc=doc, code_font=code_font, code_bg_color=code_bg_color, override_style=False
                )
...
def add_formatted_text_to_heading(paragraph, text: str, mode: str = "apply", doc=None):
    ...
    """
    from docwen.config.config_manager import config_manager
    from docwen.utils.docx_utils import extract_format_from_paragraph_style, set_font_family
