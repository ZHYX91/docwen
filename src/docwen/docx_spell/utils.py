...
def copy_run_formatting(source_run, target_run):
    ...
    try:
        ...
        logger.warning(f"复制run格式时出现警告: {e!s}")
...
def copy_paragraph_format(source_para, target_para):
    ...
    try:
        ...
        logger.warning(f"深拷贝段落格式(w:pPr)时出现警告: {e!s}")
        ...
        if not run_splits:
            ...
            split_plan.append(
                {
                    ...
                    "splits": [{"text": run.text, "is_error": error_index is not None, "error_index": error_index}],
                }
            )
        else:
            # 需要拆分这个run
            ...
            for split_pos in run_splits:
                ...
                if relative_pos > last_pos:
                    ...
                    splits.append({"text": text_part, "is_error": error_index is not None, "error_index": error_index})
            ...
            split_plan.append({"original_run": run, "run_start_pos": current_pos, "splits": splits})
...
def rebuild_paragraph_with_splits(old_paragraph, split_plan, doc):
    ...
    try:
        ...
        logger.error(f"重建段落失败: {e!s}", exc_info=True)
        raise
