...
# CLI临时语言设置（不保存到配置）
_cli_locale: str | None = None


def init_cli_locale(lang: str | None = None):
    """
    初始化CLI语言设置
    ...
    if lang:
        ...
        try:
            ...
            valid_locales = []
            for loc in I18nManager().get_available_locales():
                code = loc.get("code")
                if isinstance(code, str) and code:
                    valid_locales.append(code)
        except Exception:
...
def cli_t(key: str, default: str | None = None, **kwargs) -> str:
    """
    CLI翻译函数
