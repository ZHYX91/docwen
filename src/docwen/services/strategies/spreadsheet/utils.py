...
def preprocess_table_file(
    ...
    if actual_format in ["xls", "et", "ods"]:
        logger.info(f"检测到{actual_format.upper()}格式，从副本转换为XLSX: {Path(temp_input).name}")

        try:
            from docwen.converter.formats.spreadsheet import ods_to_xlsx, office_to_xlsx
            ...
            if converted_path:
                logger.info(f"{actual_format.upper()}转XLSX成功: {Path(converted_path).name}")
                return SpreadsheetPreprocessResult(
                    processed_file=converted_path,
                    actual_format=actual_format,
                    intermediates=[IntermediateItem(kind="preprocessed_xlsx", path=converted_path)],
                    preprocess_chain=[*preprocess_chain, "xlsx"],
                )
            else:
            ...
            raise RuntimeError(f"{actual_format.upper()}转XLSX失败: {e}") from e
...
_preprocess_table_file = preprocess_table_file
