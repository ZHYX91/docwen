...
class ImageToPdfStrategy(BaseStrategy):
    ...
    def execute(
        ...
        try:
            ...
            effective_format = actual_format
            if effective_format is None:
                from docwen.utils.file_type_utils import detect_actual_file_format

                effective_format = detect_actual_file_format(file_path)
                logger.debug(f"自动检测图片文件格式(用于临时副本与后续处理): {effective_format}")
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                temp_input = prepare_input_file(file_path, temp_dir, effective_format or "jpg")
                logger.debug(f"已创建输入副本: {Path(temp_input).name}")

                # 步骤2：检测并预处理特殊格式（从副本处理）
                file_to_convert = self._preprocess_image(temp_input, temp_dir, None, effective_format)

                # 记录中间文件（如果有格式转换）
                intermediate_file = None
                if effective_format in ["bmp", "heic", "heif"] and file_to_convert != temp_input:
                    intermediate_file = file_to_convert
                    logger.debug(f"记录中间文件: {Path(intermediate_file).name}")
                ...
                inputs_for_pdf = file_to_convert
                if is_multipage_tiff(file_to_convert):
                    if progress_callback:
                        progress_callback(t("conversion.progress.processing_images"))
                    pages = extract_tiff_pages(
                        file_path=file_to_convert,
                        output_dir=temp_dir,
                        actual_format=effective_format or "tiff",
                        progress_callback=progress_callback,
                    )
                    inputs_for_pdf = [path for _, path in pages]

                if layout_fun is None:
                    # 原图嵌入模式：不传递layout_fun参数
                    pdf_bytes = img2pdf.convert(inputs_for_pdf)
                else:
                    # A4/A3模式：传递layout_fun参数
                    pdf_bytes = img2pdf.convert(inputs_for_pdf, layout_fun=layout_fun)
                if pdf_bytes is None:
                    raise RuntimeError("img2pdf 未返回有效的 PDF 数据")

                # 生成输出文件名
                description = get_image_format_description(effective_format) if effective_format else "fromImage"
                output_filename = Path(
                    generate_output_path(
                        file_path, section="", add_timestamp=True, description=description, file_type="pdf"
                    )
                ).name

                # 保存PDF到临时目录
                temp_output = str(Path(temp_dir) / output_filename)
                with Path(temp_output).open("wb") as f:
                    f.write(pdf_bytes)

                # 准备最终输出路径
                target_output_path = str(Path(output_dir) / output_filename)
                ...
                if not saved_path:
                    raise RuntimeError("保存 PDF 失败")
                output_path = saved_path

                # 处理中间文件保留
                should_keep = self._should_keep_intermediates()
                if should_keep:
                    logger.info("保留中间文件，移动临时目录中的中间产物到输出目录")
    ...
    def _preprocess_image(
        self, file_path: str, temp_dir: str, cancel_event=None, actual_format: str | None = None
    ) -> str:
        """
        ...
        if actual_format == "bmp":
            logger.info(f"检测到BMP格式，转换为PNG: {Path(file_path).name}")

            try:
                from docwen.converter.formats.image import bmp_to_png

                output_path = str(Path(temp_dir) / f"{Path(file_path).stem}.png")
                converted_path = bmp_to_png(file_path, output_path=output_path)
                logger.info(f"BMP转PNG成功: {Path(converted_path).name}")
        ...
        # HEIC/HEIF格式需要转换为PNG
            ...
            logger.info(f"检测到{actual_format.upper()}格式，转换为PNG: {Path(file_path).name}")

            try:
                from docwen.converter.formats.image import heic_to_png

                output_path = str(Path(temp_dir) / f"{Path(file_path).stem}.png")
                converted_path = heic_to_png(file_path, output_path=output_path)
                logger.info(f"{actual_format.upper()}转PNG成功: {Path(converted_path).name}")
                return converted_path

            except Exception as e:
                logger.error(f"{actual_format.upper()}转PNG失败: {e}")
                raise RuntimeError(f"{actual_format.upper()}转PNG失败: {e}") from e
