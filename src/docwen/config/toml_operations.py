...
# 进程内 TOML 读取缓存：避免重复 I/O 与解析（按 mtime 自动失效）
_TOML_DICT_CACHE: dict[str, tuple[float, dict[str, Any]]] = {}


def _invalidate_toml_cache(filepath: str | Path) -> None:
    try:
        key = str(Path(filepath))
        _TOML_DICT_CACHE.pop(key, None)
    except Exception:
        return
...
def read_toml_file(filepath: str | Path) -> dict[str, Any]:
    """
    读取 TOML 文件内容并转换为字典
    ...
    if not filepath.is_file():
        safe_log.debug("TOML文件不存在: %s", filepath)
        return {}
    ...
    try:
        ...
        try:
            ...
            mtime = -1.0
        cached = _TOML_DICT_CACHE.get(cache_key)
        ...
        # 转换为普通字典
        result = tomlkit_to_dict(parsed)
        _TOML_DICT_CACHE[cache_key] = (mtime, result)
        return result

    except Exception as e:
        safe_log.error("解析TOML文件失败: %s | 错误: %s", filepath, str(e))
        _invalidate_toml_cache(filepath)
        return {}


def write_toml_file(filepath: str | Path, data: dict[str, Any]) -> bool:
    """
    将字典数据写入 TOML 文件
...
def read_toml_document(filepath: str | Path) -> TOMLDocument | None:
    """
...
def write_toml_document(filepath: str | Path, doc: TOMLDocument) -> bool:
    """
...
def update_toml_value(filepath: str | Path, section: str, key: str, value: Any, create_missing: bool = True) -> bool:
    """
...
def tomlkit_to_dict(toml_data: TOMLDocument | Any) -> Any:
    """
    将 tomlkit 对象转换为普通字典
...
def get_toml_value(filepath: str | Path, section: str, key: str, default: Any = None) -> Any:
    """
    获取 TOML 文件中的特定值
    ...
    try:
        ...
        safe_log.error("获取TOML值失败: %s -> %s.%s | 错误: %s", filepath, section, key, str(e))
        return default


def validate_toml_syntax(filepath: str | Path) -> bool:
    """
    验证 TOML 文件语法是否正确
...
def extract_inline_comments(filepath: str | Path, section: str) -> dict[str, str]:
    """
...
def save_mapping_with_comments(
    filepath: str | Path, section: str, mapping_data: dict[str, list], comments_data: dict[str, str]
) -> bool:
    """
    保存映射数据和注释到 TOML 文件
    ...
    try:
        ...
        success = write_toml_document(filepath, doc)
        if success:
            safe_log.info("保存映射数据和注释成功: %s -> %s (共 %d 条)", filepath, section, len(mapping_data))

        return success

    except Exception as e:
        safe_log.error("保存映射数据和注释失败: %s -> %s | 错误: %s", filepath, section, str(e))
        return False
