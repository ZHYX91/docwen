...
class OutputConfigMixin:
    ...
    def get_output_manifest_config(self) -> dict[str, Any]:
        return self.get_output_config_block().get("manifest", {})
    ...
    def get_save_manifest(self) -> bool:
        manifest_config = self.get_output_manifest_config()
        save_to_output = manifest_config.get("save_to_output", False)
        safe_log.debug("保存 manifest 设置: %s", save_to_output)
        return save_to_output

    def get_mask_manifest_input_path(self) -> bool:
        manifest_config = self.get_output_manifest_config()
        mask = manifest_config.get("mask_input_path", True)
        safe_log.debug("manifest 输入路径脱敏设置: %s", mask)
        return mask
