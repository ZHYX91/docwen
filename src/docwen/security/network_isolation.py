...
def block_socket_module():
    ...
    socket_any: Any = socket
    socket_any.connect = blocked_connect
    socket_any.create_connection = blocked_create_connection
    socket_any.gethostbyname = blocked_gethostbyname
    socket_any.getaddrinfo = blocked_getaddrinfo
...
def block_urllib_module():
    """阻断urllib模块的所有网络功能"""
    urllib_any: Any = urllib.request
    urllib_any.urlopen = lambda *args, **kwargs: (_ for _ in ()).throw(
        NetworkIsolationError("urllib.urlopen已被完全阻断 - 此应用不允许任何网络访问")
    )

    urllib_any.URLopener = BlockedURLOpener
    urllib_any.FancyURLopener = BlockedURLOpener

    # 安装全局的URL打开器阻断
    urllib_any.install_opener(BlockedURLOpener())
...
def block_ssl_module():
    ...
    try:
        ...
        ssl_any: Any = ssl
        ssl_any.wrap_socket = blocked_wrap_socket
...
def initialize_network_isolation():
    ...
    try:
        ...
        logger.error(f"网络隔离初始化过程中发生错误: {e!s}")
        raise
...
def get_network_isolation_status():
    ...
    socket_any: Any = socket
    http_any: Any = http.client
    ssl_any: Any = ssl
    status = {
        "socket_blocked": socket_any.socket == BlockedSocket,
        "http_blocked": http_any.HTTPConnection == BlockedHTTPConnection,
        ...
        "ssl_blocked": hasattr(ssl_any, "wrap_socket") and ssl_any.wrap_socket.__module__ != "ssl",
    }
