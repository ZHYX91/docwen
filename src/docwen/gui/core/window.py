...
from docwen.gui.components.selector_manager import SelectorManager
from docwen.i18n import t  # 导入翻译函数
from docwen.utils.dpi_utils import ScalableMixin, initialize_dpi_manager
...
class MainWindow(ScalableMixin):
    ...
    def __init__(self, root: tb.Window, config_manager: Any, initial_file_path: str | None = None):
        ...
        def _parse_env_scale(value: str | None) -> float | None:
            if value is None:
                return None
            raw = value.strip()
            if not raw:
            ...
            try:
                if raw.endswith("%"):
                    number = float(raw[:-1].strip())
                    scale_value = number / 100.0
                else:
                    scale_value = float(raw)
            except ValueError:
                return None
            ...
            if scale_value == 0:
                return None
            if scale_value > 10:
                scale_value = scale_value / 100.0
            return scale_value

        # 初始化DPI管理器
        env_scaling_enabled = _parse_env_bool(os.environ.get("DOCWEN_DPI_SCALING"))
        env_forced_scale = _parse_env_scale(os.environ.get("DOCWEN_UI_SCALE"))
        config_scaling_enabled = self.config_manager.is_dpi_scaling_enabled()
        config_ui_scale = self.config_manager.get_ui_scale()
        config_forced_scale = None if not config_ui_scale else float(config_ui_scale)

        scaling_enabled = env_scaling_enabled if env_scaling_enabled is not None else config_scaling_enabled
        forced_scale = env_forced_scale if env_forced_scale is not None else config_forced_scale

        if env_scaling_enabled is not None or env_forced_scale is not None:
            logger.info(
                "检测到环境变量DPI覆盖: DOCWEN_DPI_SCALING=%s, DOCWEN_UI_SCALE=%s",
                env_scaling_enabled,
                env_forced_scale,
            )
        elif config_forced_scale is not None or not config_scaling_enabled:
            logger.info(
                "检测到配置DPI覆盖: enable_dpi_scaling=%s, ui_scale=%s", config_scaling_enabled, config_ui_scale
            )

        initialize_dpi_manager(root, scaling_enabled=scaling_enabled, forced_scale=forced_scale)
        logger.info("⏱ DPI初始化+配置读取 耗时: %.3fs", time.perf_counter() - _init_t0)
        ...
        # 获取窗口位置（配置以100%缩放下的逻辑值保存；运行时使用缩放后的像素值）
        self.window_x = self.config_manager.get_center_panel_screen_x()
        self.window_y = self.scale(self.config_manager.get_window_y())

        # 位置跟踪变量（用于实时跟踪窗口位置）
        self.center_panel_screen_x: int | None = None  # 跟踪中栏在屏幕上的X坐标
        self.current_y: int | None = self.window_y  # 跟踪窗口的Y坐标（像素）
        ...
        self.selected_template: tuple[str, str] | None = None
        self.location_button: tb.Button | None = None
        ...
        # 设置主窗口引用，供子组件访问（必须在布局构建之前，否则子组件无法获取回调）
        cast(Any, self.main_container)._main_window = self
        ...
        logger.info("⏱ _create_layout_with_builder 耗时: %.3fs", time.perf_counter() - _t_layout)
        ...
        logger.info("⏱ MainWindow.__init__ 总耗时: %.3fs", time.perf_counter() - _init_t0)
    ...
    def _on_window_configure(self, event):
        ...
        try:
            ...
            if self.current_y is None:
                self.current_y = window_y
                logger.debug(f"窗口Y坐标已初始化: {window_y}")
            elif abs(window_y - self.current_y) > 2:
                self.current_y = window_y
    ...
    def _setup_window_properties(self):
        ...
        self.root.withdraw()
        logger.info("⏱ withdraw 耗时: %.3fs", time.perf_counter() - _t0)
        ...
        logger.info("⏱ geometry/minsize/resizable 耗时: %.3fs", time.perf_counter() - _t1)

        _t2 = time.perf_counter()
        self.root.title(t("common.app_name") + t("common.offline_version"))  # 国际化窗口标题
        logger.info("⏱ title (含首次i18n加载) 耗时: %.3fs", time.perf_counter() - _t2)

        _t3 = time.perf_counter()
        self._set_window_position()
        logger.info("⏱ _set_window_position 耗时: %.3fs", time.perf_counter() - _t3)

        _t4 = time.perf_counter()
        self._set_window_theme()
        logger.info("⏱ _set_window_theme 耗时: %.3fs", time.perf_counter() - _t4)

        _t5 = time.perf_counter()
        self._set_window_transparency()
        logger.info("⏱ _set_window_transparency 耗时: %.3fs", time.perf_counter() - _t5)

        _t6 = time.perf_counter()
        self.root.deiconify()
        logger.info("⏱ deiconify 耗时: %.3fs", time.perf_counter() - _t6)

        logger.info("⏱ _setup_window_properties 总耗时: %.3fs", time.perf_counter() - _t0)
    ...
    def _calculate_window_position(self) -> tuple[int, int]:
        ...
        if center_panel_x is None:
            center_panel_x = self.scale(self.config_manager.get_center_panel_screen_x())
    ...
    def _set_window_transparency(self):
        ...
        try:
            ...
            logger.warning(f"设置窗口透明度失败: {e!s}")
    ...
    def _create_layout_with_builder(self):
        ...
        if (
            self.center_frame is None
            or self.template_frame is None
            or self.batch_frame is None
            or self.bottom_frame is None
            or self.tabbed_batch_file_list is None
        ):
            raise RuntimeError("主窗口布局组件创建失败")
    ...
    def _setup_grid_layout(self):
        ...
        self._apply_grid_layout_weights()

        # 初始化时只显示中栏（在列2）
        assert self.center_frame is not None
        self.center_frame.grid(row=0, column=2, sticky="nsew", padx=(self.margin, self.margin))
        logger.debug("5列统一布局配置完成")

    def _apply_grid_layout_weights(self):
        expand_side_panels = False
        try:
            expand_side_panels = bool(self.config_manager.should_expand_side_panels())
        except Exception:
            expand_side_panels = False

        triple_mode = self.batch_panel_visible and self.template_panel_visible
        layout = self._compute_grid_layout(
            expand_side_panels=expand_side_panels,
            triple_mode=triple_mode,
            left_width=self.left_width,
            center_width=self.center_width,
            right_width=self.right_width,
        )
        for col, cfg in layout.items():
            self.main_container.grid_columnconfigure(col, **cfg)

    @staticmethod
    def _compute_grid_layout(
        expand_side_panels: bool, triple_mode: bool, left_width: int, center_width: int, right_width: int
    ) -> dict:
        if expand_side_panels and triple_mode:
            return {
                0: {"weight": 0, "minsize": 0},
                1: {"weight": 2, "minsize": int(left_width)},
                2: {"weight": 4, "minsize": int(center_width)},
                3: {"weight": 2, "minsize": int(right_width)},
                4: {"weight": 0, "minsize": 0},
            }
        return {
            0: {"weight": 1, "minsize": 0},
            1: {"weight": 0, "minsize": 0},
            2: {"weight": 4, "minsize": int(center_width)},
            3: {"weight": 0, "minsize": 0},
            4: {"weight": 1, "minsize": 0},
        }
    ...
    def show_template_selector(self):
        ...
        if not self.template_panel_visible:
            ...
            self.center_panel_screen_x = int(center_panel_screen_x)
            logger.debug(f"基于当前位置更新中栏屏幕坐标: {self.center_panel_screen_x}")

            self.template_panel_visible = True

            # 在5列布局中，模板面板固定在列3
            assert self.template_frame is not None
    ...
    def show_conversion_panel(self, category: str, current_format: str, file_path: str | None = None):
        """
        显示格式转换面板
    ...
    def _update_window_geometry(self):
        ...
        logger.debug(
            f"窗口几何已更新: 位置({x}, {y}), 尺寸({width}x{height}), 可调整大小: {can_resize_horizontally}x纵向"
        )
    ...
    def show_action_panel(self):
        """显示操作面板"""
        logger.debug("显示操作面板")
        assert self.bottom_frame is not None
    ...
    def show_batch_panel(self):
        ...
        if not self.batch_panel_visible:
            ...
            # 在5列布局中，批量面板固定在列1
            assert self.batch_frame is not None
    ...
    def set_transient_status(self, key: str, message: str, message_type: str = "secondary", ttl_ms: int | None = None):
        if self.status_bar:
            self.status_bar.set_transient_message(key, message, message_type, ttl_ms)

    def clear_transient_status(self, key: str):
        if self.status_bar:
            self.status_bar.clear_transient_message(key)

    def clear_all_transient_status(self):
        if self.status_bar:
    ...
    def _save_gui_state(self):
        ...
        使用跟踪的位置值，确保准确性
        """
        if not self.remember_gui_state:
            return
        try:
            # 使用跟踪的中栏X坐标
            ...
            center_panel_screen_x_at_100_percent = self.unscale(int(center_panel_screen_x))
            window_y_at_100_percent = self.unscale(int(window_y))
            height_at_100_percent = self.unscale(int(current_height))

            # 保存配置
            self.config_manager.update_config_value(
                "gui_config", "window", "center_panel_screen_x", int(center_panel_screen_x_at_100_percent)
            )
            self.config_manager.update_config_value("gui_config", "window", "window_y", int(window_y_at_100_percent))
            self.config_manager.update_config_value("gui_config", "window", "default_height", height_at_100_percent)

            logger.info(
                f"GUI状态已保存: 中栏X({center_panel_screen_x_at_100_percent}), 窗口Y({window_y_at_100_percent}), 原始高度{height_at_100_percent}"
            )
        except Exception as e:
            logger.error(f"保存GUI状态失败: {e!s}")

    def _on_status_bar_location_clicked(self, file_path: str):
        if file_path and Path(file_path).exists():
            self.event_handler.logic.open_and_select_file(file_path)
    ...
    def _on_batch_files_added(self, added_files, failed_files):
        ...
        if hasattr(self, "tabbed_file_manager") and self.tabbed_file_manager:
            if not self.tabbed_batch_file_list:
                logger.warning("批量文件列表未初始化，无法刷新UI")
                return
            # 获取当前激活的选项卡类别
    ...
    def _show_error(self, message: str, title: str | None = None):
        ...
        参数:
            message: 错误消息内容
            title: 错误弹窗标题，默认为翻译后的"错误"
        """
        if title is None:
            title = t("common.error")  # 使用翻译后的错误标题
        ...
        try:
            ...
            if self.status_bar:
                self.status_bar.add_message(f"{t('common.error')}: {message}", "danger")
        ...
        try:
            ...
            if success:
                ...
                if not preview_only:
                    ...
                    self._show_error(
                        f"主题 '{theme_name}' 不可用。\n可用主题: {', '.join(available_themes)}", "主题错误"
                    )
    ...
    def setup_table_action_panel(self, file_path: str, file_list: list[str] | None = None):
        """设置电子表格文件操作面板"""
        if self.action_panel:
