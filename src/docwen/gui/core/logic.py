...
class MainWindowLogic:
    ...
    def handle_action(self, action_type: str, file_path: str, options: Dict[str, Any]):
        ...
        from docwen.utils.gui_utils import show_info_dialog

        status_info = get_expiration_status()
        if status_info.status == ExpirationStatus.EXPIRED:
            show_info_dialog(
                t('components.status_bar.trial_expired_title'),
                t('components.status_bar.trial_expired_message'),
                alert=True
            )
    ...
    def _run_in_background(self, operation_id: str, action_type: str, file_path: str, 
        ...
        try:
            ...
            if not file_category or not actual_format:
                from docwen.utils.file_type_utils import get_actual_file_category, detect_actual_file_format
            ...
            # 3. 查找策略类 (使用延迟导入的 get_strategy)
            try:
                get_strategy = _get_strategy_func()
            ...
            # 对于Markdown文件，添加校对选项（直接传递布尔字典）
            if file_category == 'text':
                strategy_options['proofread_options'] = self._extract_proofread_options(options)
            
            # 对于文档校对，添加校对选项（直接传递布尔字典）
            ...
            if cancel_event.is_set():
                ...
                message_to_return = result.message or (str(result.error) if result.error else t('messages.operation_failed'))
    ...
    def _check_queue(self):
        ...
        try:
            ...
            if message == t('conversion.messages.operation_cancelled'):
                self.main_window.add_status_message(t('components.status_bar.operation_cancelled'), "warning", False)
            elif success:
                show_location = not self.main_window.batch_panel_visible
                self.main_window.add_status_message(message or t('components.status_bar.operation_success'), "success", show_location, output_path)
                if output_path:
                ...
                show_error_dialog(t('components.status_bar.conversion_failed'), message or t('conversion.messages.conversion_failed_check_log'))
                self.main_window.add_status_message(t('components.status_bar.conversion_failed'), "danger", False)
    ...
    def _update_animation(self, dot_count=1):
        ...
        dots = "." * dot_count
        self.main_window.add_status_message(t('components.status_bar.processing_wait', dots=dots), "secondary", False)
        
        next_dot_count = (dot_count % 3) + 1
        self.main_window.root.after(300, self._update_animation, next_dot_count)
    
    def _extract_proofread_options(self, options: Dict[str, Any]) -> Dict[str, bool]:
        """
        从选项字典中提取校对选项
        
        参数:
            options: 包含校对选项的字典，可能包含:
                - symbol_pairing: bool
                - symbol_correction: bool
                - typos_rule: bool
                - sensitive_word: bool
    ...
    def _batch_process_files(self, files: list, action_type: str, 
        ...
        try:
            ...
            for i, file_path in enumerate(files, 1):
                ...
                self.main_window.root.after(0,
                    lambda c=i, tot=total, fn=os.path.basename(file_path): 
                        self.main_window.add_status_message(t('components.status_bar.processing_file', current=c, total=tot, filename=fn), "secondary", False)
                )
            ...
            message = t('components.status_bar.batch_completed', success=success_count, failed=failed_count)
        ...
        try:
            ...
            if target_format and actual_format:
                ...
                if is_same and not is_image_compress:
                    return (True, t('components.status_bar.skipped_same_format', format=actual_format.upper()), None, action_type)
            ...
            if cancel_event.is_set():
                return (False, t('conversion.messages.operation_cancelled'), None, action_type)
            
            message_to_return = result.message or (str(result.error) if result.error else t('messages.operation_failed'))
            return (result.success, message_to_return, result.output_path, action_type)

        except Exception as e:
            return (False, t('messages.processing_exception', error=str(e)), None, action_type)
    ...
    def _process_merge_images_to_tiff(self, file_list: list, options: Dict[str, Any], cancel_event: threading.Event):
        ...
        try:
            if cancel_event.is_set():
                self.processing_queue.put((False, t('conversion.messages.operation_cancelled'), None, "merge_images_to_tiff"))
                return
