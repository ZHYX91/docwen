...
class ThemeManager:
    ...
    def __init__(self):
        ...
        self._root: tk.Tk | None = None  # 主窗口引用
        self._current_theme: str | None = None  # 当前主题名称
    ...
    def apply_theme(self, theme_name: str, preview_only: bool = False) -> bool:
        ...
        try:
            ...
            try:
                available_themes = cast(Any, self._root).style.theme_names()
            ...
            # 先处理所有待处理的事件（如组件销毁）
            logger.debug("处理待处理的UI事件...")
            try:
                self._root.update_idletasks()
                logger.debug("✓ 待处理事件已处理完成")
            except tk.TclError as e:
                logger.debug(f"处理待处理事件时出错（可能窗口已销毁）: {e}")

            # 只对主窗口应用主题 - 所有子窗口会自动继承
            logger.info("开始应用主题到主窗口...")
            ...
            logger.error(f"❌ 应用主题时发生异常: {type(e).__name__}: {e!s}")
        ...
        try:
            ...
            if has_style:
                ...
                try:
                    # 获取当前主题（用于日志）
                    try:
                        style = cast(Any, window).style
                        current_theme = style.theme_use()
                        logger.debug(f"  >> 当前主题: {current_theme}")
                    except Exception:
                        logger.debug("  >> 无法获取当前主题")
                    ...
                    if not theme_applied:
                        logger.debug("  >> ✗ 主题未成功应用")
                        return False

                    # 更新界面
                    logger.debug("  >> 调用 window.update_idletasks()...")
                    try:
                        window.update_idletasks()
                        logger.debug("  >> ✓ update_idletasks() 完成")
                ...
                if self._root and hasattr(self._root, "style"):
                    logger.debug("  >> 根窗口有 style 属性")
                    try:
                        logger.debug(f"  >> 调用根窗口的 style.theme_use('{theme_name}')...")
                        cast(Any, self._root).style.theme_use(theme_name)
            ...
            if "bad window path name" not in error_msg and "has been destroyed" not in error_msg:
                ...
                logger.debug("  >> 窗口已销毁，跳过应用主题")
            return False
        except Exception as e:
            logger.warning(f"  >> 应用主题到窗口时发生异常: {type(e).__name__}: {e!s}")
            logger.exception("  >> 异常详情:")
            return False
    ...
    def get_available_themes(self) -> list[str]:
        ...
        try:
            return list(cast(Any, self._root).style.theme_names())
        except Exception as e:
            logger.error(f"获取主题列表失败: {e!s}")
            return []
