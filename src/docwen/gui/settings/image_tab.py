...
# OCR语言配置值和翻译键映射
OCR_LANGUAGE_CONFIG_VALUES = ["auto", "chinese", "chinese_cht", "english", "japanese", "korean", "latin", "cyrillic"]
...
class ImageTab(BaseSettingsTab):
    ...
    def _create_extraction_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.image.extraction_section"), SectionStyle.DANGER
        )
    ...
    def _create_compress_options_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.image.compress_section"), SectionStyle.INFO
        )
        ...
        # 说明文本
        desc_label = tb.Label(frame, text=t("settings.image.compress_desc"), bootstyle="secondary")
        ...
        lossless_frame.pack(fill="x", pady=(0, scale(8)))
        ...
        create_info_icon(lossless_frame, t("settings.image.compress_lossless_tooltip"), "info").pack(
        ...
        create_info_icon(limit_frame, t("settings.image.compress_limit_size_tooltip"), "info").pack(
            side="left", padx=(scale(5), 0)
        )

        # 大小限制输入
        input_frame = tb.Frame(frame)
        input_frame.pack(fill="x", pady=(scale(10), 0))
        ...
        tb.Entry(input_frame, textvariable=self.size_limit_var, width=10).pack(side="left", padx=(scale(5), scale(5)))

        self.size_unit_var = tk.StringVar(value=size_unit)
        tb.Combobox(input_frame, textvariable=self.size_unit_var, values=["KB", "MB"], state="readonly", width=5).pack(
            side="left"
        )
    ...
    def _create_pdf_tiff_options_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.image.pdf_tiff_section"), SectionStyle.WARNING
        )
        ...
        # PDF质量设置
        pdf_label = tb.Label(
            frame, text=t("settings.image.pdf_size_label"), font=(self.small_font, self.small_size, "bold")
        )
        ...
        pdf_radio_frame.pack(fill="x", pady=(0, scale(15)))
        ...
        tb.Radiobutton(
            ...
            bootstyle="primary",
        ).pack(side="left", padx=(0, scale(15)))
        ...
        # TIFF模式设置
        tiff_label = tb.Label(
            frame, text=t("settings.image.tiff_mode_label"), font=(self.small_font, self.small_size, "bold")
        )
        tiff_label.pack(anchor="w", pady=(0, scale(10)))
        ...
        smart_frame.pack(fill="x", pady=(0, scale(8)))
        ...
        create_info_icon(smart_frame, t("settings.image.tiff_keep_alpha_tooltip"), "info").pack(
            side="left", padx=(scale(5), 0)
        )
        ...
        create_info_icon(rgb_frame, t("settings.image.tiff_no_alpha_tooltip"), "info").pack(
            side="left", padx=(scale(5), 0)
        )
    ...
    def _create_ocr_language_selector(self, parent: tk.Widget, initial_value: str):
        ...
        label = tb.Label(label_frame, text=t("settings.image.ocr_language_label"), bootstyle="secondary")
        label.pack(side="left")

        info = create_info_icon(label_frame, t("settings.image.ocr_language_tooltip"), "info")
    ...
    def apply_settings(self) -> bool:
        ...
        try:
            ...
            for key, value in settings.items():
                if not self.config_manager.update_config_value("conversion_defaults", "image", key, value):
                    success = False
