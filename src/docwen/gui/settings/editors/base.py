...
class BaseEditorDialog(tb.Toplevel, ScalableMixin, ABC):
    ...
    def __init__(self, parent: tk.Widget, config_manager: Any, on_save: Callable[[], None] | None = None):
        ...
        self.current_item_id: str | None = None  # 当前选中的项目ID
        self.checkbox_vars: dict[str, tk.BooleanVar] = {}  # 复选框变量

        # UI组件引用
        self.status_label: tb.Label | None = None
        self.item_list_frame: tb.Frame | None = None
        self.item_canvas: tk.Canvas | None = None
        self.name_var: tk.StringVar | None = None
        self.name_entry: tb.Entry | None = None
        self.description_text: tb.Text | None = None
    ...
    def _create_item_list(self, parent: tk.Widget):
        ...
        list_frame.pack(fill="both", expand=True, pady=(0, self.scale(10)))
    ...
    def _create_action_buttons(self, parent: tk.Widget):
        ...
        action_frame.pack(fill="x", pady=(0, self.scale(5)))
        ...
        tb.Button(
            action_frame, text=f"➕ {t('editors.common.add')}", command=self._on_add_item, bootstyle="success", width=10
        ).grid(row=0, column=0, padx=self.scale(2))

        tb.Button(
            action_frame, text=f"📋 {t('editors.common.copy')}", command=self._on_copy_item, bootstyle="info", width=10
        ).grid(row=0, column=1, padx=self.scale(2))

        tb.Button(
            action_frame,
            text=f"➖ {t('editors.common.delete')}",
            command=self._on_delete_item,
            bootstyle="danger",
            width=10,
        ).grid(row=0, column=2, padx=self.scale(2))
        ...
        tb.Button(
            order_frame,
            text=f"⬆ {t('editors.common.move_up')}",
        ...
        tb.Button(
            order_frame,
            text=f"⬇ {t('editors.common.move_down')}",
    ...
    def _create_bottom_buttons(self, parent: tb.Frame):
        ...
        button_frame.pack(fill="x", pady=(self.scale(10), 0))

        # 左侧提示
        self.status_label = tb.Label(
            button_frame, text="", bootstyle="secondary", font=(self.small_font, self.small_size)
        )
        self.status_label.pack(side="left")

        # 右侧按钮
        tb.Button(
            button_frame, text=t("editors.common.cancel"), command=self._on_close, bootstyle="secondary", width=10
        ).pack(side="right", padx=(self.scale(5), 0))

        tb.Button(
            button_frame, text=t("editors.common.save"), command=self._on_save, bootstyle="success", width=10
        ).pack(side="right")
    ...
    def _mark_changed(self):
        ...
        if not self.has_unsaved_changes:
            ...
            if self.status_label:
                self.status_label.configure(text=t("editors.common.unsaved_changes_hint"))
    ...
    def _generate_unique_id(self, prefix: str = "custom", existing_ids: set[str] | None = None) -> str:
        """
        生成唯一ID
    ...
    def _on_move_up(self):
        ...
        # 交换位置
        self.item_order[index], self.item_order[index - 1] = self.item_order[index - 1], self.item_order[index]
    ...
    def _on_copy_item(self):
        """复制当前项目"""
        if not self.current_item_id:
            self._show_info(t("editors.common.prompt"), t("editors.common.select_item_first"))
            return
    ...
    def _on_delete_item(self):
        ...
        if self._is_item_system(item):
            self._show_info(t("editors.common.cannot_delete"), t("editors.common.system_item_hint"))
            return
        ...
        confirm_msg = t("editors.common.confirm_delete_message").replace("{name}", item_name)
        if not self._show_confirm(t("editors.common.confirm_delete"), confirm_msg):
            return
    ...
    def _on_close(self):
        """关闭对话框"""
        if self.has_unsaved_changes and not self._show_confirm(
            t("editors.common.confirm_close"), t("editors.common.unsaved_close_message")
        ):
            return
