...
class NumberingScheme:
    ...
    def get_display_name(self) -> str:
        """获取显示名称（支持国际化）"""
        if self.name_key:
            return t(f"editors.numbering_add.names.{self.name_key}", default=self.name or self.scheme_id)
        return self.name or self.scheme_id

    def get_display_description(self) -> str:
        """获取显示说明（支持国际化）"""
        if self.description_key:
            return t(f"editors.numbering_add.descriptions.{self.description_key}", default=self.description)
    ...
    def to_config(self) -> dict:
        ...
        if self.description_key:
            config["description_key"] = self.description_key
        else:
...
class FormatValidator:
    ...
    def __init__(self, number_styles: dict[str, dict] | None = None):
        """初始化验证器"""
        if number_styles:
...
class HeadingNumberingEditorDialog(BaseEditorDialog):
    ...
    def __init__(self, parent: tk.Widget, config_manager: Any, on_save: Callable[[], None] | None = None):
        ...
        # UI组件引用
        self.default_scheme_combo: tb.Combobox | None = None
        self.default_scheme_var: tk.StringVar | None = None
        self.level_entries: dict[int, tb.Entry] = {}
        self.level_status_labels: dict[int, tb.Label] = {}
        self.preview_text: tb.Text | None = None

        # 格式验证器
        self.validator: FormatValidator | None = None

        # 延迟更新定时器ID
        self._preview_update_id: str | None = None
    ...
    def _load_config_data(self):
        ...
        try:
            ...
            self._show_error(
                t("editors.numbering_add.load_failed"), t("editors.numbering_add.load_failed_message", error=e)
            )
    ...
    def _create_default_scheme_selector(self, parent: tk.Widget):
        ...
        frame.pack(fill="x", pady=(0, self.scale(10)))

        tb.Label(frame, text=t("editors.numbering_add.default_scheme"), font=(self.small_font, self.small_size)).pack(
            side="left"
        )
        ...
        self.default_scheme_combo = tb.Combobox(
            frame, textvariable=self.default_scheme_var, values=scheme_names, state="readonly", width=18
        )
        self.default_scheme_combo.pack(side="left", padx=(self.scale(5), 0), fill="x", expand=True)
    ...
    def _create_item_editor(self, parent: tb.Frame):
        ...
        tb.Label(name_frame, text=t("editors.common.name"), width=12, anchor="e").pack(side="left")

        self.name_var = tk.StringVar()
        name_entry = tb.Entry(name_frame, textvariable=self.name_var, width=40)
        name_entry.pack(side="left", padx=(self.scale(5), 0), fill="x", expand=True)
        ...
        tb.Label(desc_frame, text=t("editors.common.description"), width=12, anchor="e").pack(side="left", anchor="n")

        description_text = tb.Text(desc_frame, width=40, height=2, font=(self.small_font, self.small_size))
        description_text.pack(side="left", padx=(self.scale(5), 0), fill="x", expand=True)
        description_text.bind("<KeyRelease>", lambda e: self._on_content_changed())
    ...
    def _create_level_config_section(self, parent: tb.Frame):
        ...
        frame.pack(fill="both", expand=True, pady=(0, self.scale(10)))
        ...
        tb.Button(
            help_frame, text=t("editors.numbering_add.format_help"), command=self._show_format_help, bootstyle="link"
        ).pack(side="right")

    def _create_level_row(self, parent: tb.Frame, level: int):
        """创建单个级别的配置行"""
        row_frame = tb.Frame(parent)
        row_frame.pack(fill="x", pady=self.scale(2))
        ...
        entry = tb.Entry(row_frame, textvariable=entry_var, width=35, font=(self.small_font, self.small_size))
        entry.pack(side="left", padx=(self.scale(5), self.scale(5)), fill="x", expand=True)
        ...
        # 快速插入按钮
        tb.Button(
            row_frame, text="📋", command=lambda lv=level: self._show_placeholder_menu(lv), bootstyle="link", width=3
        ).pack(side="left")

        # 验证状态标签
        status_label = tb.Label(row_frame, text="", width=3, font=(self.small_font, self.small_size))
        status_label.pack(side="left")
        self.level_status_labels[level] = status_label

    def _create_preview_section(self, parent: tb.Frame):
        """创建预览区域"""
        frame = tb.Labelframe(parent, text=t("editors.numbering_add.realtime_preview"), padding=self.scale(5))
        frame.pack(fill="x")
        frame.configure(height=self.scale(self.PREVIEW_HEIGHT))
        frame.pack_propagate(False)

        preview_text = tb.Text(frame, height=8, font=(self.small_font, self.small_size), state="disabled", wrap="word")
    ...
    def _can_delete_item(self, item: NumberingScheme) -> bool:
        ...
        if item.scheme_id == self.default_scheme_id:
            self._show_info(
                t("editors.numbering_add.cannot_delete"), t("editors.numbering_add.cannot_delete_default_scheme")
            )
            return False
        return True
    ...
    def _update_preview(self):
        ...
        try:
            ...
            max_show_level = min(max_defined_level, 7)
            for level in range(1, max_show_level + 1):
                format_str = scheme.levels.get(level, "")
                indent = "  " * (level - 1)
    ...
    def _show_placeholder_menu(self, level: int):
        ...
        # 当前级别占位符
        menu.add_command(label=t("editors.numbering_add.menu_current_level", level=level), state="disabled")
        ...
        for dec, desc in decorations:
            menu.add_command(label=f"  {dec}  {desc}", command=lambda d=dec, e=entry: self._insert_placeholder(e, d))
        ...
        if level > 1:
            ...
            menu.add_command(
                label=f"  {hierarchical}", command=lambda p=hierarchical, e=entry: self._insert_placeholder(e, p)
            )
    ...
    def _on_save(self):
        ...
        try:
            ...
            from docwen.config.toml_operations import read_toml_document, write_toml_document
            ...
            for scheme_id, scheme in self.items.items():
                ...
                if scheme.description_key:
                    scheme_table["description_key"] = scheme.description_key
                    if "description" in scheme_table:
                        del scheme_table["description"]
                else:
                    scheme_table["description"] = scheme.description
                    if "description_key" in scheme_table:
                        del scheme_table["description_key"]
            ...
            self._show_error(
                t("editors.numbering_add.save_failed"), t("editors.numbering_add.save_failed_with_error", error=e)
            )
