...
class NumberingPatternsEditorDialog(BaseEditorDialog):
    ...
    def __init__(self, parent: tk.Widget, config_manager: Any, on_save: Callable[[], None] | None = None):
        ...
        # UI组件引用
        self.regex_var: tk.StringVar | None = None
        self.regex_entry: tb.Entry | None = None
        self.test_input_var: tk.StringVar | None = None
        self.test_input_entry: tb.Entry | None = None
        self.test_result_label: tb.Label | None = None
        self.test_status_label: tb.Label | None = None

        # 延迟测试定时器ID
        self._test_update_id: str | None = None
    ...
    def _load_config_data(self):
        ...
        try:
            ...
            self._show_error(
                t("editors.numbering_clean.load_failed"), t("editors.numbering_clean.load_failed_message", error=e)
            )
    ...
    def _create_item_editor(self, parent: tb.Frame):
        ...
        tb.Label(name_frame, text=t("editors.numbering_clean.name"), width=12, anchor="e").pack(side="left")
        ...
        tb.Label(desc_frame, text=t("editors.numbering_clean.description"), width=12, anchor="e").pack(
            side="left", anchor="n"
        )
        ...
        tb.Label(regex_frame, text=t("editors.numbering_clean.regex"), width=12, anchor="e").pack(side="left")

        self.regex_var = tk.StringVar()
        regex_entry = tb.Entry(
            regex_frame, textvariable=self.regex_var, width=40, font=(self.small_font, self.small_size)
        )
        regex_entry.pack(side="left", padx=(self.scale(5), 0), fill="x", expand=True)
    ...
    def _create_regex_test(self, parent: tb.Frame):
        ...
        tb.Label(input_frame, text=t("editors.numbering_clean.input"), width=12, anchor="e").pack(side="left")

        self.test_input_var = tk.StringVar()
        test_input_entry = tb.Entry(input_frame, textvariable=self.test_input_var, width=40)
        test_input_entry.pack(side="left", padx=(self.scale(5), 0), fill="x", expand=True)
        self.test_input_entry = test_input_entry
        ...
        tb.Label(result_frame, text=t("editors.numbering_clean.result"), width=12, anchor="e").pack(side="left")

        self.test_result_label = tb.Label(result_frame, text="", font=(self.small_font, self.small_size), anchor="w")
        self.test_result_label.pack(side="left", padx=(self.scale(5), 0), fill="x", expand=True)

        # 测试状态
        status_frame = tb.Frame(frame)
        status_frame.pack(fill="x")

        tb.Label(status_frame, text=t("editors.numbering_clean.status"), width=12, anchor="e").pack(side="left")

        self.test_status_label = tb.Label(status_frame, text="", font=(self.small_font, self.small_size))
        self.test_status_label.pack(side="left", padx=(self.scale(5), 0))

        # 示例按钮
        tb.Button(
            status_frame, text=t("editors.numbering_clean.load_example"), command=self._load_example, bootstyle="link"
        ).pack(side="right")
    ...
    def _create_placeholder_info(self, parent: tb.Frame):
        ...
        # 创建可滚动的文本区域
        info_text = tb.Text(frame, height=8, font=(self.small_font, self.small_size), state="normal", wrap="word")
    ...
    def _run_regex_test(self):
        ...
        try:
            ...
            if result != test_input:
                ...
                if self.test_status_label:
                    self.test_status_label.configure(
                        text=f"✅ {t('editors.numbering_clean.match_success')}", bootstyle="success"
                    )
            else:
                # 无匹配
                if self.test_result_label:
                ...
                if self.test_status_label:
                    self.test_status_label.configure(
                        text=f"⚠️ {t('editors.numbering_clean.no_match')}", bootstyle="warning"
                    )
            ...
            if self.test_status_label:
                self.test_status_label.configure(
                    text=f"❌ {t('editors.numbering_clean.regex_error')}: {e}", bootstyle="danger"
                )
        except Exception as e:
            if self.test_result_label:
                self.test_result_label.configure(text="")
            if self.test_status_label:
                self.test_status_label.configure(
                    text=f"❌ {t('editors.numbering_clean.test_error')}: {e}", bootstyle="danger"
                )
    ...
    def _load_example(self):
        ...
        for example in rule.examples:
            menu.add_command(label=example, command=lambda ex=example: self._set_test_input(ex))
    ...
    def _on_save(self):
        ...
        try:
            ...
            rules_table = doc_any["rules"]

            # 删除不再存在的规则（保护系统规则）
            existing_rule_ids = list(rules_table.keys())
            current_rule_ids = set(self.items.keys())
            for rule_id in existing_rule_ids:
                if rule_id not in current_rule_ids and not rules_table[rule_id].get("is_system", False):
            ...
            if success:
                ...
                self._show_error(
                    t("editors.numbering_clean.save_failed"), t("editors.numbering_clean.save_failed_message")
                )

        except Exception as e:
            logger.error(f"保存配置失败: {e}", exc_info=True)
            self._show_error(
                t("editors.numbering_clean.save_failed"), t("editors.numbering_clean.save_failed_with_error", error=e)
            )
