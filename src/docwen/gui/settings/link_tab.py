...
class LinkTab(BaseSettingsTab):
    ...
    def _create_markdown_link_format_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.link.format_section"), SectionStyle.PRIMARY
        )
    ...
    def _create_non_embed_links_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.link.non_embed_section"), SectionStyle.SUCCESS
        )

        # 说明文本
        desc = tb.Label(frame, text=t("settings.link.non_embed_desc"), bootstyle="secondary")
        desc.pack(anchor="w", pady=(0, scale(10)))
        self.bind_label_wraplength(desc, frame, min_wraplength=scale(320))
    ...
    def _create_embed_links_section(self):
        ...
        frame = self.create_section_frame(self.scrollable_frame, t("settings.link.embed_section"), SectionStyle.WARNING)

        # 说明文本
        desc = tb.Label(frame, text=t("settings.link.embed_desc"), bootstyle="secondary")
        desc.pack(anchor="w", pady=(0, scale(10)))
        self.bind_label_wraplength(desc, frame, min_wraplength=scale(320))
        ...
        cell_00.grid(row=0, column=0, sticky="nsew", padx=(0, scale(5)))

        cell_01 = tb.Frame(grid_frame)  # 第1行右列
        cell_01.grid(row=0, column=1, sticky="nsew", padx=(scale(5), 0))

        cell_10 = tb.Frame(grid_frame)  # 第2行左列
        cell_10.grid(row=1, column=0, sticky="nsew", padx=(0, scale(5)))

        cell_11 = tb.Frame(grid_frame)  # 第2行右列
        cell_11.grid(row=1, column=1, sticky="nsew", padx=(scale(5), 0))
    ...
    def _create_spinbox_in_column(
        ...
        # 创建Spinbox
        spinbox = tb.Spinbox(
            container, from_=from_val, to=to_val, textvariable=variable, bootstyle="secondary", command=command
        )
        spinbox.pack(fill="x")
