...
class FormattingTab(BaseSettingsTab):
    ...
    def _create_docx_to_md_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.formatting.docx_to_md_section"), SectionStyle.PRIMARY
        )
        ...
        format_title_frame.pack(fill="x", pady=(0, scale(10)))
        ...
        format_info = create_info_icon(format_title_frame, t("settings.formatting.format_processing_tooltip"), "info")
        ...
        first_column.grid(row=0, column=0, sticky="nsew", padx=(0, scale(5)))

        # 第二列
        second_column = tb.Frame(columns_frame)
        second_column.grid(row=0, column=1, sticky="nsew", padx=(scale(5), scale(5)))

        # 第三列
        third_column = tb.Frame(columns_frame)
        third_column.grid(row=0, column=2, sticky="nsew", padx=(scale(5), 0))
    ...
    def _create_docx_to_md_hr_config(self, parent_frame):
        ...
        title_frame.pack(fill="x", pady=(0, scale(10)))
        ...
        info = create_info_icon(title_frame, t("settings.formatting.separator_mapping_tooltip"), "info")
        info.pack(side="left", padx=(self.layout_config.widget_spacing, 0))
    ...
    def _create_syntax_config(self, parent_frame):
        ...
        title_label = tb.Label(
            title_frame, text=t("settings.formatting.syntax_selection"), font=(self.small_font, self.small_size, "bold")
        )
        title_label.pack(side="left")

        info = create_info_icon(title_frame, t("settings.formatting.syntax_selection_tooltip"), "info")
        info.pack(side="left", padx=(self.layout_config.widget_spacing, 0))
        ...
        left_column.grid(row=0, column=0, sticky="nsew", padx=(0, scale(5)))

        # 右列
        right_column = tb.Frame(columns_frame)
        right_column.grid(row=0, column=1, sticky="nsew", padx=(scale(5), 0))
    ...
    def _create_md_to_docx_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.formatting.md_to_docx_section"), SectionStyle.PRIMARY
        )
        ...
        format_info = create_info_icon(
            format_title_frame, t("settings.formatting.md_format_processing_tooltip"), "info"
        )
        format_info.pack(side="left", padx=(self.layout_config.widget_spacing, 0))
    ...
    def _create_list_separator_config(self, parent_frame):
        ...
        info = create_info_icon(title_frame, t("settings.formatting.list_separator_tooltip"), "info")
        info.pack(side="left", padx=(self.layout_config.widget_spacing, 0))
        ...
        self.list_separator_entry = tb.Entry(
            input_frame, textvariable=self.list_separator_var, width=20, bootstyle="secondary"
        )
    ...
    def _create_md_to_docx_hr_config(self, parent_frame):
        ...
        info = create_info_icon(title_frame, t("settings.formatting.md_separator_mapping_tooltip"), "info")
        info.pack(side="left", padx=(self.layout_config.widget_spacing, 0))
    ...
    def _create_table_style_config(self, parent_frame):
        ...
        title_label = tb.Label(
            title_frame, text=t("settings.formatting.table_style"), font=(self.small_font, self.small_size, "bold")
        )
        title_label.pack(side="left")

        info = create_info_icon(title_frame, t("settings.formatting.table_style_tooltip"), "info")
        info.pack(side="left", padx=(self.layout_config.widget_spacing, 0))
        ...
        # 第一列 - 内置样式
        first_column = tb.Frame(columns_frame)
        first_column.grid(row=0, column=0, sticky="nsew", padx=(0, scale(10)))

        # 第二列 - 自定义样式
        second_column = tb.Frame(columns_frame)
        second_column.grid(row=0, column=1, sticky="nsew", padx=(scale(10), 0))
        ...
        builtin_radio.pack(anchor="w", pady=(0, scale(5)))
        ...
        self.builtin_style_combo.pack(fill="x", pady=(0, scale(5)))
        ...
        custom_radio.pack(anchor="w", pady=(0, scale(5)))
        ...
        self.custom_style_entry.pack(fill="x", pady=(0, scale(5)))
    ...
    def get_settings(self) -> dict[str, Any]:
        ...
        return {
            ...
            "preserve_table_header_formatting": self.preserve_table_header_formatting_combo.get_config_value()
            == "true",
            # MD → DOCX 格式处理模式
    ...
    def apply_settings(self) -> bool:
        ...
        try:
            ...
            if not self.config_manager.update_config_value(
                "conversion_config", "docx_to_md", "preserve_formatting", settings["preserve_formatting"]
            ):
            ...
            # === MD → DOCX 配置 ===
            if not self.config_manager.update_config_value(
                "conversion_config", "md_to_docx", "formatting_mode", settings["formatting_mode"]
            ):
                logger.error("保存 formatting_mode 失败")
                return False

            if not self.config_manager.update_config_value(
                "conversion_config", "md_to_docx", "heading_formatting_mode", settings["heading_formatting_mode"]
            ):
            ...
            # === YAML列表拼接符 ===
            if not self.config_manager.update_config_value(
                "conversion_config", "md_to_docx", "list_separator", settings["list_separator"]
            ):
            ...
            for key in syntax_keys:
                if not self.config_manager.update_config_value("conversion_config", "syntax", key, settings[key]):
            ...
            # === 分隔符转换配置 - 文档转MD ===
            if not self.config_manager.update_config_value(
                "conversion_config", "horizontal_rule.docx_to_md", "page_break", settings["docx_hr_page_break"]
            ):
                logger.error("保存 docx_hr_page_break 失败")
                return False

            if not self.config_manager.update_config_value(
                "conversion_config", "horizontal_rule.docx_to_md", "section_break", settings["docx_hr_section_break"]
            ):
            ...
            # === 分隔符转换配置 - MD转文档 ===
            if not self.config_manager.update_config_value(
                "conversion_config", "horizontal_rule.md_to_docx", "dash", settings["md_hr_dash"]
            ):
                logger.error("保存 md_hr_dash 失败")
                return False

            if not self.config_manager.update_config_value(
                "conversion_config", "horizontal_rule.md_to_docx", "asterisk", settings["md_hr_asterisk"]
            ):
                logger.error("保存 md_hr_asterisk 失败")
                return False

            if not self.config_manager.update_config_value(
                "conversion_config", "horizontal_rule.md_to_docx", "underscore", settings["md_hr_underscore"]
            ):
            ...
            # === 表格样式配置 ===
            if not self.config_manager.update_config_value(
                "style_table", "md_to_docx", "table_style_mode", settings["table_style_mode"]
            ):
                logger.error("保存 table_style_mode 失败")
                return False

            if not self.config_manager.update_config_value(
                "style_table", "md_to_docx", "builtin_style_key", settings["builtin_style_key"]
            ):
                logger.error("保存 builtin_style_key 失败")
                return False

            if not self.config_manager.update_config_value(
                "style_table", "md_to_docx", "custom_style_name", settings["custom_style_name"]
            ):
