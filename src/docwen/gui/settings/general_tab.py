...
from docwen.i18n import get_available_locales, get_current_locale, set_locale, t
...
class GeneralTab(BaseSettingsTab):
    ...
    def _create_language_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.general.language_section"), SectionStyle.SUCCESS
        )
        ...
        info = create_info_icon(label_frame, t("settings.general.language_tooltip"), "info")
        info.pack(side="left", padx=(self.layout_config.widget_spacing, 0))
        ...
        combobox = tb.Combobox(
            frame, textvariable=self.language_var, values=locale_names, state="readonly", bootstyle="secondary"
        )
        combobox.pack(fill="x")
        ...
        hint_label.pack(anchor="w", pady=(scale(5), 0))
    ...
    def _create_theme_section(self):
        ...
        # 创建区域框架
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.general.theme_section"), SectionStyle.PRIMARY
        )
        ...
        # 主题预览区域
        preview_label = tb.Label(frame, text=t("settings.general.theme_preview"), bootstyle="secondary")
        preview_label.pack(anchor="w", pady=(scale(10), scale(5)))

        preview_frame = tb.Frame(frame, bootstyle="default", height=scale(60))
        preview_frame.pack(fill="x", pady=(0, scale(5)))
    ...
    def _create_theme_preview(self, parent: tb.Frame):
        ...
        # 示例按钮
        sample_button = tb.Button(parent, text=t("settings.general.sample_button"), bootstyle="primary", width=14)
        sample_button.pack(side="left", padx=scale(10), pady=scale(10))

        # 示例标签
        sample_label = tb.Label(parent, text=t("settings.general.sample_text"), bootstyle="secondary")
        sample_label.pack(side="right", padx=scale(10), pady=scale(10))
    ...
    def _create_transparency_section(self):
        ...
        # 创建区域框架
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.general.transparency.section"), SectionStyle.INFO
        )

        # 透明度启用开关
        self.transparency_enabled_var = tk.BooleanVar(value=self.config_manager.is_transparency_enabled())
        ...
        slider_frame.pack(fill="x", pady=(0, scale(5)))
        ...
        transparency_slider.pack(side="left", fill="x", expand=True, padx=(0, scale(10)))

        # 透明度值标签（宽度设为5以确保能显示100%）
        transparency_percent = int(transparency_value * 100)
        self.transparency_value_label = tb.Label(
            slider_frame, text=f"{transparency_percent}%", width=5, bootstyle="secondary"
        )
    ...
    def _create_window_behavior_section(self):
        ...
        # 创建区域框架
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.general.window.section"), SectionStyle.WARNING
        )

        # 记住窗口位置和大小
        self.remember_state_var = tk.BooleanVar(value=self.config_manager.should_remember_gui_state())
        self.create_checkbox_with_info(
            frame,
        ...
        # 启动时自动居中窗口
        self.auto_center_var = tk.BooleanVar(value=self.config_manager.should_auto_center())
        self.create_checkbox_with_info(
            frame,
        ...
        self.expand_side_panels_var = tk.BooleanVar(value=self.config_manager.should_expand_side_panels())
        self.create_checkbox_with_info(
            frame,
            t("settings.general.window.expand_side_panels"),
            self.expand_side_panels_var,
            t("settings.general.window.expand_side_panels_tooltip"),
            self._on_expand_side_panels_changed,
        )
    ...
    def _on_expand_side_panels_changed(self):
        expand = self.expand_side_panels_var.get()
        logger.info(f"左右栏参与扩展设置变更: {expand}")
        self.on_change("expand_side_panels", expand)
    ...
    def get_settings(self) -> dict[str, Any]:
        ...
        settings = {
            ...
            "auto_center": self.auto_center_var.get(),
            "expand_side_panels": self.expand_side_panels_var.get(),
            ...
            for key, value in settings.items():
                if key == "locale":
                    ...
                    result = self.config_manager.update_config_value(
                        "gui_config", "transparency", "default_value", value
                    )
                elif key in ["remember_gui_state", "auto_center", "default_mode", "expand_side_panels"]:
