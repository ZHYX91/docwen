...
class OutputTab(BaseSettingsTab):
    ...
    def _create_intermediate_files_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.output.intermediate_section"), SectionStyle.PRIMARY
        )

        # 说明文本
        desc = tb.Label(frame, text=t("settings.output.save_intermediate_desc"), bootstyle="secondary", justify="left")
        desc.pack(anchor="w", pady=(0, scale(10)))
    ...
    def _create_output_directory_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.output.directory_section"), SectionStyle.INFO
        )
        ...
        custom_path_label_frame.pack(fill="x", pady=(scale(10), scale(5)))

        tb.Label(custom_path_label_frame, text=t("settings.output.custom_path_label"), bootstyle="secondary").pack(
            anchor="w"
        )

        path_input_frame = tb.Frame(frame)
        path_input_frame.pack(fill="x", pady=(0, scale(10)))

        self.custom_path_var = tk.StringVar(value=custom_path)
        self.custom_path_entry = tb.Entry(path_input_frame, textvariable=self.custom_path_var, bootstyle="secondary")
        self.custom_path_entry.pack(side="left", fill="x", expand=True, padx=(0, scale(5)))
    ...
    def _create_date_subfolder_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.output.date_folder.section"), SectionStyle.SUCCESS
        )
        ...
        format_label_frame.pack(fill="x", pady=(scale(10), scale(5)))

        tb.Label(format_label_frame, text=t("settings.output.date_folder.format_label"), bootstyle="secondary").pack(
            anchor="w"
        )
    ...
    def _browse_custom_path(self):
        ...
        if not initial_dir or not Path(initial_dir).exists():
            initial_dir = str(Path("~").expanduser())

        selected_dir = filedialog.askdirectory(title=t("settings.output.browse_title"), initialdir=initial_dir)
    ...
    def _create_output_behavior_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame, t("settings.output.behavior.section"), SectionStyle.WARNING
        )
    ...
    def apply_settings(self) -> bool:
        ...
        try:
            ...
            if not self.config_manager.update_config_value(
                "output_config", "intermediate_files", "save_to_output", settings["save_intermediate_files"]
            ):
                success = False

            # 输出目录模式
            if not self.config_manager.update_config_value(
                "output_config", "directory", "mode", settings["output_mode"]
            ):
                success = False

            # 自定义路径
            if not self.config_manager.update_config_value(
                "output_config", "directory", "custom_path", settings["custom_path"]
            ):
                success = False

            # 日期子文件夹开关
            if not self.config_manager.update_config_value(
                "output_config", "directory", "create_date_subfolder", settings["create_date_subfolder"]
            ):
                success = False

            # 日期格式
            if not self.config_manager.update_config_value(
                "output_config", "directory", "date_folder_format", settings["date_folder_format"]
            ):
                success = False

            # 自动打开文件夹
            if not self.config_manager.update_config_value(
                "output_config", "behavior", "auto_open_folder", settings["auto_open_folder"]
            ):
                success = False
