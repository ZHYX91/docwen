...
class TextTab(BaseSettingsTab):
    ...
    def _get_scheme_mappings(self):
        ...
        try:
            ...
            # 后备默认值
            id_to_name = {
                "hierarchical_standard": t("editors.numbering_add.names.hierarchical_standard")
            }
    ...
    def _create_md_to_docx_numbering_section(self):
        ...
        frame = self.create_section_frame(
            ...
            t("settings.text.md_to_docx_section"),
        ...
        scheme_label = tb.Label(
            right_frame,
            text=t("settings.text.scheme_label"),
        ...
        self.md_to_docx_scheme_combo = tb.Combobox(
            right_frame,
            textvariable=self.md_to_docx_scheme_var,
            values=scheme_names,
            state="readonly",
            width=15
        )
        self.md_to_docx_scheme_combo.pack(side="left")
        self.md_to_docx_scheme_combo.bind(
            '<<ComboboxSelected>>',
            lambda e: self._on_md_to_docx_scheme_changed()
        )
    ...
    def _create_numbering_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame,
            t("settings.text.numbering_settings_section"),
            SectionStyle.INFO
        )
    ...
    def _create_template_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame,
            t("settings.text.template_section"),
            SectionStyle.SUCCESS
        )
        
        # 获取当前配置
        current_template_type = self.config_manager.get_default_md_template_type()
        current_display = t("settings.text.template_docx") if current_template_type == "docx" else t("settings.text.template_xlsx")
        
        # 模板类型选项（国际化）
        template_options = [
            t("settings.text.template_docx"),
            t("settings.text.template_xlsx")
        ]
    ...
    def _create_validation_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame,
            t("settings.text.validation_section"),
            SectionStyle.INFO
        )
    ...
    def _create_dictionary_section(self):
        ...
        frame = self.create_section_frame(
            self.scrollable_frame,
            t("settings.text.dictionary_section"),
    ...
    def _on_edit_numbering_schemes(self):
        ...
        try:
            from docwen.gui.settings.editors.numbering_add import (
            ...
            try:
                ...
                MessageBox.showerror(
                    t("common.error"),
                    t("settings.text.editor_open_failed", error=str(e)),
                    parent=self
                )
            except Exception:
                pass
    ...
    def _on_edit_numbering_patterns(self):
        ...
        try:
            from docwen.gui.settings.editors.numbering_clean import (
    ...
    def get_settings(self) -> Dict[str, Any]:
        ...
        settings = {
            ...
            "to_docx_default_scheme": scheme_name_to_id.get(
                self.md_to_docx_scheme_var.get(),
                "gongwen_standard"
            ),
            # 模板设置
        ...
        try:
            ...
            for key in text_keys:
                if not self.config_manager.update_config_value(
                    "conversion_defaults", "text", key, settings[key]
                ):
                    success = False
            ...
            if not self.config_manager.update_config_value(
                ...
                success = False
                logger.error(f"更新模板配置失败")
            
            # 保存校对设置到conversion_defaults.toml的[document]节
            validation_keys = ["enable_symbol_pairing", "enable_symbol_correction", 
