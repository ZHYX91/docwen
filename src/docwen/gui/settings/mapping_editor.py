...
class MappingEditorDialog(tb.Toplevel, ScalableMixin):
    ...
    def __init__(
        ...
        self.table_container: tb.Frame | None = None
    ...
    def _get_column_headers(self) -> list[str]:
        ...
        return headers.get(
            self.mapping_type,
            [t("editors.mapping.generic_key"), t("editors.mapping.generic_value_list"), t("editors.mapping.comment")],
        )
    ...
    def _create_description_label(self, parent: tb.Frame) -> None:
        ...
        description_label.pack(anchor="w", pady=(0, self.scale(10)))
    ...
    def _create_operation_buttons(self, parent: tb.Frame) -> None:
        ...
        button_frame.pack(side="bottom", fill="x", pady=(self.scale(10), 0))
    ...
    def _create_left_buttons(self, parent: tb.Frame) -> None:
        ...
        for text, style, command in buttons:
            btn = tb.Button(left_frame, text=text, bootstyle=style, command=command)
    ...
    def _create_right_buttons(self, parent: tb.Frame) -> None:
        ...
        # 保存按钮
        save_button = tb.Button(right_frame, text=t("common.save"), bootstyle="success", command=self._on_save)
        save_button.pack(side="right", padx=(self.config.BUTTON_PADDING, 0))

        # 取消按钮
        cancel_button = tb.Button(right_frame, text=t("common.cancel"), bootstyle="secondary", command=self.destroy)
    ...
    def _create_table_container(self, parent: tb.Frame) -> None:
        ...
        table_frame.pack(fill="both", expand=True, pady=(0, self.scale(10)))
    ...
    def _rebuild_table(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"列宽设置成功: {self.config.COL_WIDTH_KEY}/{self.config.COL_WIDTH_VALUES}/{self.config.COL_WIDTH_COMMENT}"
            )
    ...
    def _on_add_entry(self) -> None:
        ...
        dialog = EntryEditDialog(self, self.mapping_type, operation="add")
    ...
    def _on_delete_entry(self) -> None:
        ...
        if self._show_confirm_dialog(
            t("editors.mapping.confirm_delete"), t("editors.mapping.confirm_delete_message", entry_key=key)
        ):
            if key in self.current_mapping:
                del self.current_mapping[key]
            ...
            if self.config_file_path:
                ...
                if section:
                    success = save_mapping_with_comments(
                        self.config_file_path, section, self.current_mapping, self.comments_dict
                    )
    ...
    def _show_duplicate_key_dialog(self, key: str, new_comment: str) -> str:
        ...
        # 格式化当前值显示
        current_values_str = (
            ", ".join(current_values) if current_values else t("editors.mapping.duplicate_key_no_comment")
        )
        ...
        dialog = DuplicateKeyDialog(
            self, key=key, current_values=current_values_str, current_comment=current_comment_display
        )
...
class DuplicateKeyDialog(tb.Toplevel, ScalableMixin):
    ...
    def __init__(self, parent: tk.Misc, key: str, current_values: str, current_comment: str):
        """
        初始化重复键处理对话框
    ...
    def _create_interface(self) -> None:
        ...
        message_label = tb.Label(
            main_frame, text=message, wraplength=self.scale(self.DIALOG_WIDTH - 40), justify="left"
        )
        message_label.pack(fill="x", pady=(0, self.scale(20)))
        ...
        overwrite_btn = tb.Button(
            button_frame, text=t("editors.mapping.overwrite"), bootstyle="warning", command=self._on_overwrite, width=12
        )
        overwrite_btn.pack(side="left", padx=(0, self.scale(10)))

        # 合并按钮
        merge_btn = tb.Button(
            button_frame, text=t("editors.mapping.merge"), bootstyle="success", command=self._on_merge, width=12
        )
        merge_btn.pack(side="left", padx=(0, self.scale(10)))

        # 取消按钮
        cancel_btn = tb.Button(
            button_frame, text=t("common.cancel"), bootstyle="secondary", command=self._on_cancel, width=12
        )
...
class EntryEditDialog(tb.Toplevel, ScalableMixin):
    ...
    def __init__(
        ...
        self.comment_entry: tb.Entry | None = None
    ...
    def _create_key_input(self, parent: tb.Frame) -> None:
        ...
        key_frame.pack(fill="x", pady=(0, self.scale(10)))

        key_label = tb.Label(key_frame, text=self._get_key_label(), bootstyle="primary")
        key_label.pack(anchor="w")

        key_entry = tb.Entry(key_frame, textvariable=self.key_value, width=50)
        key_entry.pack(fill="x", pady=(5, 0))

    def _create_values_input(self, parent: tb.Frame) -> None:
        """创建值输入区域"""
        values_frame = tb.Frame(parent)
        values_frame.pack(fill="x", pady=(0, self.scale(10)))

        values_label = tb.Label(values_frame, text=self._get_values_label(), bootstyle="primary")
        values_label.pack(anchor="w")

        self.values_text = tb.Text(values_frame, width=50, height=self.TEXT_HEIGHT)
    ...
    def _create_comment_input(self, parent: tb.Frame) -> None:
        ...
        comment_frame.pack(fill="x", pady=(0, self.scale(10)))

        comment_label = tb.Label(comment_frame, text=t("editors.mapping.comment_label"), bootstyle="primary")
        comment_label.pack(anchor="w")

        comment_entry = tb.Entry(comment_frame, textvariable=self.comment_value, width=50)
        comment_entry.pack(fill="x", pady=(self.scale(5), 0))
        self.comment_entry = comment_entry

    def _create_example_label(self, parent: tb.Frame) -> None:
        """创建示例标签"""
        example_frame = tb.Frame(parent)
        example_frame.pack(fill="x", pady=(0, self.scale(15)))

        example_label = tb.Label(
            example_frame, text=self._get_example_text(), bootstyle="secondary", font=(self.small_font, self.small_size)
        )
    ...
    def _create_buttons(self, parent: tb.Frame) -> None:
        ...
        # 确定按钮
        ok_button = tb.Button(button_frame, text=t("common.ok"), bootstyle="success", command=self._on_ok)
        ok_button.pack(side="right", padx=(self.scale(5), 0))

        # 取消按钮
        cancel_button = tb.Button(button_frame, text=t("common.cancel"), bootstyle="secondary", command=self.destroy)
        cancel_button.pack(side="right", padx=(0, self.scale(5)))
