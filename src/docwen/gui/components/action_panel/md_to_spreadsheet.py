...
class MdToSpreadsheetMixin(_ActionPanelBase):
    """
    MD转表格功能混入类
    ...
    def setup_for_md_to_spreadsheet(self: ActionPanelHost, file_path: str):
        """
        设置为MD转表格处理模式
        ...
        self.status_var.set(t("action_panel.md_to_spreadsheet.ready"))
        logger.info("MD转表格操作面板设置完成")

    def _create_md_to_spreadsheet_buttons(self: ActionPanelHost):
        """
        创建MD转电子表格格式的按钮
        ...
        self.convert_excel_button = tb.Button(
            first_row_frame,
            text=t("action_panel.md_to_spreadsheet.generate_xlsx"),
        ...
        ToolTip(self.convert_excel_button, t("action_panel.md_to_spreadsheet.generate_xlsx_tooltip"))

        self.convert_xls_button = tb.Button(
            first_row_frame,
            text=t("action_panel.md_to_spreadsheet.generate_xls"),
            command=self._on_convert_xls_clicked,
            bootstyle=self.button_colors["info"],
            **self.button_style_2,
        )
        self.convert_xls_button.grid(row=0, column=1)
        ToolTip(self.convert_xls_button, t("action_panel.md_to_spreadsheet.generate_xls_tooltip"))
        ...
        self.convert_ods_button = tb.Button(
            second_row_frame,
            text=t("action_panel.md_to_spreadsheet.generate_ods"),
        ...
        ToolTip(self.convert_ods_button, t("action_panel.md_to_spreadsheet.generate_ods_tooltip"))

        self.convert_csv_button = tb.Button(
            second_row_frame,
            text=t("action_panel.md_to_spreadsheet.generate_csv"),
            command=self._on_convert_csv_clicked,
            bootstyle=self.button_colors["warning"],
            **self.button_style_2,
        )
        self.convert_csv_button.grid(row=0, column=1)
        ToolTip(self.convert_csv_button, t("action_panel.md_to_spreadsheet.generate_csv_tooltip"))

        logger.debug("MD到电子表格系列按钮创建完成 - 两行布局")

    def _on_convert_excel_clicked(self: ActionPanelHost):
        """处理生成XLSX按钮点击事件"""
        if self.on_action:
            self.on_action("convert_md_to_xlsx", self.file_path, self.get_selected_options())

    def _on_convert_xls_clicked(self: ActionPanelHost):
        """处理生成XLS按钮点击事件"""
        if self.on_action:
            self.on_action("convert_md_to_xls", self.file_path, {})

    def _on_convert_ods_clicked(self: ActionPanelHost):
        """处理生成ODS按钮点击事件"""
        if self.on_action:
            self.on_action("convert_md_to_ods", self.file_path, {})

    def _on_convert_csv_clicked(self: ActionPanelHost):
        """处理生成CSV按钮点击事件"""
        if self.on_action:
