...
class MdToDocumentMixin(_ActionPanelBase):
    ...
    def setup_for_md_to_document(self: ActionPanelHost, file_path: str):
        """
        设置为MD转文档处理模式
    ...
    def _create_md_to_document_buttons(self: ActionPanelHost):
        """
        创建MD转文档格式的按钮
        ...
        ToolTip(self.convert_doc_button, t("action_panel.md_to_document.generate_doc_tooltip"))
        ...
        ToolTip(self.convert_odt_button, t("action_panel.md_to_document.generate_odt_tooltip"))
        ...
        ToolTip(self.convert_rtf_button, t("action_panel.md_to_document.generate_rtf_tooltip"))

        logger.debug("MD到文档系列按钮创建完成 - 两行布局")

    def _create_md_to_document_options(self: ActionPanelHost):
        """
        创建MD转文档的生成选项
        ...
        self.md_to_doc_options_frame = tb.Labelframe(
            self.button_container, text=t("action_panel.generation_options"), bootstyle="info"
        )
        ...
        if default_scheme_id in scheme_id_to_name:
            ...
            default_scheme_id = next(iter(self._md_numbering_schemes.keys()))
        ...
        remove_numbering_container.grid(
            row=0, column=0, columnspan=2, sticky="w", padx=(scale(10), scale(10)), pady=(0, scale(5))
        )
        ...
        add_numbering_container.grid(
            row=1, column=0, columnspan=2, sticky="w", padx=(scale(10), scale(10)), pady=(0, scale(5))
        )
        ...
        add_numbering_info = create_info_icon(
            add_numbering_container, t("action_panel.md_to_document.add_new_numbering_tooltip"), bootstyle="info"
        )
        ...
        options = [
            (
                t("action_panel.proofread.symbol_pairing"),
                SYMBOL_PAIRING,
                t("action_panel.proofread.symbol_pairing_tooltip"),
            ),
            (t("action_panel.proofread.typos_rule"), TYPOS_RULE, t("action_panel.proofread.typos_rule_tooltip")),
            (
                t("action_panel.proofread.symbol_correction"),
                SYMBOL_CORRECTION,
                t("action_panel.proofread.symbol_correction_tooltip"),
            ),
            (
                t("action_panel.proofread.sensitive_word"),
                SENSITIVE_WORD,
                t("action_panel.proofread.sensitive_word_tooltip"),
            ),
        ]
        ...
        for i, (text, key, tooltip_text) in enumerate(options):
            ...
            checkbox = tb.Checkbutton(option_frame, text=text, variable=var, bootstyle="round-toggle")
    ...
    def _get_default_proofread_options(self: ActionPanelHost) -> dict[str, bool]:
        """
        从配置文件获取默认校对选项
    ...
    def _on_md_add_numbering_toggle(self: ActionPanelHost):
        """
