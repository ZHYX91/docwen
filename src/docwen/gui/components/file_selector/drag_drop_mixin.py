...
from docwen.gui.core.mixins_protocols import DragDropHost, TabbedFileSelectorHost
...
class DragDropMixin:
    ...
    def _setup_drag_drop(self: DragDropHost):
        """
        设置拖拽支持
        ...
        try:
            ...
            logger.error(f"设置文件列表拖拽支持失败: {e!s}")
            self.drag_enabled = False

    def on_drag_enter(self: DragDropHost, event):
        """
        处理拖拽进入事件
    ...
    def on_drag_leave(self: DragDropHost, event):
        """
        处理拖拽离开事件
    ...
    def on_drop(self: DragDropHost, event):
        """
        处理文件放置事件
        ...
        try:
            ...
            # 解析拖拽的文件
            files = self.parse_dropped_files(file_data)

            if len(files) > 0:
                logger.info(f"文件列表拖拽文件: {len(files)} 个文件")

                # 处理文件（支持文件夹递归）
                processed_files = self.process_batch_files(files)
                logger.info(f"处理后得到 {len(processed_files)} 个支持文件")

                if processed_files:
                    # 获取 TabbedFileSelector 的引用
                    tabbed_list = self.get_tabbed_batch_file_list()
            ...
            logger.error(f"处理文件列表拖拽失败: {e!s}")

    def get_tabbed_batch_file_list(self: DragDropHost) -> TabbedFileSelectorHost | None:
        """
        获取 TabbedFileSelector 的引用
    ...
    def parse_dropped_files(self: DragDropHost, file_data: str) -> list[str]:
        """
        解析拖拽的文件数据
    ...
    def process_batch_files(self: DragDropHost, files: list[str]) -> list[str]:
        """
        处理批量模式下的拖拽文件
