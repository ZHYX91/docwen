...
def show_skip_dialog(master, file_info: "FileInfo") -> None:
    ...
    file_dir = str(Path(file_info.file_path).parent)

    message = t(
        "components.file_dialogs.skip_message", reason=skip_reason, filename=file_info.file_name, location=file_dir
    )

    logger.info(f"显示跳过对话框: {file_info.file_name}")

    # 显示信息对话框
    messagebox.showinfo(t("components.file_dialogs.skip_title"), message, parent=master)

    # 询问是否打开文件位置
    if messagebox.askyesno(
        t("components.status_bar.open_location_title"), t("components.status_bar.open_location_confirm"), parent=master
    ):
...
def show_error_dialog(master, file_info: "FileInfo") -> None:
    ...
    message = t("components.file_dialogs.error_message", filename=file_info.file_name, error=error_message)

    logger.info(f"显示错误对话框: {file_info.file_name}")

    # 显示错误对话框
    messagebox.showerror(t("components.file_dialogs.error_title"), message, parent=master)
