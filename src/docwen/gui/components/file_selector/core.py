...
from .dialogs import show_skip_dialog, show_error_dialog, open_file_location
...
class FileSelectorCore:
    ...
    def _create_file_item(self, file_info: FileInfo, index: int):
        ...
        try:
            ...
            if file_info.is_selected:
                item_frame.configure(bg='SystemButtonFace', highlightthickness=border_width)
            else:
                item_frame.configure(bg='SystemButtonFace', highlightthickness=0)
        ...
        if file_info.status == 'completed':
            ...
            if complete_icon:
                ...
                index_label = tb.Label(
                    item_frame, text=str(index + 1),
                    font=(self.small_font, self.small_size),
                    bootstyle="default", anchor=tk.CENTER, cursor="hand2"
                )
            ...
            if skip_icon:
                ...
                index_label = tb.Label(
                    item_frame, text=str(index + 1),
                    font=(self.small_font, self.small_size),
                    bootstyle="warning", anchor=tk.CENTER, cursor="hand2"
                )
                index_label.bind("<Button-1>", lambda e, fi=file_info: self._show_skip_dialog(fi))
            ...
            if fail_icon:
                ...
                index_label = tb.Label(
                    item_frame, text=str(index + 1),
                    font=(self.small_font, self.small_size),
                    bootstyle="danger", anchor=tk.CENTER, cursor="hand2"
                )
                index_label.bind("<Button-1>", lambda e, fi=file_info: self._show_error_dialog(fi))
        else:
            # 显示序号
        ...
        # 文件名标签
        name_label = tb.Label(
            info_frame, text=file_info.file_name,
            font=(self.small_font, self.small_size),
            bootstyle="warning", anchor=tk.W, cursor="hand2"
        )
        ...
        # 文件路径
        path_label = tb.Label(
            info_frame, text=os.path.dirname(file_info.file_path),
        ...
        if file_info.warning_message:
            warning_label = tb.Label(
                info_frame, text=file_info.warning_message,
                font=(self.micro_font, self.micro_size),
                bootstyle="warning", anchor=tk.W
            )
        ...
        # 文件大小
        size_label = tb.Label(
            item_frame, text=file_info.file_size,
            font=(self.micro_font, self.micro_size),
        ...
        open_button = tb.Button(
            item_frame, image=open_icon, bootstyle="secondary-link",
        ...
        remove_button = tb.Button(
            item_frame, image=remove_icon, bootstyle="danger-link",
    ...
    def add_files(self, file_paths: List[str], auto_select_first: bool = False) -> Tuple[List[str], List[Tuple[str, str]]]:
        ...
        if not self.files:
            ...
            for file_path in non_matching_files:
                ...
                failed_files.append((file_path, t("messages.category_mismatch_detail", 
