...
from docwen.gui.components.file_selector import FileInfo
from docwen.i18n import t
from docwen.utils.file_type_utils import get_category_name
...
class SelectorManager:
    ...
    def on_tab_changed(self, new_category: str, old_category: str | None = None):
        """
        处理选项卡切换事件
        ...
        # 注意：不在此处清除初始化标志
        # 因为启动时会自动触发一次选项卡设置，如果在这里清除会导致启动消息丢失
        # 只有模式切换和文件添加才是"真正的用户操作"
    ...
    def on_files_added(self, added_files: list[str], failed_files: list[tuple]):
        ...
        # 显示状态消息
        if added_files:
            category = self.tabbed_file_list.get_current_category() or "unknown"
            self.current_category = category
            category_name = get_category_name(category, t_func=t)
            status_msg = t("components.status_bar.files_added", count=len(added_files), category=category_name)
            self.status_bar.add_message(status_msg, "success", False)

        if failed_files:
            error_msg = t("components.status_bar.files_add_failed", count=len(failed_files))
    ...
    def on_mode_changed(self, new_mode: str):
        ...
        # 用户操作，清除初始化标志（避免重复显示"软件启动"消息）
        if self._is_initializing:
            self._is_initializing = False
            logger.debug("清除初始化标志（模式切换事件）")
    ...
    def on_file_selected(self, file_info: FileInfo):
        ...
        if (
            ...
            and previous_file
            and previous_file.actual_category == file_info.actual_category
            and previous_file.actual_format == file_info.actual_format
        ):
        ...
        # 显示选中文件状态
        category_name = get_category_name(file_info.actual_category, t_func=t)
        status_msg = t("components.status_bar.file_selected", filename=file_info.file_name, category=category_name)
    ...
    def on_template_selected(self, template_type: str, template_name: str):
        ...
        # 更新状态栏
        status_msg = t("components.status_bar.template_selected", name=template_name)
    ...
    def refresh_ui(self, reason: str | None = None):
        """
        统一的UI刷新方法
        根据当前完整状态决定UI显示
    ...
    def _show_no_files_state(self):
        ...
        if self.current_mode == "batch" and hasattr(self.file_drop_area, "_switch_to_empty_state"):
            self.file_drop_area._switch_to_empty_state()

        # 更新状态栏 - 根据初始化状态和模式显示不同消息
        if self._is_initializing:
            # 启动阶段：显示友好的启动消息
            status_msg = t("components.status_bar.app_started")
            logger.debug("初始化阶段，显示启动消息")
            self.status_bar.add_message(status_msg, "secondary", False)
        elif self.current_mode == "single":
            # 单文件模式：不显示选项卡相关信息
            # 用户在单文件模式下不需要知道批量文件列表的选项卡状态
            logger.debug("单文件模式，不显示选项卡无文件提示")
            return
        else:
            # 批量模式：显示选项卡无文件提示
            category_name = (
                get_category_name(self.current_category, t_func=t) if self.current_category else t("file_types.unknown")
            )
            status_msg = t("components.status_bar.no_files_in_tab", category=category_name)
            logger.debug(f"批量模式，显示无文件提示: {status_msg}")
            self.status_bar.add_message(status_msg, "secondary", False)
    ...
    def _setup_document_ui(self, file_info: FileInfo, current_files: list[str]):
        ...
        if hasattr(self.action_panel, "setup_for_document_file"):
            ...
            self.action_panel.setup_for_document_file(
                file_info.file_path, current_files if self.current_mode == "batch" else None
            )
    ...
    def _setup_unknown_ui(self):
        ...
        # 显示错误状态
        self.status_bar.add_message(t("components.status_bar.unknown_file_type"), "danger", False)
