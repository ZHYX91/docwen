...
class ImageSectionMixin:
    ...
    def _create_image_buttons(self: ConversionPanelHost):
        """
        创建图片类格式转换按钮
        ...
        hint_label = tb.Label(
            ...
            text=t("conversion_panel.image.convert_to_other_formats"),
    ...
    def _create_compress_options(self: ConversionPanelHost):
        """创建压缩选项区域"""
        compress_options_frame = tb.Labelframe(
            self.conversion_container, text=t("conversion_panel.image.compression_options"), bootstyle="info"
        )
        compress_options_frame.grid(
            row=4, column=0, columnspan=2, sticky="ew", padx=scale(5), pady=(scale(10), scale(5))
        )
        ...
        lossless_frame.grid(row=0, column=0, sticky="w", padx=scale(10), pady=scale(3))

        self.compress_lossless_radio = tb.Radiobutton(
            lossless_frame,
            text=t("conversion_panel.image.highest_quality"),
        ...
        quality_info = create_info_icon(
            lossless_frame, t("conversion_panel.image.highest_quality_tooltip"), bootstyle="info"
        )
        quality_info.pack(side=tk.LEFT)

        # 限制文件大小单选按钮（第1行）
        limit_frame = tb.Frame(radio_frame, bootstyle="default")
        limit_frame.grid(row=1, column=0, sticky="w", padx=scale(10), pady=scale(3))

        self.compress_limit_radio = tb.Radiobutton(
            limit_frame,
            text=t("conversion_panel.image.limit_file_size"),
            variable=self.compress_mode_var,
            value="limit_size",
            command=self._on_compress_mode_changed,
            bootstyle="primary",
        )
        self.compress_limit_radio.pack(side=tk.LEFT, padx=(0, scale(5)))

        limit_info = create_info_icon(
            limit_frame, t("conversion_panel.image.limit_file_size_tooltip"), bootstyle="info"
        )
        limit_info.pack(side=tk.LEFT)
        ...
        size_label = tb.Label(
            input_frame,
            text=t("conversion_panel.image.file_size_limit"),
        ...
        self.size_warning_label = tb.Label(
            warning_frame, text="", font=(small_font, small_size), bootstyle="danger", anchor=tk.W
        )
        self.size_warning_label.grid(row=0, column=0, sticky="w")

    def _create_saveas_pdf_section(self: ConversionPanelHost):
        """创建另存为PDF区域"""
        saveas_hint_label = tb.Label(
            self.saveas_container,
            text=t("conversion_panel.image.convert_to_layout"),
            font=(self.default_font, self.default_size),
            bootstyle="secondary",
            anchor=tk.CENTER,
        )
        ...
        self.convert_image_to_pdf_button = tb.Button(
            self.saveas_container,
            text=t("conversion_panel.image.save_as_pdf"),
        ...
        ToolTip(self.convert_image_to_pdf_button, t("conversion_panel.image.save_as_pdf_tooltip"))

        # OFD按钮（禁用）
        ofd_button = tb.Button(
            self.saveas_container,
            text=t("conversion_panel.image.save_as_ofd"),
            command=lambda: None,
            bootstyle="secondary",
            **self.button_style_2col,
            state="disabled",
        ...
        ToolTip(ofd_button, t("conversion_panel.image.save_as_ofd_tooltip"))

        # PDF尺寸选项
        size_options_frame = tb.Labelframe(
            self.saveas_container, text=t("conversion_panel.image.size_options"), bootstyle="info"
        )
        size_options_frame.grid(row=2, column=0, columnspan=2, sticky="ew", padx=scale(5), pady=(0, scale(5)))
        ...
        self.quality_original_radio = tb.Radiobutton(
            quality_radio_frame,
            text=t("conversion_panel.image.original_embed"),
        ...
        self.quality_a4_radio = tb.Radiobutton(
            quality_radio_frame,
            text=t("conversion_panel.image.fit_a4"),
            variable=self.pdf_quality_var,
            value="a4",
            bootstyle="primary",
            command=self._on_pdf_quality_changed,
        ...
        self.quality_a3_radio = tb.Radiobutton(
            quality_radio_frame,
            text=t("conversion_panel.image.fit_a3"),
            variable=self.pdf_quality_var,
            value="a3",
            bootstyle="primary",
            command=self._on_pdf_quality_changed,
    ...
    def _create_image_merge_section(self: ConversionPanelHost):
        """创建图片合并section"""
        logger.debug("创建图片合并section")

        self.extra_frame.config(text=t("conversion_panel.image.merge_images"))
        ...
        hint_label = tb.Label(
            ...
            text=t("conversion_panel.image.merge_images_hint"),
            font=(self.default_font, self.default_size),
            bootstyle="secondary",
            anchor=tk.CENTER,
        )
        ...
        self.merge_tiff_button = tb.Button(
            self.extra_container,
            text=t("conversion_panel.image.merge_to_tif"),
        ...
        tiff_options_frame = tb.Labelframe(
            self.extra_container, text=t("conversion_panel.image.conversion_options"), bootstyle="info"
        )
        ...
        tiff_radio_frame.grid_columnconfigure(0, weight=1)

        # 保留透明选项（第0行）
        option1_frame = tb.Frame(tiff_radio_frame, bootstyle="default")
        option1_frame.grid(row=0, column=0, sticky="w", padx=scale(10), pady=scale(3))

        self.tiff_mode_smart_radio = tb.Radiobutton(
            option1_frame,
            text=t("conversion_panel.image.preserve_transparency"),
        ...
        tiff_smart_info = create_info_icon(
            option1_frame, t("conversion_panel.image.preserve_transparency_tooltip"), bootstyle="info"
        )
        tiff_smart_info.pack(side=tk.LEFT)

        # 不保留透明选项（第1行）
        option2_frame = tb.Frame(tiff_radio_frame, bootstyle="default")
        option2_frame.grid(row=1, column=0, sticky="w", padx=scale(10), pady=scale(3))

        self.tiff_mode_rgb_radio = tb.Radiobutton(
            option2_frame,
            text=t("conversion_panel.image.no_transparency"),
            variable=self.tiff_mode_var,
            value="RGB",
            bootstyle="primary",
        )
        ...
        tiff_rgb_info = create_info_icon(
            option2_frame, t("conversion_panel.image.no_transparency_tooltip"), bootstyle="info"
        )
        tiff_rgb_info.pack(side=tk.LEFT)

        logger.debug("图片合并section创建完成")

    def _on_compress_mode_changed(self: ConversionPanelHost):
        """处理压缩模式切换事件"""
    ...
    def _on_size_input_changed(self: ConversionPanelHost):
        """处理文件大小输入变更事件"""
        ...
        if not input_text.strip():
            self._show_size_warning(t("conversion_panel.image.enter_size_limit_warning"))
        ...
        if is_valid:
            ...
            try:
                int(input_text)
                if unit == "KB":
                    self._show_size_warning(t("conversion_panel.image.kb_range_warning"))
                else:
                    self._show_size_warning(t("conversion_panel.image.mb_range_warning"))
            except ValueError:
                self._show_size_warning(t("conversion_panel.image.enter_valid_number_warning"))
            self._disable_format_buttons_for_compress()

    def _show_size_warning(self: ConversionPanelHost, message: str):
        """显示文件大小警告信息"""
        if hasattr(self, "size_warning_label") and self.size_warning_label:
            self.size_warning_label.config(text=message)

    def _clear_size_warning(self: ConversionPanelHost):
        """清除文件大小警告信息"""
        if hasattr(self, "size_warning_label") and self.size_warning_label:
            self.size_warning_label.config(text="")

    def _disable_format_buttons_for_compress(self: ConversionPanelHost):
        """在压缩模式下输入无效时禁用所有格式按钮"""
    ...
    def _on_convert_to_pdf_clicked(self: ConversionPanelHost):
        """处理图片转PDF按钮点击事件"""
        if self.on_action and self.current_file_path:
            quality_mode = self.pdf_quality_var.get() if hasattr(self, "pdf_quality_var") else ""
            if not quality_mode:
                logger.warning(t("conversion_panel.image.no_pdf_size_option_warning"))
                return
    ...
    def _on_pdf_quality_changed(self: ConversionPanelHost):
        """处理PDF尺寸选项变更事件"""
    ...
    def _on_merge_tiff_clicked(self: ConversionPanelHost):
        """处理合并为TIFF按钮点击事件"""
        if self.on_action and self.current_file_path:
