...
from docwen.utils.font_utils import get_default_font
...
class ConversionPanelBase(tb.Frame):
    ...
    def _create_widgets(self):
        ...
        self.conversion_frame = tb.Labelframe(
            self, text=t("conversion_panel.format_conversion"), bootstyle="success"
        )
        ...
        self.saveas_frame = tb.Labelframe(
            self, text=t("conversion_panel.save_as"), bootstyle="danger"
        )
    ...
    def _get_image_button_tooltip(self, fmt_upper: str, is_same_format: bool, button_state: str) -> str:
        ...
        if is_compress_mode:
            if fmt_upper in self.COMPRESSIBLE_FORMATS:
                ...
                return format_tips.get(fmt_upper, t("conversion_panel.other_format_compression_tooltip", format=fmt_upper))
        else:
    ...
    def _get_layout_button_tooltip(self, fmt_upper: str) -> str:
        """获取版式格式按钮的tooltip"""
        tooltips = {
            'PDF': t("conversion_panel.pdf_conversion_tooltip") if self.current_format and self.current_format.lower() in ['ofd', 'xps'] else t("conversion_panel.same_format_tooltip", format="PDF"),
