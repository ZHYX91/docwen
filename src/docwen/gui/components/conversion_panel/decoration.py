...
from docwen.gui.core.mixins_protocols import DecorationHost
...
class DecorationMixin:
    ...
    def _create_decoration_area(self: DecorationHost):
        """
        创建装饰区域
    ...
    def _on_decoration_canvas_configure(self: DecorationHost, event):
        """
        处理 Canvas 大小变化事件
    ...
    def _get_theme_color(self: DecorationHost) -> str:
        """
        获取当前主题的装饰线条颜色
        ...
        try:
            ...
            result = line_color if isinstance(line_color, str) else str(line_color)
    ...
    def _get_background_color(self: DecorationHost) -> str:
        """
        获取当前主题的背景颜色
        ...
        try:
            ...
            colors = style.colors
            bg_color = colors.bg
            result = bg_color if isinstance(bg_color, str) else str(bg_color)
    ...
    def _blend_colors(self: DecorationHost, bg_color: str, fg_color: str, alpha: float) -> str:
        """
        混合两个颜色（模拟透明度效果）
    ...
    def _draw_diagonal_stripes(self: DecorationHost):
        """
        绘制交叉网格（菱形网格）装饰图案
        ...
        for i in range(num_lines):
            ...
            if 0 <= x1 <= width and 0 <= x2 <= width and 0 <= y1 <= height and 0 <= y2 <= height:
                canvas.create_line(x1, y1, x2, y2, fill=blended_color, width=line_width, tags="grid")
    ...
    def refresh_decoration(self: DecorationHost):
        """
        刷新装饰区域（主题切换时调用）
