...
class SpreadsheetSectionMixin:
    ...
    def _create_spreadsheet_buttons(self: ConversionPanelHost):
        """
        创建表格类格式转换按钮
    ...
    def _create_spreadsheet_merge_section(self: ConversionPanelHost):
        """创建表格汇总section"""
        ...
        ToolTip(self.merge_tables_button, t("conversion_panel.spreadsheet.merge_tables_button_tooltip"))

        # 汇总选项边框
        merge_options_frame = tb.Labelframe(
            self.extra_container, text=t("conversion_panel.spreadsheet.merge_options"), bootstyle="info"
        )
        ...
        merge_row_info = create_info_icon(
            row0_frame, t("conversion_panel.spreadsheet.merge_by_row_tooltip"), bootstyle="info"
        )
        ...
        merge_col_info = create_info_icon(
            row1_frame, t("conversion_panel.spreadsheet.merge_by_column_tooltip"), bootstyle="info"
        )
        ...
        merge_cell_info = create_info_icon(
            row2_frame, t("conversion_panel.spreadsheet.merge_by_cell_tooltip"), bootstyle="info"
        )
        ...
        bind_label_wraplength_to_container(
            self.reference_table_label, reference_table_frame, min_wraplength=scale(200), padding=scale(20)
        )

        logger.debug("表格汇总section创建完成")

    def _on_merge_mode_changed(self: ConversionPanelHost):
        """处理汇总模式变更事件"""
        if hasattr(self, "merge_tables_button") and self.merge_tables_button:
            mode_var = self.merge_mode_var if hasattr(self, "merge_mode_var") else None
            mode = mode_var.get() if mode_var is not None else 0
    ...
    def _on_merge_tables_clicked(self: ConversionPanelHost):
        """处理汇总表格按钮点击事件"""
    ...
    def set_reference_table(self: ConversionPanelHost, file_name: str):
        """
        设置当前选中的基准表格文件名
