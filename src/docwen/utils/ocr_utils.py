...
_current_ocr_language: str | None = None  # 当前加载的OCR语言
...
def resolve_ocr_language(ocr_language: str | None = None) -> str:
    """
    解析实际使用的OCR语言
...
def get_ocr(ocr_language: str | None = None):
    """
    获取OCR实例（单例模式，支持多语言）
    ...
    with _ocr_lock:
        ...
        if _ocr_instance is None:
            try:
                ...
                # 模型目录路径
                model_dir = Path(base_dir) / "models" / "rapidocr"

                # 获取对应语言的模型文件
                models = OCR_LANGUAGE_MODELS.get(target_language, OCR_LANGUAGE_MODELS[OCR_LANGUAGE_CHINESE])

                det_model_path = model_dir / models["det"]
                rec_model_path = model_dir / models["rec"]
                cls_model_path = model_dir / models["cls"]
                ...
                if missing_models:
                    error_msg = (
                        f"OCR模型文件缺失 (语言: {target_language})，请下载模型文件到 models/rapidocr/ 目录:\n"
                        + "\n".join(missing_models)
                    )
                ...
                raise ImportError("RapidOCR未安装，请运行: pip install rapidocr_onnxruntime") from e
            except Exception as e:
...
def extract_text_simple(image_path: str, cancel_event=None) -> str:
    ...
    try:
        ...
        if ocr is None:
            return ""
        result, elapse = ocr(str(image_file))
        ...
        for item in result:
            ...
            if confidence_value is not None and confidence_value > 0.5:
                ...
                logger.debug(f"识别文字: {text} (置信度: {confidence_value:.2f})")
            ...
            if confidence_value is not None and confidence_value > 0.5:
                ...
                if isinstance(bbox, (list, tuple)) and len(bbox) >= 4:
                    ...
                    if (
                        ...
                        and len(p0) >= 2
                        and len(p2) >= 2
                        and isinstance(p0[1], (int, float))
                        and isinstance(p2[1], (int, float))
                    ):
                        height = float(p2[1]) - float(p0[1])
                    else:
                        height = 0.0
                    ...
                    text_blocks.append({"text": text if isinstance(text, str) else str(text), "font_size": font_size})
                    logger.debug(
                        f"识别文字: {text} (高度: {height:.0f}px, 字号: {font_size}pt, 置信度: {confidence_value:.2f})"
                    )
                else:
                    # 如果边界框格式不正确，使用默认字号
                    text_blocks.append({"text": text if isinstance(text, str) else str(text), "font_size": 12})
                    logger.debug(f"识别文字: {text} (边界框格式异常，使用默认字号12pt, 置信度: {confidence_value:.2f})")
