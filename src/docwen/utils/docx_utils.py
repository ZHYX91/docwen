...
from docwen.utils.validation_utils import contains_chinese, contains_east_asian
...
def extract_format_from_rpr(rPr, source_name=""):
    ...
    try:
        ...
        logger.warning(f"从rPr提取格式失败: {e!s}")
    ...
    for run in paragraph.runs:
        if contains_chinese(run.text):
            try:
                ...
                logger.info(
                    f"找到有效Run: 中文字体={para_fonts.get('eastAsia')}, 英文字体={para_fonts.get('ascii')}, 字号={get_font_size_name(para_fonts.get('sz'))}"
                )
                return para_fonts
            except Exception as e:
                logger.error(f"提取Run格式失败: {e!s}")
                continue
...
def extract_format_from_paragraph_style(paragraph):
    ...
    if paragraph.style is not None:
        ...
        try:
            ...
            logger.warning(f"从段落样式提取格式失败: {e!s}")
    ...
    while current_style is not None and hasattr(current_style, "base_style"):
        if current_style.base_style is not None:
            ...
            try:
                ...
                logger.warning(f"从父样式提取格式失败: {e!s}")
    ...
    try:
        ...
        logger.warning(f"获取默认样式失败: {e!s}")

    # 4. 使用默认设置填充缺失值
    for key in fonts:
        if fonts[key] is None:
            fonts[key] = DEFAULT_FONTS.get(key)
...
def apply_body_formatting(run, fonts):
    ...
    # 设置字体
    hint_east_asia = contains_east_asian(run.text) if run.text else False
    if hint_east_asia:
        logger.debug("检测到东亚语境，设置 w:hint=eastAsia")
    set_font_family(rPr, fonts, hint_east_asia=hint_east_asia)
...
def apply_paragraph_format(target_para, source_para):
    ...
    try:
        ...
        if source_para.style:
            ...
            logger.debug(
                f"复制段落样式: {source_para.style.name if hasattr(source_para.style, 'name') else source_para.style}"
            )
        ...
        logger.warning(f"应用段落格式时出现警告: {e!s}")
...
def set_font_family(rPr, fonts, hint_east_asia: bool = False):
    """
    设置字体族

    参数:
        rPr: run 属性元素
        fonts: 字体设置字典，需包含 eastAsia、ascii、hAnsi 键
        hint_east_asia: 是否设置 w:hint="eastAsia"。
            设置后，General Punctuation 区间的标点符号（如中文引号、“”、省略号……等）
            会优先使用 eastAsia 字体，而非 hAnsi 字体。通常在 run 文本处于东亚语境时启用。
    """
    # 清除现有设置
...
def is_inside_fallback(element) -> bool:
    ...
    """
    mc_fallback_tag = "{{{}}}Fallback".format(NAMESPACES["mc"])
