...
def latex_to_mathml(latex_str: str) -> str | None:
    ...
    try:
        ...
        if r"\begin{" in latex_str and "matrix" in latex_str:
            logger.debug("latex2mathml失败，尝试手动矩阵转换")
...
def mathml_to_omml(mathml_str: str) -> str | None:
    ...
    try:
        ...
        omml_math = etree.Element("{{{}}}oMath".format(OMML_NS["m"]), nsmap={"m": OMML_NS["m"]})
        ...
        if child_tag == "mo":
            ...
            if open_char in BRACKET_PAIRS:
                ...
                if close_idx > i + 1:
                    ...
                    d_elem = etree.SubElement(omml_parent, "{{{}}}d".format(OMML_NS["m"]))

                    # 设置括号属性
                    dPr = etree.SubElement(d_elem, "{{{}}}dPr".format(OMML_NS["m"]))
                    begChr = etree.SubElement(dPr, "{{{}}}begChr".format(OMML_NS["m"]))
                    begChr.set("{{{}}}val".format(OMML_NS["m"]), open_char)
                    endChr = etree.SubElement(dPr, "{{{}}}endChr".format(OMML_NS["m"]))
                    endChr.set("{{{}}}val".format(OMML_NS["m"]), expected_close)

                    # 转换括号内的内容
                    e_elem = etree.SubElement(d_elem, "{{{}}}e".format(OMML_NS["m"]))
...
def _convert_mathml_to_omml_node(mathml_node, omml_parent):
    ...
    if tag == "math":
        ...
        run = etree.SubElement(omml_parent, "{{{}}}r".format(OMML_NS["m"]))
        text_elem = etree.SubElement(run, "{{{}}}t".format(OMML_NS["m"]))
        text_elem.text = mathml_node.text or ""

    elif tag == "mfrac":
        frac = etree.SubElement(omml_parent, "{{{}}}f".format(OMML_NS["m"]))
        num = etree.SubElement(frac, "{{{}}}num".format(OMML_NS["m"]))
        den = etree.SubElement(frac, "{{{}}}den".format(OMML_NS["m"]))
        children = list(mathml_node)
        if len(children) >= 2:
    ...
    # 上标 - 适配非标准结构（latex2mathml 可能生成 >2 个子节点）
        ...
        ssup = etree.SubElement(omml_parent, "{{{}}}sSup".format(OMML_NS["m"]))
        base = etree.SubElement(ssup, "{{{}}}e".format(OMML_NS["m"]))
        sup = etree.SubElement(ssup, "{{{}}}sup".format(OMML_NS["m"]))
        children = list(mathml_node)
    ...
    # 下标 - 适配非标准结构
        ...
        ssub = etree.SubElement(omml_parent, "{{{}}}sSub".format(OMML_NS["m"]))
        base = etree.SubElement(ssub, "{{{}}}e".format(OMML_NS["m"]))
        sub = etree.SubElement(ssub, "{{{}}}sub".format(OMML_NS["m"]))
        children = list(mathml_node)
    ...
    # 同时上下标 - 适配非标准结构
        ...
        subsup = etree.SubElement(omml_parent, "{{{}}}sSubSup".format(OMML_NS["m"]))
        base = etree.SubElement(subsup, "{{{}}}e".format(OMML_NS["m"]))
        sub = etree.SubElement(subsup, "{{{}}}sub".format(OMML_NS["m"]))
        sup = etree.SubElement(subsup, "{{{}}}sup".format(OMML_NS["m"]))
        children = list(mathml_node)
    ...
    # 根式
        ...
        rad = etree.SubElement(omml_parent, "{{{}}}rad".format(OMML_NS["m"]))
        deg = etree.SubElement(rad, "{{{}}}deg".format(OMML_NS["m"]))  # 默认为空（平方根）
        base = etree.SubElement(rad, "{{{}}}e".format(OMML_NS["m"]))
    ...
    # 根式 (n次根)
        ...
        rad = etree.SubElement(omml_parent, "{{{}}}rad".format(OMML_NS["m"]))
        deg = etree.SubElement(rad, "{{{}}}deg".format(OMML_NS["m"]))
        base = etree.SubElement(rad, "{{{}}}e".format(OMML_NS["m"]))
        children = list(mathml_node)
        if len(children) >= 2:
    ...
    # 矩阵 (MathML mtable → OMML m)
        ...
        # 创建OMML矩阵
        m_elem = etree.SubElement(omml_parent, "{{{}}}m".format(OMML_NS["m"]))

        # 遍历所有行
        for mtr in mathml_node:
            if etree.QName(mtr).localname == "mtr":
                mr = etree.SubElement(m_elem, "{{{}}}mr".format(OMML_NS["m"]))

                # 遍历行中的所有单元格
                for mtd in mtr:
                    if etree.QName(mtd).localname == "mtd":
                        e_elem = etree.SubElement(mr, "{{{}}}e".format(OMML_NS["m"]))
    ...
    # mfenced（括号包围的内容，如矩阵外围括号）
        ...
        begChr.set("{{{}}}val".format(OMML_NS["m"]), open_char)
        endChr = etree.SubElement(dPr, "{{{}}}endChr".format(OMML_NS["m"]))
        endChr.set("{{{}}}val".format(OMML_NS["m"]), close_char)
    ...
    if tag == "oMath":
        ...
        if naryPr is not None:
            ...
            if chr_elem is not None:
                op_char = chr_elem.attrib.get("{{{}}}val".format(OMML_NS["m"]), "∫")

            lim_loc_elem = _get_child_by_tag(naryPr, "limLoc")
            if lim_loc_elem is not None:
                lim_loc = lim_loc_elem.attrib.get("{{{}}}val".format(OMML_NS["m"]), "subSup")
        ...
        if dPr is not None:
            ...
            if begChr is not None:
                open_char = begChr.get("{{{}}}val".format(OMML_NS["m"]), "(")
            endChr = _get_child_by_tag(dPr, "endChr")
            if endChr is not None:
                close_char = endChr.get("{{{}}}val".format(OMML_NS["m"]), ")")
    ...
    if tag == "math":
                ...
                if curr_tag == "mo" and next_tag == "mo":
                    ...
                    if matrix_elem is not None:
                        ...
                        if curr_text == "[" and (next_text == "]" or next_text.startswith("]")):
                            matrix_type = "bmatrix"
                            logger.debug("math层级检测到 [矩阵] 模式，使用 bmatrix")
                        elif curr_text == "(" and (next_text == ")" or next_text.startswith(")")):
                            matrix_type = "pmatrix"
                            logger.debug("math层级检测到 (矩阵) 模式，使用 pmatrix")
                        elif curr_text == "|" and (next_text == "|" or next_text.startswith("|")):
                            matrix_type = "vmatrix"
                            logger.debug("math层级检测到 |矩阵| 模式，使用 vmatrix")
            ...
            if i + 2 < len(children):
                ...
                if curr_tag == "mo" and next_tag == "mo":
                    ...
                    if matrix_elem is not None:
                        ...
                        if curr_text == "[" and (next_text == "]" or next_text.startswith("]")):
                            matrix_type = "bmatrix"
                            logger.debug("mrow层级检测到 [矩阵] 模式，使用 bmatrix")
                        elif curr_text == "(" and (next_text == ")" or next_text.startswith(")")):
                            matrix_type = "pmatrix"
                            logger.debug("mrow层级检测到 (矩阵) 模式，使用 pmatrix")
                        elif curr_text == "|" and (next_text == "|" or next_text.startswith("|")):
                            matrix_type = "vmatrix"
                            logger.debug("mrow层级检测到 |矩阵| 模式，使用 vmatrix")
        ...
        def contains_mtable(elem):
            ...
            if etree.QName(elem).localname == "mtable":
                return True
            return any(contains_mtable(child) for child in elem)
        ...
        if has_mtable:
            ...
            if mtable is not None:
                ...
                if rows:
                    ...
                    logger.warning("矩阵行提取失败，返回空矩阵")
                    return f"\\begin{{{matrix_type}}}\n\\end{{{matrix_type}}}"
            else:
                # mtable 为 None，返回空矩阵
                logger.warning("未找到 mtable 元素，返回空矩阵")
                return f"\\begin{{{matrix_type}}}\n\\end{{{matrix_type}}}"
...
def parse_latex_from_markdown(md_text: str) -> list:
    ...
    for match in re.finditer(block_pattern, md_text, re.DOTALL):
        ...
        formulas.append({"latex": latex, "is_inline": False, "start": match.start(), "end": match.end()})
    ...
    for match in re.finditer(inline_pattern, md_text):
        ...
        if not is_in_block:
            formulas.append(
                {"latex": match.group(1).strip(), "is_inline": True, "start": match.start(), "end": match.end()}
            )
