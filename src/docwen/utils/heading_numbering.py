...
class HeadingFormatter:
    ...
    # 数字样式到转换函数的映射
    STYLE_CONVERTERS: ClassVar[dict[str, Any]] = {
    ...
    def __init__(self, scheme_config: object):
        """
        初始化标题序号格式化器
        ...
        if not isinstance(scheme_config, Mapping):
            raise ValueError("配置格式错误")

        self.scheme_config = dict(scheme_config)
    ...
    def increment_level(self, level: int):
        ...
        logger.debug(f"递增标题级别 {level}，计数器状态: {self.counters[:level]}")
    ...
    def _parse_template(self, template: str, current_level: int) -> str:
        ...
        def replace_placeholder(match):
            ...
            if not (1 <= ref_level <= 9):
                logger.warning(
                    f"模板中引用的级别 {ref_level} 无效（应在1-9之间），位于级别{current_level}的模板: {template}"
                )
            ...
            if counter_value == 0:
                logger.warning(f"级别{current_level}的模板引用了级别{ref_level}，但该级别计数器为0，使用默认值1")
                counter_value = 1

            # 获取样式转换函数
            if style not in self.STYLE_CONVERTERS:
                logger.warning(f"未知的数字样式: {style}，使用半角阿拉伯数字")
            ...
            try:
                ...
                logger.debug(f"占位符替换: {{level={ref_level}, style={style}}} -> {counter_value} -> '{converted}'")
                return converted
            except Exception as e:
                logger.error(f"转换数字 {counter_value} 为样式 {style} 失败: {e}，使用原始数字")
                return str(counter_value)
    ...
    def get_scheme_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "levels_defined": len([k for k in self.scheme_config if k.startswith("level_")]),
        }


def get_formatter_from_config(config_manager, scheme_name: str) -> HeadingFormatter | None:
    """
...
def process_md_numbering(
    content: str, remove_existing: bool = True, add_new: bool = True, scheme_id: str = "gongwen_standard"
) -> str:
    """
