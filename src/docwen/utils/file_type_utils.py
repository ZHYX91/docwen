...
ACTUAL_FORMAT_TO_CATEGORY: Dict[str, str] = {
    ...
    # 版式类
    'pdf': 'layout', 'xps': 'layout', 'ofd': 'layout',
...
def get_category_name(category: str) -> str:
    ...
    """
    return t(f"file_types.{category}_category", default=category)
...
def get_allowed_types_by_category(category: str) -> str:
    ...
    if category not in FILE_CATEGORIES:
        return t("file_types.unknown_type", default="Unknown")
...
def validate_file_format(file_path: str) -> dict:
    ...
    if not is_match:
        ...
        if actual_format in ['txt', 'md', 'csv']:
            # 区分各种纯文本格式
            format_display = {
                'txt': t("file_types.format_txt", default="TXT (Plain Text)"),
                'md': t("file_types.format_md", default="Markdown"),
                'csv': t("file_types.format_csv", default="CSV (Table Text)")
            }
            warning_message = t("file_types.format_mismatch", format=format_display[actual_format], default=f"⚠️ Actual format: {format_display[actual_format]}")
            logger.warning(f"检测到格式不匹配: {file_path} (扩展名: {ext_name}, 实际: {format_display[actual_format]})")
        else:
            warning_message = t("file_types.format_mismatch", format=actual_name, default=f"⚠️ Actual format: {actual_name}")
        ...
        # 不是纯文本格式，也不是未知格式
        warning_message = t("file_types.format_unverifiable", default="⚠️ Cannot verify actual file format")
