...
# 类别名称通过 i18n 系统国际化
# 使用 get_category_name() 函数获取对应语言的类别名称

# 所有支持的扩展名集合
SUPPORTED_EXTENSIONS: set[str] = {ext for category in FILE_CATEGORIES.values() for ext in category}

_FORMAT_CACHE_MAX = 2048
_FORMAT_CACHE: dict[str, tuple[float, int, str]] = {}


def _get_file_stat_key(file_path: str) -> tuple[float, int] | None:
    try:
...
def _detect_actual_file_format_uncached(file_path: str) -> str:
    """
    通过文件签名检测文件实际格式（无缓存）
        ...
        # DOCX/XLSX/ODT/ODS/OFD文件签名: PK 03 04 (ZIP格式)
            ...
            try:
                ...
                with zipfile.ZipFile(file_path, "r") as zip_file:
                    ...
                    if "FixedDocumentSequence.fdseq" in file_list or any(
                        name.startswith("Documents/") and name.endswith(".fpage") for name in file_list
                    ):
                    ...
                    ext = Path(file_path).suffix.lower()
                    ext_map = {".xlsx": "xlsx", ".xps": "xps", ".ods": "ods", ".odt": "odt", ".ofd": "ofd"}
                    return ext_map.get(ext, "docx")

            except Exception as e:
                logger.warning(f"ZIP文件详细检测失败: {e!s}，回退到扩展名")
                ext = Path(file_path).suffix.lower()
                ext_map = {".xlsx": "xlsx", ".xps": "xps", ".ods": "ods", ".odt": "odt", ".ofd": "ofd"}
                return ext_map.get(ext, "docx")
            ...
            if _is_text_file(file_path):
                ...
                logger.debug(f"纯文本文件，内容检测为{text_format.upper()}格式: {file_path}")
                return text_format
...
def _detect_text_format(file_path: str, sample_lines: int = 50) -> str:
    ...
    try:
        ...
        for line in content:
            ...
            # 表格分隔线检测
            elif markdown_table_separator.match(stripped) and stripped.count("-") >= 3:
        ...
        if (
            yaml_front_matter
            or markdown_heading_count > 0
            or markdown_table_count > 0
            or code_block_markers >= 2
            or wiki_link_count > 0
            or markdown_link_count > 0
        ):
            # 记录详细的检测结果
            features = []
        ...
        if len(content) >= 2:  # 至少需要2行才能判断表格
            ...
            delimiters = [",", "\t", ";", "|"]
            best_delimiter = None
            min_avg_cols = 2 if ext == ".csv" else 3
            ...
            for delimiter in delimiters:
                ...
                if len(column_counts) >= 2:
                    ...
                    # 要求平均至少3列，避免将含逗号的普通文本误判为CSV
                    if avg_cols >= min_avg_cols:
                        # 检查是否所有行的列数完全相同（真正的CSV每行列数一致）
                        unique_counts = set(column_counts)
                        if len(unique_counts) == 1:  # 列数必须完全一致
                            # 找到有效的CSV格式
            ...
            if best_delimiter is not None:
                delimiter_name = {",": "逗号", "\t": "制表符", ";": "分号", "|": "竖线"}.get(
    ...
    try:
        ...
        with olefile.OleFileIO(file_path) as ole:
            ...
            if any(indicator in stream_names for indicator in word_indicators):
                logger.debug("检测到Word文档特征流，判定为word类型")
                return "word"

            # 检测Excel工作簿特征流
            excel_indicators = ["Workbook", "Book", "BOOK"]
            if any(indicator in stream_names for indicator in excel_indicators):
                logger.debug("检测到Excel工作簿特征流，判定为excel类型")
                return "excel"

            # 无法确定
            logger.debug("无法通过OLE内部流确定类型")
            return "unknown"

    except ImportError:
        logger.warning("olefile库未安装，无法解析OLE内部结构")
...
def detect_actual_file_format(file_path: str) -> str:
    ...
    返回:
        str: 文件实际格式标识符
    """
    stat_key = _get_file_stat_key(file_path)
    if stat_key is None:
        return _detect_actual_file_format_uncached(file_path)

    cache_key = os.path.normpath(file_path)
    cached = _FORMAT_CACHE.get(cache_key)
    if cached and cached[0] == stat_key[0] and cached[1] == stat_key[1]:
        return cached[2]

    actual_format = _detect_actual_file_format_uncached(file_path)
    if len(_FORMAT_CACHE) >= _FORMAT_CACHE_MAX:
        try:
            _FORMAT_CACHE.pop(next(iter(_FORMAT_CACHE)))
        except Exception:
            _FORMAT_CACHE.clear()
    _FORMAT_CACHE[cache_key] = (stat_key[0], stat_key[1], actual_format)
...
def get_category_name(category: str, t_func=None) -> str:
    """
    获取类别显示名称（支持国际化）

    从 i18n 语言文件获取对应语言的类别名称。

    参数:
        category: 文件类别 (text/document/spreadsheet/layout/image)

    返回:
        str: 国际化的类别显示名称，如 "文本类" 或 "Text"
    """
    if callable(t_func):
        return str(t_func(f"file_types.{category}_category", default=category))
    return category
...
def get_allowed_types_by_category(category: str, t_func=None) -> str:
    """
    根据类别获取允许的文件类型字符串（用逗号分隔）

    会自动合并等价格式（如 TIF/TIFF → TIF, JPG/JPEG → JPG）以节省显示空间

    参数:
        category: 文件类别

    返回:
        str: 允许的文件类型字符串，用逗号分隔
    """
    if category not in FILE_CATEGORIES:
        if callable(t_func):
            return str(t_func("file_types.unknown_type", default="Unknown"))
        return "Unknown"
    ...
    # 等价格式映射：将别名映射到主格式名
    # 键是要隐藏的别名，值是要保留的主格式
    equivalent_aliases = {".tiff": ".tif", ".jpeg": ".jpg", ".heif": ".heic"}

    # 过滤掉等价格式的别名
    filtered_extensions = []
    for ext in extensions:
        if ext not in equivalent_aliases:
            filtered_extensions.append(ext)

    # 将扩展名转换为大写格式（去掉点号）
    type_names = [ext[1:].upper() for ext in filtered_extensions]
    # 用英文逗号连接
...
def _has_known_signature(file_path: str) -> bool:
    ...
    try:
        ...
        if b"ftypheic" in header[:12]:
            return True
        return bool(header[:4] == b"RIFF" and len(header) >= 12 and header[8:12] == b"WEBP")
...
def validate_file_format(file_path: str, actual_format: str | None = None, t_func=None) -> dict:
    """
    验证文件格式是否匹配（扩展名与实际格式）
    ...
    def are_formats_equivalent(fmt1: str, fmt2: str) -> bool:
        ...
        if fmt1 == fmt2:
            return True
        return any(fmt1 in equiv_group and fmt2 in equiv_group for equiv_group in equivalent_formats)
    ...
    def _t(key: str, default: str | None = None, **kwargs) -> str:
        if callable(t_func):
            return str(t_func(key, default=default, **kwargs))
    ...
    if not is_match:
        ...
        if actual_format in ["txt", "md", "csv"]:
            # 区分各种纯文本格式
            format_display = {
                "txt": _t("file_format_validation.format_txt", default="TXT (Plain Text)"),
                "md": _t("file_format_validation.format_md", default="Markdown"),
                "csv": _t("file_format_validation.format_csv", default="CSV (Table Text)"),
            }
            warning_message = _t(
                "file_format_validation.format_mismatch",
                format=format_display[actual_format],
                default=f"⚠️ Actual format: {format_display[actual_format]}",
            )
            logger.warning(f"检测到格式不匹配: {file_path} (扩展名: {ext_name}, 实际: {format_display[actual_format]})")
        else:
            warning_message = _t(
                "file_format_validation.format_mismatch", format=actual_name, default=f"⚠️ Actual format: {actual_name}"
            )
        ...
        # 不是纯文本格式，也不是未知格式
        warning_message = _t("file_format_validation.format_unverifiable", default="⚠️ Cannot verify actual file format")
...
def get_actual_file_category(file_path: str, actual_format: str | None = None) -> str:
    """
    基于实际文件格式获取文件类别
    避免伪装后缀名问题
    ...
    from docwen.formats import CATEGORY_MARKDOWN, CATEGORY_UNKNOWN, category_from_actual_format

    category = category_from_actual_format(actual_format)
    if category == CATEGORY_MARKDOWN:
        return "text"
...
def get_strategy_file_category(file_path: str, actual_format: str | None = None) -> str:
    """
    获取“策略路由”使用的文件类别。

    约定：返回值严格限定为 markdown/document/spreadsheet/layout/image/unknown。
    """
    from docwen.formats import CATEGORY_UNKNOWN, category_from_actual_format

    actual_format = actual_format or detect_actual_file_format(file_path)
    category = category_from_actual_format(actual_format)
    if category != CATEGORY_UNKNOWN:
        return category

    ui_category = get_file_category(file_path)
    ui_to_strategy = {
        "text": "markdown",
        "document": "document",
    ...
    return ui_to_strategy.get(ui_category, CATEGORY_UNKNOWN)


def get_file_info(file_path: str, t_func=None) -> dict:
    """
    获取完整的文件信息
    包含实际格式、类别、验证结果等
    ...
    """
    actual_format = detect_actual_file_format(file_path)
    validation_result = validate_file_format(file_path, actual_format=actual_format, t_func=t_func)
    actual_category = get_actual_file_category(file_path, actual_format=actual_format)
