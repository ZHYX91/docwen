...
def clean_text_in_data(data):
    ...
    if isinstance(data, list):
        return [clean_text_in_data(item) for item in data]
    if isinstance(data, tuple):
        return tuple(clean_text_in_data(item) for item in data)
    if isinstance(data, set):
        return {clean_text_in_data(item) for item in data}
    if isinstance(data, Iterable) and not isinstance(data, str):  # 处理其他可迭代类型
        return [clean_text_in_data(item) for item in data]
    if isinstance(data, str):  # 处理字符串
