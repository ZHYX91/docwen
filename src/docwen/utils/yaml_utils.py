...
from docwen.utils.text_utils import clean_text, format_yaml_value
...
def parse_yaml(yaml_content: str) -> dict:
    ...
    try:
        ...
        logger.error(f"YAML解析错误: {e!s}", exc_info=True)
        return {}
...
def process_list_field(value) -> str:
    ...
    if isinstance(value, str) and value.startswith("[") and value.endswith("]"):
        try:
            ...
            logger.debug(f"字符串解析为列表失败: {value}, 错误: {e!s}，按普通字符串处理")
            # 解析失败，继续按普通字符串处理

    # 数字类型处理
    if isinstance(value, (int, float)):
        logger.debug(f"数字类型，转换为字符串: {value!s}")
        return str(value)
    ...
    if isinstance(value, (list, tuple)):
        ...
        for item in value:
            try:
                ...
                logger.error(f"处理列表项失败: {item}, 错误: {e!s}")
                # 出错时使用原始字符串
    ...
    # 其他类型转换为字符串
    logger.debug(f"其他类型 {type(value).__name__}，转换为字符串: {value!s}")
    return str(value)
