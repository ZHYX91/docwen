...
class I18nManager:
    ...
    def _scan_available_locales(self):
        ...
        try:
            ...
            toml_files = glob.glob(os.path.join(self._locales_dir, "*.toml"))
            ...
            for file_path in toml_files:
                try:
                    ...
                    logger.error("扫描语言文件 %s 失败: %s", file_path, str(e))
            ...
            logger.info("已加载 %d 种语言: %s", len(self.AVAILABLE_LOCALES), 
            ...
            if locales_data and option_id in locales_data:
                ...
                if isinstance(allowed_locales, list):
                    ...
                    if "*" in allowed_locales or current_locale in allowed_locales:
                        ...
                        logger.debug("选项 %s 不适用于当前语言 %s（允许: %s）", 
                                   option_id, current_locale, allowed_locales)
            else:
                # 没有语言标记的选项默认显示（向后兼容）
                result[option_id] = option_name
                logger.debug("选项 %s 无语言标记，默认显示", option_id)
        
        logger.info("获取本地化选项: %s -> %d 个选项（当前语言: %s）", 
                   section_key, len(result), current_locale)
        return result
    
    def _get_section_data(self, section_key: str) -> Optional[Dict[str, Any]]:
        """
        获取指定节的数据
    ...
    def get_style_format(self, style_key: str) -> Optional[Dict[str, Any]]:
        """
        获取样式格式配置
