...
def _get_all_styles_with_type(root):
    ...
    """
    styles = set()
    for style_elem in root.findall('.//w:style', namespaces=NSMAP):
        # 获取样式类型
        style_type = style_elem.get(f'{{{WORD_NAMESPACE}}}type', '')
        # 获取样式名称
        name_elem = style_elem.find('w:name', namespaces=NSMAP)
        if name_elem is not None:
            name_val = name_elem.get(f'{{{WORD_NAMESPACE}}}val', '')
...
def _get_all_style_ids(root):
    ...
    ids = set()
    for style_elem in root.findall('.//w:style', namespaces=NSMAP):
        style_id = style_elem.get(f'{{{WORD_NAMESPACE}}}styleId', '')
        if style_id:
            ids.add(style_id)
    return ids
...
def _inject_style_if_needed(root, existing_styles, existing_ids, target_name, expected_type, default_style_id, style_template, progress_callback=None):
    ...
    if conflicting_style is not None:
        conflicting_type = conflicting_style.get(f'{{{WORD_NAMESPACE}}}type', '')
        if conflicting_type != expected_type:
            # 发现冲突：同名但类型不同
            conflicting_id = conflicting_style.get(f'{{{WORD_NAMESPACE}}}styleId', '')
            logger.warning(f"发现冲突样式 '{target_name}'（类型={conflicting_type}，期望={expected_type}），将删除并重新注入")
            
            # 通知UI
            if progress_callback:
                type_names = {'paragraph': '段落', 'character': '字符'}
                conflict_type_name = type_names.get(conflicting_type, conflicting_type)
                expected_type_name = type_names.get(expected_type, expected_type)
                progress_callback(f"修正冲突样式: '{target_name}' ({conflict_type_name}→{expected_type_name})")
...
def ensure_styles(template_path, progress_callback=None, cleanup_callback=None):
    ...
    if progress_callback and injected_count > 0:
        if conflict_count > 0:
            progress_callback(f"已注入 {injected_count} 个样式（修正 {conflict_count} 个冲突）")
        else:
            progress_callback(f"已注入 {injected_count} 个缺失样式")
