...
def ensure_numbering_part(template_path: str, cleanup_callback=None) -> str:
    ...
    try:
        ...
        # 创建临时副本
        temp_dir = tempfile.mkdtemp(prefix='gongwen_numbering_')
...
def _add_numbering_part(docx_path: str):
    ...
    if rels_path in file_contents:
        ...
        if existing is None:
            ...
            # 添加关系
            rel_elem = etree.SubElement(rels_root, '{%s}Relationship' % REL_NS)
    ...
    if ct_path in file_contents:
        ...
        existing = ct_root.find(f'.//*[@PartName="/word/numbering.xml"]')
        if existing is None:
            # 添加 Override
            override_elem = etree.SubElement(ct_root, '{%s}Override' % CT_NS)
            override_elem.set('PartName', '/word/numbering.xml')
            override_elem.set('ContentType', NUMBERING_CONTENT_TYPE)
            
            file_contents[ct_path] = etree.tostring(
...
def get_max_abstract_num_id(numbering_root) -> int:
    ...
    max_id = -1
    for abstract_num in numbering_root.findall('.//{%s}abstractNum' % WORD_NS):
...
def get_max_num_id(numbering_root) -> int:
    ...
    max_id = 0
    for num in numbering_root.findall('.//{%s}num' % WORD_NS):
        num_id = num.get('{%s}numId' % WORD_NS)
        if num_id:
            try:
