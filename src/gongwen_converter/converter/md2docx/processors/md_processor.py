...
from gongwen_converter.utils.heading_utils import remove_heading_numbering
...
def process_md_body(
    ...
    formatter: Optional[HeadingFormatter] = None
) -> list:
    """
        ...
        if heading_match:
            ...
            if final_text and final_text[-1] in PUNCTUATION_SET:
                ...
                if i + 1 < n:
                    ...
                    is_next_code_block = next_line_content.startswith('```') or next_line_content.startswith('~~~')
                    ...
                    is_special_element = (
                        is_next_heading or is_next_formula or is_next_table or
                        is_next_quote or is_next_code_block or
                        is_next_unordered_list or is_next_ordered_list or is_next_hr
                    )
            ...
            if SETEXT_H1_REGEX.match(line_content):
                ...
                if processed and processed[-1].get('type') == 'content' and last_content_line_no == i - 1:
                    ...
                    # 前面没有可转换的文本，=== 作为普通文本处理
                    logger.debug(f"=== 前面没有可转换的普通文本，当作普通内容处理")
                    # 继续后续处理（会被当作普通文本）
            ...
            if line_content.startswith('$$'):
                ...
                is_complete_single_line = (
                    len(line_content) > 4 and 
                    line_content.endswith('$$') and 
                    line_content[2:-2].strip()  # 中间有实际内容
                )
...
普通正文文本段落2
    ...
    for i, para in enumerate(paragraphs):
        ...
        para_type = para['type']
        if para_type == 'heading_with_content':
            logger.info(f"{prefix}[{i}] 标题({level}): {para['text']}")
            logger.info(f"{prefix}    -> 正文: {para['content']}")
        elif para_type == 'table':
            logger.info(f"{prefix}[{i}] 表格: {len(para['table_data']['headers'])}列")
        elif para_type == 'list_item':
            list_type = para.get('list_type', 'unknown')
            logger.info(f"{prefix}[{i}] 列表项({list_type}, level={level}): {para.get('text', '')[:50]}...")
        elif para_type == 'quote':
            logger.info(f"{prefix}[{i}] 引用块(level={level}): {para.get('text', '')[:50]}...")
        elif para_type == 'code_block':
            lang = para.get('language', '无')
            logger.info(f"{prefix}[{i}] 代码块(语言={lang})")
        else:
            text = para.get('text', '')
            logger.info(f"{prefix}[{i}] {para_type}({level}): {text[:50]}...")
