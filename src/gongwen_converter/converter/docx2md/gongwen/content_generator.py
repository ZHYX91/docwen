...
def generate_main_content(
    ...
    for idx, para in enumerate(doc.paragraphs):
        ...
        if config_manager.is_horizontal_rule_enabled():
            break_type, break_value = detect_page_or_section_break(para)
            if break_type:
        ...
        # 处理段落内容
        paragraph_content = _process_paragraph_content(
            para, para_text, idx,
            remove_numbering, add_numbering, formatter
        )
...
def _process_paragraph_content(
    ...
    # 检测标题级别（双重检测策略）
    # 1. 先根据序号格式检测
    detected_text, heading_level = detect_heading_level(para_text)
