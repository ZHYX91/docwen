...
from gongwen_converter.utils.formula_utils import (
    omml_to_mathml,
    mathml_to_latex,
    is_formula_supported,
    OMML_NS
)
from gongwen_converter.utils.docx_utils import is_inside_fallback
...
def _is_boolean_format_enabled(rPr, tag_name: str) -> bool:
    ...
    返回:
        bool: 是否启用
    """
    elem = rPr.find('{%s}%s' % (WORD_NS, tag_name))
    if elem is None:
        return False
    
    val = elem.get('{%s}val' % WORD_NS)
    # 没有 val 属性 = 启用，val="true"/"1"/其他 = 启用
    # val="false"/"0" = 显式禁用
    return val not in ('false', '0')


def _apply_format_markers_from_xml(run_elem, text: str, preserve_formatting: bool = True, syntax_config: dict = None) -> str:
    """
    ...
    try:
        ...
        rPr = run_elem.find('{%s}rPr' % WORD_NS)
        if rPr is None:
            return text
        ...
        is_underline = False
        u_elem = rPr.find('{%s}u' % WORD_NS)
        if u_elem is not None:
            u_val = u_elem.get('{%s}val' % WORD_NS)
            # val="none" 表示无下划线，其他值（如 "single"）表示有下划线
            is_underline = u_val is not None and u_val != 'none'
        
        is_superscript = False
        is_subscript = False
        is_highlight = rPr.find('{%s}highlight' % WORD_NS) is not None
        
        # 检测上下标
        vertAlign = rPr.find('{%s}vertAlign' % WORD_NS)
        if vertAlign is not None:
            val = vertAlign.get('{%s}val' % WORD_NS)
            if val == 'superscript':
                is_superscript = True
            elif val == 'subscript':
                is_subscript = True
...
def has_formulas_in_paragraph(paragraph) -> bool:
    ...
    try:
        ...
        # 查找所有oMath元素
        omaths = p_elem.findall('.//{%s}oMath' % OMML_NS['m'])
...
def extract_formulas_from_paragraph(paragraph) -> List[Dict]:
    ...
    try:
        ...
        # 查找所有oMath元素
        all_omaths = p_elem.findall('.//{%s}oMath' % OMML_NS['m'])
...
def process_paragraph_with_formulas(
    ...
    try:
        ...
        logger.debug(f"段落包含 {len(formulas)} 个公式，开始按顺序处理 runs (preserve_formatting={preserve_formatting})")
        ...
        def process_element(elem):
            ...
            if tag == '{%s}r' % w_ns:
                ...
                for run_child in elem:
                    ...
                    elif run_child.tag == '{%s}oMath' % m_ns:
            ...
            # 处理超链接、智能标签等容器元素
            elif tag in ['{%s}hyperlink' % w_ns, '{%s}smartTag' % w_ns, 
                         '{%s}sdt' % w_ns, '{%s}sdtContent' % w_ns]:
                for child in elem:
                    process_element(child)
