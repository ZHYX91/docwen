...
from gongwen_converter.utils.docx_utils import (
...
from .formula_processor import process_paragraph_with_formulas, has_formulas_in_paragraph, is_formula_supported
                ...
                for para_idx, paragraph in enumerate(cell.paragraphs):
                    ...
                    if has_formula:
                        ...
                        if formula_md:
                            ...
                            # 如果公式转换失败，至少保留段落文本
                            logger.warning(f"表格单元格公式转换失败，尝试提取文本")
    ...
    try:
        for table_idx, table in enumerate(doc.tables):
            ...
            tables_info.append({
                ...
                'position': position_info.get('index', -1)
            })
        ...
        logger.error(f"提取表格结构失败: {str(e)}", exc_info=True)
        return []
...
def save_nested_table_as_md(nested_table, output_folder, parent_file_name, nested_index, 
                            images_info, config_manager, options=None):
    """
    保存嵌套表格为独立md文件
    ...
    # 1. 生成md文件路径（使用统一工具）
    dummy_input_path = os.path.join(output_folder, parent_file_name + ".docx")
...
def convert_table_to_md_with_images(table, table_index, images_info, output_folder, 
                                    parent_file_name, config_manager, options=None):
    """
    ...
    # 默认选项
    if options is None:
        options = {'keep_images': True, 'enable_ocr': False}
            ...
            for cell_idx, cell in enumerate(row.cells):
                ...
                for para in cell.paragraphs:
                    ...
                    if has_formula:
                        ...
                        should_preserve_format = preserve_table_header_formatting if is_header_row else preserve_formatting
                        formula_md = process_paragraph_with_formulas(
                            para,
                            preserve_formatting=should_preserve_format,
                ...
                if nested_tables:
                    logger.info(f"表{table_index+1}行{row_idx+1}列{cell_idx+1}检测到 {len(nested_tables)} 个嵌套表格")
                    
                    for nested_table in nested_tables:
                        md_name, md_content = save_nested_table_as_md(
                            nested_table, 
                            output_folder, 
                            parent_file_name,
...
def replace_image_markers_in_table(table_md, images_info, keep_images, enable_ocr, 
                                   output_folder, progress_callback=None, cancel_event=None):
    """
