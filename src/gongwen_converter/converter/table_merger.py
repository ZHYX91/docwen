...
class TableMerger:
    ...
    def merge_tables(
        ...
        try:
            ...
            for i, collect_file in enumerate(collect_files, 1):
                if progress_callback:
                    progress_callback(f"正在汇总第 {i}/{len(collect_files)} 个表格...")
    ...
    def _preprocess_table(self, file_path: str, is_base: bool = False) -> str:
        ...
        if actual_format == 'xlsx':
            ...
            # 先将CSV转为xlsx
            xlsx_path = convert_csv_to_xlsx(temp_input)
            # 复制到临时目录
            ...
            from gongwen_converter.converter.formats.office import office_to_xlsx
    ...
    def _save_result(self, workbook: Workbook, base_file: str) -> str:
        ...
        # 构建输出文件名（与合并功能保持一致）
        output_filename = f"汇总表格_{timestamp}.xlsx"
