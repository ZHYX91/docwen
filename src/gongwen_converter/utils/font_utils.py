...
def get_system_fonts() -> List[str]:
    ...
    try:
        ...
        logger.error(f"获取系统字体失败: {str(e)}")
        # 返回一个默认字体列表作为回退
        fallback_fonts = [
            "Arial", "Helvetica", "Times New Roman", "Courier New",
            "Microsoft Sans Serif", "Tahoma", "Verdana", "SimSun",
            "Microsoft YaHei", "SimHei", "KaiTi", "FangSong"
        ]
        _system_fonts_cache = fallback_fonts
        return fallback_fonts
    
def get_available_font(font_families: List[str]) -> Optional[str]:
    """
    根据字体家族列表选择第一个可用的字体
...
def apply_font_styles(root: tk.Tk):
    ...
    try:
        ...
        logger.info(f"已应用字体样式: 默认={font_name} {font_size}pt, 标题={title_font_name} {title_font_size}pt, 小字体={small_font_name} {small_font_size}pt, 大字体={large_font_name} {large_font_size}pt")
        
    except Exception as e:
        logger.error(f"应用字体样式失败: {str(e)}")
...
if __name__ == "__main__":
    ...
    print(f"系统字体数量: {len(fonts)}")
    
    # 搜索特定字体
    search_terms = ["YaHei", "PingFang", "Noto", "SimSun", "SimHei", "KaiTi", "FangSong", "Arial", "Helvetica"]
    found_fonts = []
    
    for term in search_terms:
        matches = [font for font in fonts if term.lower() in font.lower()]
        if matches:
            found_fonts.extend(matches)
            print(f"找到包含 '{term}' 的字体: {matches}")
    ...
    # 测试获取默认字体
    default_font, default_size = get_default_font()
    print(f"\n默认字体: {default_font} {default_size}pt")
    
    # 测试获取标题字体
    title_font, title_size = get_title_font()
    print(f"标题字体: {title_font} {title_size}pt")
    
    # 测试获取小字体
    small_font, small_size = get_small_font()
    print(f"小字体: {small_font} {small_size}pt")
    
    # 测试获取大字体
    large_font, large_size = get_large_font()
    print(f"大字体: {large_font} {large_size}pt")
    
    # 测试获取可用字体
    test_families = ["Microsoft YaHei", "PingFang SC", "Noto Sans CJK SC", "SimSun"]
    available = get_available_font(test_families)
    print(f"可用字体测试: {test_families} -> {available}")
