...
from gongwen_converter.gui.components.base_dialog import BaseDialog
...
class SettingsDialog(BaseDialog):
    ...
    def _create_interface(self):
        ...
        # 创建主容器
        self.main_container = tb.Frame(self, padding=DIALOG_CONFIG.padding)
    ...
    def _create_general_tab(self):
        ...
        try:
            ...
            logger.error(f"导入通用设置选项卡失败: {str(e)}")
    ...
    def _create_link_tab(self):
        ...
        try:
            ...
            logger.error(f"导入链接设置选项卡失败: {str(e)}")
            self._show_tab_error("链接设置", str(e))
        except Exception as e:
            logger.error(f"创建链接设置选项卡失败: {str(e)}")
            self._show_tab_error("链接设置", str(e))
    ...
    def _create_image_tab(self):
        ...
        try:
            ...
            logger.error(f"导入图片设置选项卡失败: {str(e)}")
            self._show_tab_error("图片设置", str(e))
        except Exception as e:
            logger.error(f"创建图片设置选项卡失败: {str(e)}")
            self._show_tab_error("图片设置", str(e))
    ...
    def _create_output_tab(self):
        ...
        try:
            ...
            logger.error(f"导入输出设置选项卡失败: {str(e)}")
    ...
    def _create_logging_tab(self):
        ...
        try:
            ...
            logger.error(f"导入日志设置选项卡失败: {str(e)}")
    ...
    def _apply_preview_settings(self, category: str, key: str, value: Any):
        ...
        try:
            ...
            # 处理通用设置的预览
            if category == "gui_config":
                # 主题变更不再预览，只在保存时应用
                if key == "default_transparency" and self.main_window.is_transparency_enabled():
                    # 透明度可以安全预览（不涉及style.theme_use()）
                    if (hasattr(self.main_window, 'root') and 
                        self.main_window.root and 
    ...
    def _on_ok(self):
        ...
        logger.debug(f"设置 is_closing = True")
        ...
        try:
            ...
            if success and self.modified_settings:
                logger.info(f"步骤2: 在对话框关闭前应用设置到运行时...")
                logger.debug(f"要应用的设置: {self.modified_settings}")
            ...
            logger.error("=" * 60)
            logger.error(f"确定按钮处理过程中发生异常: {type(e).__name__}: {str(e)}", exc_info=True)
