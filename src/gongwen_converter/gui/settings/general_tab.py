...
from gongwen_converter.i18n import t, get_available_locales, get_current_locale, set_locale
...
class GeneralTab(BaseSettingsTab):
    ...
    def _create_theme_section(self):
        ...
        english_to_display = {v: k for k, v in self.theme_mapping.items()}
        current_theme_display = english_to_display.get(current_theme, current_theme)
    ...
    def _get_available_themes(self):
        ...
        for key in theme_keys:
            display_name = t(f"settings.general.themes.{key}")
            # 如果翻译不存在（返回[key]格式），使用英文原名
            if display_name.startswith("["):
                display_name = key.capitalize()
            display_to_english[display_name] = key
        
        # 返回排序后的显示名称列表
        display_themes = sorted(display_to_english.keys())
        
        logger.debug(f"可用主题数量: {len(display_themes)}")
        return display_themes, display_to_english
    ...
    def _on_theme_changed(self, event=None):
        ...
        new_theme_english = self.theme_mapping.get(new_theme_display, new_theme_display)
        
        logger.info(f"主题已选择: {new_theme_display} -> {new_theme_english}（将在保存时应用）")
    ...
    def _create_window_behavior_section(self):
        ...
        # 默认文件处理模式
        mode_options = [
            t("settings.general.window.single_mode"),
            t("settings.general.window.batch_mode")
        ]
        current_mode = self.config_manager.get_default_mode()
        current_display_value = mode_options[0] if current_mode == "single" else mode_options[1]
        
        # 保存模式选项用于后续转换
        self.mode_option_single = mode_options[0]
        self.mode_option_batch = mode_options[1]
    ...
    def _on_default_mode_changed(self, event=None):
        ...
        new_mode_config = "single" if new_mode_display == self.mode_option_single else "batch"
    ...
    def get_settings(self) -> Dict[str, Any]:
        ...
        # 转换主题名称
        current_theme_display = self.theme_var.get()
        current_theme_english = self.theme_mapping.get(current_theme_display, current_theme_display)
        
        # 转换默认模式
        mode_display = self.default_mode_var.get()
        mode_config = "single" if mode_display == self.mode_option_single else "batch"
