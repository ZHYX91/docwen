...
class NumberingPatternsEditorDialog(BaseEditorDialog):
    ...
    def _create_right_panel(self, parent: tb.Frame):
        ...
        # 规则编辑区域
        self._create_rule_editor(right_frame)
    ...
    def _create_rule_editor(self, parent: tb.Frame):
        """创建规则编辑区域"""
        frame = tb.Labelframe(parent, text="⚙️ 规则编辑", padding=5)
    ...
    def _create_regex_test(self, parent: tb.Frame):
        """创建正则测试区域"""
        frame = tb.Labelframe(parent, text="🧪 正则测试", padding=5)
        frame.pack(fill="x", pady=(0, 10))
    ...
    def _create_placeholder_info(self, parent: tb.Frame):
        """创建占位符说明区域"""
        frame = tb.Labelframe(parent, text="📖 可用占位符", padding=5)
  ...
  {circled}      - 带圈数字（①-㊿）
  {unit_suffix}  - 中文单位后缀（章节条款等）
  {unit_prefix}  - 中文单位前缀（卷篇册等）
  {letter}       - 拉丁字母（大小写）

使用说明：
  • 占位符会被替换为实际的字符集正则
  • 规则按列表顺序执行，首个匹配成功即返回
  • 正则表达式使用 Python re 模块语法"""
    ...
    def _run_regex_test(self):
        ...
        try:
            ...
            if result != test_input:
                ...
                if self.test_status_label:
                    self.test_status_label.configure(text="✅ 匹配成功", bootstyle="success")
            else:
                # 无匹配
                if self.test_result_label:
                    self.test_result_label.configure(text=test_input)
                if self.test_status_label:
                    self.test_status_label.configure(text="⚠️ 无匹配", bootstyle="warning")
                
        except re.error as e:
            if self.test_result_label:
                self.test_result_label.configure(text="")
            if self.test_status_label:
                self.test_status_label.configure(text=f"❌ 正则错误: {e}", bootstyle="danger")
        except Exception as e:
            if self.test_result_label:
                self.test_result_label.configure(text="")
            if self.test_status_label:
                self.test_status_label.configure(text=f"❌ 错误: {e}", bootstyle="danger")
