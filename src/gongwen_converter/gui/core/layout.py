...
from gongwen_converter.utils.font_utils import get_default_font, get_title_font, get_small_font, get_micro_font
...
class MainWindowLayoutBuilder(ScalableMixin):
    ...
    def build_complete_layout(self) -> Dict[str, Any]:
        ...
        # 根据3栏网格配置放置面板，每个面板都有两侧边距
        # 批量面板放在列0，中栏放在列1，模板面板放在列2
        batch_frame.grid(row=0, column=0, sticky="nsew", padx=(self.scale(10), self.scale(10)))
        logger.debug(f"批量面板框架已放置到网格: row=0, column=0, sticky=nsew, padx=(10, 10)")
        
        center_frame.grid(row=0, column=1, sticky="ns", padx=(self.scale(10), self.scale(10)))
        logger.debug(f"中栏框架已放置到网格: row=0, column=1, sticky=ns, padx=(10, 10)")
        
        template_frame.grid(row=0, column=2, sticky="nsew", padx=(self.scale(10), self.scale(10)))
        logger.debug(f"模板栏框架已放置到网格: row=0, column=2, sticky=nsew, padx=(10, 10)")
    ...
    def _configure_main_container_grid(self):
        """
        配置主容器的网格权重为三列，使用固定边距布局
        列0: 左栏 (批量面板) - 两侧边距
        列1: 中栏 (主要内容区域) - 两侧边距
        列2: 右栏 (模板面板) - 两侧边距
        """
        logger.debug("配置主容器的网格权重为三列")
        ...
        # 配置3列网格权重
        self.main_container.grid_columnconfigure(0, weight=0)  # 左栏 - 固定宽度
        self.main_container.grid_columnconfigure(1, weight=0)  # 中栏 - 固定宽度
        self.main_container.grid_columnconfigure(2, weight=0)  # 右栏 - 固定宽度

        logger.debug("主容器三列网格权重配置完成")
    ...
    def _create_top_section(self, parent: tb.Frame) -> Tuple[tb.Labelframe, Any]:
        ...
        top_frame = tb.Labelframe(
            parent, text="文件输入", padding=self.scale(10), bootstyle="info"
        )
        ...
        try:
            ...
            logger.error(f"导入文件拖拽组件失败: {str(e)}")
            file_drop_area = tb.Label(
                top_frame, text="文件拖拽功能不可用",
                font=(self.default_font, self.default_size), bootstyle="danger"
            )
    ...
    def _create_bottom_section(self, parent: tb.Frame) -> Tuple[tb.Labelframe, Any]:
        ...
        bottom_frame = tb.Labelframe(
            parent, text="操作面板", padding=self.scale(10), bootstyle="primary"
        )
        ...
        try:
            ...
            action_panel = ActionPanel(
                bottom_frame, self.config_manager,
                on_action=self.on_action, on_cancel=self.on_cancel
            )
            action_panel.grid(row=0, column=0, sticky="nsew")
            logger.debug("操作面板创建成功")
        except ImportError as e:
            logger.error(f"导入操作面板失败: {str(e)}")
            action_panel = tb.Label(bottom_frame, text="操作功能不可用", bootstyle="danger")
    ...
    def _create_button_footer(self, parent: tb.Frame) -> tb.Frame:
        ...
        disclaimer_label = tb.Label(
            button_frame, text="测试软件，不保证准确率，请以人工核对为准",
            font=(self.micro_font, self.micro_size), bootstyle="danger", justify="center"
        )
        disclaimer_label.grid(row=0, column=1, sticky="ew")
    ...
    def _create_template_section(self, parent: tb.Frame) -> Tuple[tb.Frame, Any, Any]:
        ...
        template_frame = tb.Frame(
            right_panel_container, padding=self.scale(10)
        )
        ...
        try:
            ...
            tabbed_template_selector = TabbedTemplateSelector(
                template_frame,
                on_template_selected=self.on_template_selected
            )
            tabbed_template_selector.grid(row=0, column=0, sticky="nsew")
            logger.debug("选项卡式模板选择器创建成功")
        except ImportError as e:
            logger.error(f"导入选项卡式模板选择器失败: {str(e)}")
            tabbed_template_selector = tb.Label(template_frame, text="模板选择功能不可用", bootstyle="danger")
        ...
        try:
            ...
            conversion_panel = ConversionPanel(
                ...
                on_format_selected=self._get_format_selected_callback(),
            ...
            logger.error(f"导入格式转换面板失败: {str(e)}")
            conversion_panel = tb.Label(right_panel_container, text="格式转换功能不可用", bootstyle="danger")
    ...
    def _create_batch_section(self, parent: tb.Frame) -> Tuple[tb.Frame, Any]:
        ...
        try:
            ...
            logger.error(f"导入选项卡式批量文件列表组件失败: {str(e)}")
            tabbed_batch_file_list = tb.Label(batch_frame, text="选项卡式批量文件列表功能不可用", bootstyle="danger")
