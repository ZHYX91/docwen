...
class MainWindowEventHandler:
    ...
    def on_file_dropped(self, file_path):
        ...
        参数:
            file_path: 拖拽的文件路径（字符串）或文件路径列表（批量模式）
        """
        logger.info(f"文件拖拽事件: {file_path}")
        ...
        try:
            ...
            if isinstance(file_path, list):
                logger.info(f"批量模式处理 {len(file_path)} 个文件")
                # 批量模式：添加所有文件到批量列表
                if file_path:
                    self.logic.handle_batch_files_added(file_path)
                    logger.debug(f"批量模式：已添加 {len(file_path)} 个文件")
            else:
                # 单文件模式：传递mode参数
            ...
            logger.error(f"处理文件拖拽失败: {str(e)}", exc_info=True)
    ...
    def on_template_selected(self, template_type: str, template_name: str):
        ...
        try:
            ...
            logger.error(f"处理模板选择失败: {str(e)}", exc_info=True)
            self.main_window._show_error(f"处理模板选择失败: {str(e)}")
    ...
    def on_action(self, action_type: str, file_path: str, options: Dict[str, bool]):
        ...
        try:
            ...
            logger.error(f"处理操作事件失败: {str(e)}", exc_info=True)
            self.main_window._show_error(f"处理操作失败: {str(e)}")
    ...
    def on_cancel(self):
        ...
        try:
            ...
            self.main_window._show_error(f"取消操作失败: {e}")
    ...
    def on_clear_clicked(self):
        ...
        try:
            ...
            self.main_window._show_error(f"清空操作失败: {e}")
    ...
    def on_settings_clicked(self):
        ...
        try:
            # 导入新的设置对话框
            from gongwen_converter.gui.settings.dialog import SettingsDialog
            ...
            if hasattr(settings_dialog, 'theme_to_restore') and settings_dialog.theme_to_restore:
                logger.info(f"设置对话框已取消，正在将主题恢复为 '{settings_dialog.theme_to_restore}'")
                self.main_window.refresh_theme(settings_dialog.theme_to_restore)
            
        except ImportError as e:
            logger.error(f"导入设置对话框失败: {str(e)}")
            self.main_window._show_error("设置功能暂不可用")
        except Exception as e:
            logger.error(f"打开设置对话框失败: {str(e)}")
            self.main_window._show_error(f"打开设置失败: {str(e)}")
    ...
    def on_settings_applied(self, settings: Dict[str, Any]):
        ...
        try:
            ...
            self.main_window._show_error(f"应用设置失败: {str(e)}")
    ...
    def _apply_typo_settings(self, settings: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"刷新操作面板选项失败: {str(e)}")
    ...
    def on_about_clicked(self):
        ...
        try:
            ...
            logger.error(f"导入关于对话框模块失败: {str(e)}")
            self.main_window._show_error("关于功能暂不可用")
        except Exception as e:
            logger.error(f"打开关于对话框失败: {str(e)}")
            self.main_window._show_error(f"打开关于失败: {str(e)}")
