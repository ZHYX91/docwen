...
class ConversionPanelBase(tb.Frame):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.page_input_var: Optional[tk.StringVar] = None
        self.page_input_entry: Optional[tb.Entry] = None
        self.split_pdf_button: Optional[tb.Button] = None
        self.merge_pdfs_button: Optional[tb.Button] = None
        self.pdf_info_var: Optional[tk.StringVar] = None
        self.pdf_info_label: Optional[tb.Label] = None
        self.pdf_info_title_label: Optional[tb.Label] = None
        self.page_warning_label: Optional[tb.Label] = None
        
        # 图片压缩相关变量
        self.compress_mode_var: Optional[tk.StringVar] = None
        self.size_limit_var: Optional[tk.StringVar] = None
        self.size_unit_var: Optional[tk.StringVar] = None
        self.size_limit_entry: Optional[tb.Entry] = None
        self.size_unit_combo: Optional[tb.Combobox] = None
        self.size_warning_label: Optional[tb.Label] = None
        self.pdf_quality_var: Optional[tk.StringVar] = None
        self.convert_image_to_pdf_button: Optional[tb.Button] = None
        
        # TIFF合并相关变量
        self.tiff_mode_var: Optional[tk.StringVar] = None
        self.merge_tiff_button: Optional[tb.Button] = None
        
        # 版式文件渲染相关变量
        self.layout_image_dpi_var: Optional[tk.StringVar] = None
        self.convert_to_tif_button: Optional[tb.Button] = None
        self.convert_to_jpg_button: Optional[tb.Button] = None
        ...
        self.button_colors = {
            # 文档类
            'DOCX': 'primary', 'DOC': 'info', 'ODT': 'success', 'RTF': 'warning',
            # 表格类
            'XLSX': 'primary', 'XLS': 'info', 'ET': 'info', 'ODS': 'success', 'CSV': 'warning',
            # 图片类
            'PNG': 'primary', 'JPG': 'primary', 'BMP': 'info', 'GIF': 'success',
            'TIF': 'warning', 'WebP': 'danger',
            # 版式类
            'PDF': 'danger', 'XPS': 'info', 'OFD': 'success', 'CEB': 'warning'
        }
    ...
    def _create_widgets(self):
        ...
        self.conversion_frame = tb.Labelframe(
            self, text="格式转换", bootstyle="success"
        )
        ...
        self.saveas_frame = tb.Labelframe(
            self, text="另存为", bootstyle="danger"
        )
    ...
    def _update_button_normal_mode(
        self, fmt: str, button: tb.Button, is_same_format: bool, is_compress_mode: bool
    ):
        """非压缩模式下更新按钮状态"""
        if is_same_format:
            if is_compress_mode:
                ...
                button.configure(state='disabled', bootstyle='secondary')
            tooltip_text = self._get_button_tooltip(fmt, is_same_format, 'disabled' if not is_compress_mode else 'normal')
            ToolTip(button, tooltip_text)
        else:
            original_color = self.button_colors.get(fmt, 'primary')
            button.configure(state='normal', bootstyle=original_color)
    ...
    def _on_format_clicked(self, target_format: str):
        ...
        if self.on_action and self.current_file_path:
            ...
            options = {'target_format': target_format_lower}
            ...
            if self.current_category == 'image' and hasattr(self, 'compress_mode_var') and self.compress_mode_var:
                ...
                if compress_mode == 'limit_size':
                    if hasattr(self, 'size_limit_var') and self.size_limit_var:
                        try:
    ...
    def _get_image_button_tooltip(self, fmt_upper: str, is_same_format: bool, button_state: str) -> str:
        ...
        if is_compress_mode:
            if fmt_upper in self.COMPRESSIBLE_FORMATS:
                ...
                format_tips = {
                    'PNG': "PNG只支持无损压缩，无法有效减小文件\n建议选择JPEG或WebP格式",
                    'BMP': "BMP不支持压缩，无法减小文件\n建议选择JPEG或WebP格式",
                    'GIF': "GIF只支持有限压缩，无法有效减小文件\n建议选择JPEG或WebP格式",
                    'TIF': "TIF只支持有限压缩，无法有效减小文件\n建议选择JPEG或WebP格式",
                    'TIFF': "TIF只支持有限压缩，无法有效减小文件\n建议选择JPEG或WebP格式"
                }
                return format_tips.get(fmt_upper, f"{fmt_upper}格式无法有效压缩至目标大小\n建议选择JPEG或WebP格式")
        else:
    ...
    def _get_layout_button_tooltip(self, fmt_upper: str) -> str:
        """获取版式格式按钮的tooltip"""
        tooltips = {
            'PDF': "使用内置库将OFD/XPS转换为PDF格式" if self.current_format and self.current_format.lower() in ['ofd', 'xps'] else "当前已是PDF格式，无需转换",
            'DOCX': "优先使用Word/LibreOffice转换，备选使用内置工具",
            'DOC': "先转为DOCX，再通过本地Office软件转为DOC格式",
            'TIF': "将PDF页面渲染为TIF图片，支持多页TIFF，支持选择DPI质量",
            'JPG': "将PDF页面渲染为JPG图片，支持选择DPI质量"
        }
