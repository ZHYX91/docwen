...
from gongwen_converter.utils.file_type_utils import is_supported_file, get_file_category
...
class FileDropArea(tb.Frame):
    ...
    def __init__(self, master, on_file_dropped: Optional[Callable] = None, 
                 on_show_batch_panel: Optional[Callable] = None,
                 on_hide_batch_panel: Optional[Callable] = None,
                 height: int = 120, default_mode: str = "single", **kwargs):
        """
        初始化文件拖拽区域组件
    ...
    def _create_widgets(self):
        ...
        self.document_prompt_label = tb.Label(
            self.prompt_frame,
            text="文档文件：DOCX、DOC、WPS、RTF、ODT",
            font=(self.small_font, self.small_size),
            bootstyle="secondary",
            anchor=tk.CENTER,
            padding=(0, 3, 0, 0)
        ...
        self.image_prompt_label = tb.Label(
            self.prompt_frame,
            text="图片文件：TIF、JPG、PNG、BMP、GIF、HEIC、WebP等",
            font=(self.small_font, self.small_size),
            bootstyle="secondary",
            anchor=tk.CENTER,
            padding=(0, 3, 0, 0)
    ...
    def update_display(self, selected_file_info=None):
        ...
        """
        logger.debug(f"update_display 被调用，模式: {self.mode}, 文件: {selected_file_info.file_path if selected_file_info else 'None'}")
    ...
    def _create_single_file_display(self):
        ...
        """
        from gongwen_converter.utils.file_type_utils import validate_file_format
        ...
        if self.file_path:
            ...
            file_label = tb.Label(
                ...
                anchor=tk.CENTER,
                justify=tk.CENTER,
                wraplength=scale(340)  # 自动换行，适应长文件名
            )
    ...
    def _create_batch_info_display(self):
        ...
        total_label = tb.Label(
            self.file_info_frame,
            text=f"待批量处理：共{total_files}个{category_name}文件",
            font=(self.default_font, self.default_size),
            bootstyle="primary",
            anchor=tk.CENTER,
            justify=tk.CENTER
    ...
    def _on_clear_clicked(self):
        ...
        # 先调用外部清空回调（清空所有文件）
        # 确保数据层和界面层同步清空
        if hasattr(self, 'on_clear_clicked') and self.on_clear_clicked:
    ...
    def _get_tabbed_file_manager(self):
        ...
        try:
            ...
            if current and hasattr(current, '_main_window'):
                main_window = current._main_window
                if hasattr(main_window, 'tabbed_file_manager'):
    ...
    def _validate_single_file_mode(self, files: List[str]) -> Tuple[bool, str]:
        ...
        """
        if len(files) > 1:
            return False, "单文件模式下只能拖入一个文件"
        
        if len(files) == 0:
            return False, "未检测到有效文件"
        
        file_path = files[0]
        
        # 检查是否是文件夹
        if os.path.isdir(file_path):
            return False, "单文件模式下不能拖入文件夹"
    ...
    def _process_batch_files(self, files: List[str]) -> List[str]:
        ...
        for file_path in files:
            if os.path.isdir(file_path):
                # 批量模式下跳过文件夹
                # 注意：这里不进行递归处理，保持逻辑简单
                logger.debug(f"批量模式下跳过文件夹: {file_path}")
            else:
                # 如果是文件，检查是否支持
    ...
    def _open_file_dialog(self):
        ...
        if self.mode == "single":
            ...
            file_path = filedialog.askopenfilename(
                title="选择文件",
                filetypes=[
                    ("所有支持的文件", "*.md;*.txt;*.docx;*.doc;*.wps;*.rtf;*.odt;*.xlsx;*.xls;*.et;*.csv;*.ods;*.pdf;*.xps;*.ofd;*.tif;*.tiff;*.jpg;*.jpeg;*.png;*.bmp;*.gif;*.heic;*.heif;*.webp"),
                    ("文本文件", "*.md;*.txt"),
                    ("文档文件", "*.docx;*.doc;*.wps;*.rtf;*.odt"),
                    ("表格文件", "*.xlsx;*.xls;*.et;*.csv;*.ods"),
                    ("版式文件", "*.pdf;*.xps;*.ofd"),
                    ("图片文件", "*.tif;*.tiff;*.jpg;*.jpeg;*.png;*.bmp;*.gif;*.heic;*.heif;*.webp"),
                    ("所有文件", "*.*")
                ]
            )
