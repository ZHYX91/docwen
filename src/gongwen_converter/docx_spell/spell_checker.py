...
class TextValidator:
    ...
    def __init__(self, symbol_pairing=None, symbol_correction=None, 
                 typos_rule=None, sensitive_word=None):
        """
        初始化文本验证器（支持配置覆盖）
        采用延迟加载策略，避免不必要的资源消耗
    ...
    def _configure_engine(self):
        ...
        try:
            ...
            logger.error(f"引擎配置失败: {str(e)}", exc_info=True)
            # 配置失败时，禁用所有功能以保证安全
            self.symbol_pairing_enabled = False
            self.symbol_correction_enabled = False
            self.typos_rule_enabled = False
    ...
    def validate_text(self, text: str) -> List[TextError]:
        ...
        if self.symbol_pairing_enabled:
            ...
            symbol_errors = self.symbol_pairing_check(text)
            errors.extend(symbol_errors)
            logger.info(f"符号配对检查完成，发现 {len(symbol_errors)} 个错误")
    ...
    def typos_check(self, text: str) -> List[TextError]:
        ...
        try:
            ...
            if self.symbol_correction_enabled:
                logger.debug("开始标点符号检测...")
                
                # 3.1 遍历标点符号映射表（正确标点→错误标点列表）
                for correct_punc, wrong_puncs in symbol_map.items():
                    logger.debug(f"处理正确标点: '{correct_punc}' 的 {len(wrong_puncs)} 个错误形式")
                    
                    # 确保错误列表是有效的
                    if not isinstance(wrong_puncs, list):
                        logger.warning(f"标点错误列表类型错误: {type(wrong_puncs)}, 跳过")
                        continue
                    ...
                    for wrong_punc in wrong_puncs:
                        ...
                        # 3.3 在文本中查找所有错误标点出现的位置
                        pattern = re.escape(wrong_punc)
                        for match in re.finditer(pattern, text):
                            start, end = match.span()
                            error_text = text[start:end]
                            
                            # 3.4 检查此位置是否已被错别字检测报告
                            already_reported = False
                            for error in errors:
                                # 检查错误位置是否重叠
                                if (start >= error.start_pos and end <= error.end_pos) or \
                                (error.start_pos >= start and error.end_pos <= end):
                                    already_reported = True
                                    logger.debug(f"位置 {start}-{end} 已被错别字错误覆盖，跳过标点错误报告")
                                    break
                ...
                logger.debug("符号校对已禁用，跳过检查")
            # ================== 结果统计 ==================
            logger.info(f"校对完成 | 错别字: {typo_errors} | 标点错误: {symbol_errors} | 总计: {len(errors)}")
            
        except Exception as e:
            # 记录详细错误信息
            logger.error(f"错别字校对失败: {str(e)}", exc_info=True)
            # 返回已发现的错误（可能不完整）
            ...
            # 如果是闭符号，检查是否匹配
                ...
                if stacks[opening_char]:
                    ...
                    errors.append(TextError(
                        ...
                        end_pos=i+1,
                        error_text=char,
                        suggestion="多余的闭符号，建议删除或添加匹配的开符号",
                        error_type="符号不匹配",
                        source="标点配对"
                    ))
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler("docx_spell_validator_test.log")
        ]
    )
    
    logger = logging.getLogger("TextValidatorTest")
    logger.info("文本验证器模块测试开始")
    
    # 创建验证器实例
    validator = TextValidator()
    logger.info(f"初始状态: {validator}")
    
    # 测试文本
    test_text = "少先队员因该为老人让坐。这个句子有一些(括号不匹配。"
    logger.info(f"测试文本: {test_text}")
    
    # 执行验证
    errors = validator.validate_text(test_text)
    
    # 输出结果
    if errors:
        logger.info("\n发现错误:")
        for error in errors:
            logger.info(f"- 位置: {error.start_pos}-{error.end_pos}")
            logger.info(f"  错误文本: '{error.error_text}'")
            logger.info(f"  建议: {error.suggestion}")
            logger.info(f"  类型: {error.error_type}")
            logger.info(f"  来源: {error.source}")
    else:
        logger.info("\n未发现错误")
    
    # 验证器状态
    logger.info(f"最终状态: {validator}")
    
    logger.info("文本验证器模块测试结束")
