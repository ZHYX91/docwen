...
def initialize_app():
    ...
    # 确保gongwen_converter包在Python路径中
    ...
    # 1. 运行核心安全防护
    try:
        from gongwen_converter.security.protection_utils import run_all_protections
        run_all_protections()
        ...
        # 任何安全检查失败都应立即终止程序
        logger.critical(f"核心安全检查失败，程序无法启动: {e}")
        os._exit(1) # 使用硬退出

    # 2. 初始化网络隔离
    try:
        from gongwen_converter.security.network_isolation import initialize_network_isolation
