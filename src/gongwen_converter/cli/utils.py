...
def prompt_choice(options: List[str], prompt_text: str = "请选择", default: int = 1) -> int:
    """
    让用户从选项列表中选择
    ...
    while True:
        try:
            response = input(f"{prompt_text} [1-{len(options)}, 默认{default}]: ").strip()
...
def print_file_list(files: List[str], title: str = "文件列表"):
    """
    打印文件列表
...
def validate_file(file_path: str) -> tuple[bool, str]:
    ...
    # 使用实际格式检测（与GUI保持一致）
    try:
        from gongwen_converter.utils.file_type_utils import validate_file_format, ACTUAL_FORMAT_TO_CATEGORY
