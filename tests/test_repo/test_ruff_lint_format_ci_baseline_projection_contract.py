...
def test_vis155_keeps_the_existing_lint_gate_strict_and_reachable() -> None:
    ...
    qa = _read("tools/qa.py")
    assert '[sys.executable, "-m", "ruff", "format", "--check", "."]' in qa
    assert '[sys.executable, "-m", "ruff", "check", "."]' in qa
    assert "steps += source_checks.architecture_steps()" in qa
