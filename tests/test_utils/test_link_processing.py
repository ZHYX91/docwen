...
class TestUnescapePipe:
    ...
    def test_escaped_pipe(self) -> None:
        assert unescape_pipe(r"a\|b") == "a|b"

    def test_no_escape(self) -> None:
        assert unescape_pipe("a|b") == "a|b"

    def test_none_input(self) -> None:
        assert unescape_pipe(None) is None

    def test_empty_string(self) -> None:
        assert unescape_pipe("") == ""

    def test_multiple_escapes(self) -> None:
        assert unescape_pipe(r"a\|b\|c") == "a|b|c"
...
class TestNormalizeLinkTarget:
    ...
    def test_url_decode(self) -> None:
        assert normalize_link_target("hello%20world.md") == "hello world.md"

    def test_remove_anchor(self) -> None:
        assert normalize_link_target("file.md#section") == "file.md"

    def test_remove_query_params(self) -> None:
        assert normalize_link_target("image.png?v=1") == "image.png"

    def test_combined(self) -> None:
        assert normalize_link_target("file%20name.md#heading?v=2") == "file name.md"

    def test_plain_path(self) -> None:
        assert normalize_link_target("simple.md") == "simple.md"
    ...
    def test_chinese_url_encoding(self) -> None:
        ...
        result = normalize_link_target("%E6%B5%8B%E8%AF%95.md")
        assert result == "测试.md"
...
class TestParseAnchor:
    ...
    def test_heading_anchor(self) -> None:
        assert parse_anchor("file.md#标题名") == ("file.md", "标题名", None)

    def test_block_id_anchor(self) -> None:
        assert parse_anchor("file.md#^block-id") == ("file.md", None, "block-id")

    def test_url_encoded(self) -> None:
        path, heading, block_id = parse_anchor("file%20name.md#%E6%A0%87%E9%A2%98")
        assert path == "file name.md"
        assert heading == "标题"
        assert block_id is None

    def test_query_params_removed(self) -> None:
        path, heading, block_id = parse_anchor("file.md?v=1")
        assert path == "file.md"
    ...
    def test_no_file_path_with_heading(self) -> None:
        """仅锚点，无文件路径"""
        path, heading, _block_id = parse_anchor("#标题")
        assert path == ""
        assert heading == "标题"

    def test_no_file_path_with_block_id(self) -> None:
        path, _heading, block_id = parse_anchor("#^myblock")
        assert path == ""
...
class TestExtractSectionByHeading:
    ...
    def test_extract_second_level(self) -> None:
        result = extract_section_by_heading(self.SAMPLE_CONTENT, "二级标题A")
    ...
    def test_extract_top_level(self) -> None:
        result = extract_section_by_heading(self.SAMPLE_CONTENT, "一级标题")
        assert result is not None
        assert "# 一级标题" in result
        assert "一级内容" in result

    def test_heading_not_found(self) -> None:
        result = extract_section_by_heading(self.SAMPLE_CONTENT, "不存在的标题")
        assert result is None

    def test_case_insensitive_match(self) -> None:
        content = "## Hello World\nsome content\n## Next"
        result = extract_section_by_heading(content, "hello world")
        assert result is not None
        assert "Hello World" in result

    def test_last_section_includes_to_end(self) -> None:
        result = extract_section_by_heading(self.SAMPLE_CONTENT, "二级标题B")
        assert result is not None
...
class TestExtractBlockById:
    ...
    def test_inline_block_id(self) -> None:
        ...
        result = extract_block_by_id(content, "important")
        assert result is not None
    ...
    def test_standalone_block_id(self) -> None:
        content = "前面的段落\n\n这是要引用的段落\n\n^myblock\n\n后面的段落"
        result = extract_block_by_id(content, "myblock")
        assert result is not None
        assert "要引用的段落" in result

    def test_block_id_not_found(self) -> None:
        content = "普通内容\n没有块ID"
        result = extract_block_by_id(content, "missing")
        assert result is None
...
class TestStripYamlFrontMatter:
    ...
    def test_without_yaml(self) -> None:
        content = "# 标题\n正文"
        assert strip_yaml_front_matter(content) == content
...
class TestFormatImagePlaceholder:
    ...
    def test_no_size(self) -> None:
        result = format_image_placeholder("/path/to/image.png")
        assert result == "{{IMAGE:/path/to/image.png}}"

    def test_with_width_only(self) -> None:
        result = format_image_placeholder("/path/to/image.png", width=300)
        assert "300" in result
        assert "IMAGE:" in result

    def test_with_width_and_height(self) -> None:
        result = format_image_placeholder("/path/to/image.png", width=300, height=200)
        assert "300" in result
...
class TestSplitAltTextAndSize:
    ...
    def test_no_size(self) -> None:
        alt, w, h = split_alt_text_and_size("描述文字")
        assert alt == "描述文字"
        assert w is None
        assert h is None

    def test_width_only(self) -> None:
        alt, w, h = split_alt_text_and_size("描述|300")
        assert alt == "描述"
        assert w == 300
        assert h is None

    def test_width_and_height(self) -> None:
        alt, w, h = split_alt_text_and_size("描述|300x200")
        assert alt == "描述"
        assert w == 300
        assert h == 200

    def test_width_without_height(self) -> None:
        alt, w, h = split_alt_text_and_size("描述|300x")
        assert alt == "描述"
        assert w == 300
        assert h is None

    def test_none_input(self) -> None:
        alt, w, h = split_alt_text_and_size(None)
        assert alt is None
        assert w is None
        assert h is None

    def test_empty_string(self) -> None:
        alt, w, _h = split_alt_text_and_size("")
        assert alt is None
        assert w is None

    def test_non_numeric_after_pipe(self) -> None:
        """竖线后不是数字，保持原样"""
        alt, w, _h = split_alt_text_and_size("alt|caption")
