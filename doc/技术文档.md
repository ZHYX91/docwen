...
| image_config.toml | `get_image_config_block()` | extraction_defaults | 图片提取默认设置 |
...
| `get_image_config_block()` | 获取图片配置块 | Dict |
...
docx2md/
├── __init__.py
├── core.py                 # 入口函数（支持图片和OCR）
├── element_scorer.py       # 元素评分识别（三轮评分算法）
├── content_injector.py     # 内容注入
├── table_processor.py      # 表格处理
├── textbox_processor.py    # 文本框处理
└── image_processor.py      # 图片提取处理器
```
...
```python
{
    ...
    'attachment_content': str,    # 附件markdown内容（可选）
...
|-------|------|---------|
| `main()` | CLI主程序入口 | None |
| `interactive_mode()` | 交互模式主循环 | None |
| `parse_file_path()` | 文件路径解析器 | str |
...
**设计理念**: 职责分离，router负责用户交互，策略层负责业务逻辑

**核心类**: `FileRouter`

**主要方法**:
- `route_file()` - 统一路由文件处理
- `_interactive_process_md()` - 交互式处理MD文件
- `_interactive_process_docx()` - 交互式处理DOCX文件
- `_select_template()` - 模板选择交互
- `_get_spell_check_option()` - 错别字规则选择（位标志0-15）
