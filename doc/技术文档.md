...
│   └── docwen/                # 核心代码（Python包）
│       ├── bootstrap.py       # 统一启动器
│       ├── cli_run.py         # CLI入口（实际 main 在此）
│       ├── gui_run.py         # GUI入口（实际 main 在此）
│       ├── cli/               # CLI实现（含 i18n.py）
...
| conversion_defaults.toml | `get_conversion_defaults_block()` | document, spreadsheet, image, layout | 文件处理默认设置 |
...
**组织方式**: `services/strategies/` 采用“注册表 + 子包导入触发注册”的机制，按文件类别拆分子包；部分格式互转策略通过工厂批量动态注册。
...
services.strategies (策略层：注册表 + 子包导入触发注册)
    ├─→ converter (转换核心)
    ├─→ docx_spell (校对核心)
    └─→ utils (工具函数)
...
- `src/gui_run.py` / `src/cli_run.py` - 顶层薄封装（便于直接运行）
...
3. **❌图片 ✅OCR**：文本+OCR → `*.md`（包含OCR文本）
4. **✅图片 ✅OCR**：文本+图片+OCR → `*.md` + 图片文件（并包含OCR文本）
...
- 在文件末尾导入各子包以触发策略类注册（按类别组织，部分策略动态批量注册）
...
| `markdown/` | `services/strategies/markdown/to_document.py`, `services/strategies/markdown/to_spreadsheet.py` | MD/TXT：转文档、转表格 |
...
- `LayoutToMarkdownPymupdf4llmStrategy` 为 `LayoutToMarkdownStrategy` 的兼容别名
...
**文件**: `security/expiration_check.py`
...
- 校对选项：使用 `proofread_options` 布尔字典（symbol_pairing/symbol_correction/typos_rule/sensitive_word）；缺失或 None 表示按 `proofread_config.engine.enable_*` 默认执行；`{}` 或全 False 表示跳过校对
...
| 支持的语言 | `src/docwen/i18n/locales/`（当前 11 个，文件为 .toml） |
...
### 12.5 策略注册系统
采用装饰器注册模式（`@register_conversion`/`@register_action`）+ 动态批量注册，实现大量可扩展的转换/操作策略。
