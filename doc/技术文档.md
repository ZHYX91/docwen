...
| OCR识别 | RapidOCR | ≥1.4.0 | 离线文字识别（ONNX） |
| OCR引擎 | ONNX Runtime | - | 高效跨平台推理引擎 |
| 图片处理 | Pillow | - | 图片格式转换 |
...
├── src/docwen/     # 核心代码
│   ├── bootstrap.py            # 统一启动器
│   ├── config/                 # 配置管理（Mixin架构，4个核心文件 + schemas/）
│   ├── converter/              # 转换引擎（7个子模块）
...
├── templates/                 # 模板文件
├── models/                    # OCR模型
│   └── rapidocr/             # RapidOCR ONNX模型文件（约16MB）
├── scripts/                   # 构建脚本
└── doc/                      # 文档
```
...
**核心类**: `ConfigManager`  
**设计模式**: 单例模式 + Mixin架构  
**架构**: 核心类约15个方法 + 9个功能Mixin（共100+方法）
...
**📍 位置**: `src/docwen/converter/table_converters.py`
...
- `detection.py` - 办公软件可用性检测（WPS/Office/LibreOffice）
- `fallback.py` - 三级容错转换框架（WPS → Office → LibreOffice）
...
**📍 位置**: `src/docwen/converter/formats/image.py`
...
| 存储位置 | 内容 | 用途 |
|----------|------|------|
| `i18n/locales/*.toml` [styles] 节 | 我们定义的样式名 | 注入 + 检测 |
| `configs/style_*.toml` *_aliases 列表 | 第三方软件内置样式名 | 仅检测 |
...
**技术栈**: RapidOCR ≥ 1.4.0 + ONNX Runtime
...
- 无需设置环境变量或网络隔离配置
- 模型文件位于 `models/rapidocr/` 目录（约16MB）

**模型文件**:
```
models/rapidocr/
├── ch_PP-OCRv4_det_infer.onnx    (~4MB, 检测模型)
├── ch_PP-OCRv4_rec_infer.onnx    (~10MB, 识别模型)
└── ch_ppocr_mobile_v2.0_cls_infer.onnx (~2MB, 方向分类模型)
```
