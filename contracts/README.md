...
A successful conversion returns one single-document Bundle: one preferred DOCX `document`, cardinality `one`,
artifact kinds `[document]`, and no relations. The DOCX is integrity-pinned by its size and SHA-256. No adjacent
`.docwen` file, saved original-source snapshot, or dedicated source-recovery schema is part of the contract.
Reverse conversion reconstructs current document semantics from the DOCX alone.
...
| Resolved Markdown to DOCX | one preferred DOCX `document`, independently convertible without an original-source companion |
