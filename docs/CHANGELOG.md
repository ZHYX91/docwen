...
- 统一活动记录中的本地路径显示；Windows 使用本机分隔符，不改变内部路径身份或 Markdown 链接。
- OCR 成功不再把任务标成黄色警告；未识别到文字、模型/引擎不可用和识别失败仍保留明确警告，并提供本地化摘要。
...
- 修复 Word 另存并重编号题注样式 ID 后，扩展文档导回 Markdown 被错误拒绝的问题；保留样式身份与语义冲突检查。
