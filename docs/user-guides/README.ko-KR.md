...
3. 저장한 후 DocWen 목록을 새로 고칩니다. 다시 시작할 필요가 없습니다.
...
- **설정 → 템플릿**에서 활성화되어 있고 출력 형식과 일치하는지 확인하세요.
- DOCX/XLSX 파일이 정상적으로 열리는지 확인하세요.
- 외부에서 수정한 후 목록을 새로 고치고 새 파일은 가져오세요.
...
- **설정 → 템플릿**을 엽니다. 기본 제공 템플릿은 복사한 후 편집하고 사용자 지정 템플릿은 직접 편집할 수 있습니다.
- Word, Excel 또는 WPS에서 스타일을 조정하고 저장한 후 목록을 새로 고칩니다.
- 새 파일을 생성하기 전에 사용자 지정 템플릿을 선택하세요.
