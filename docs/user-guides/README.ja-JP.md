...
3. 保存して DocWen の一覧を更新します。再起動は不要です。
...
DocWen Assistant 3.1 には DocWen 0.13.0 以降が必要です。変換・校正などの内容操作は Machine Protocol v2 と Artifact Bundle v3 を使用し、デスクトップアプリの起動／オープンは独立したローカル GUI 制御を使用します。
...
- **設定 → テンプレート**で有効になっているか、出力形式と一致するか確認してください。
- DOCX/XLSX ファイルが正常に開けるか確認してください。
- 外部で変更した後は一覧を更新し、新しいファイルはインポートしてください。
...
- **設定 → テンプレート**を開きます。内蔵テンプレートはコピーしてから、カスタムテンプレートは直接編集できます。
- Word、Excel、WPS でスタイルを調整し、保存後に一覧を更新します。
- 新しいファイルを生成する前に、そのカスタムテンプレートを選択します。
