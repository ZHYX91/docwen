...
在默认分支已接纳的提交推送数字版本标签，即在同一次运行完成普通 CI、发布测试、每平台一次构建、精确候选签证、immutable 发布和一次独立资产下载回验；版本取标签，artifact ID 与摘要由当前构建直接传递，不查找历史预检运行。手动 `verify` 在同一标签填写候选 `artifact_id` 即可只读回验；发布中断后用 `publish`、同一候选 ID 和原进度 `resume_artifact_id` 恢复，摘要由 GitHub 读取。原进度所有者必须结束，结果不明的写入先读回核实；精确已发布资产不再写入，匹配草稿只补缺项。不得通过重新构建所有作业恢复发布。
