...
When `--office-smoke` is selected, the verifier also writes a hash-bound `gui_office_smoke/office-smoke-evidence.json` inside the raw run before receipt projection. It records the native host system/release/machine, the actual backend reported by each DOCX/XLSX/Markdown→PDF route, and exact byte counts/SHA-256 values for each generated source fixture, conversion report, and verified PDF. The compact acceptance receipt retains the evidence-file hash, host/backend summary and every per-case source/report/PDF identity, selected backend executable/version identity, content checks and PDF page count. These remain reviewable after successful raw-run cleanup. Windows versions come from the selected backend registered COM executable FileVersion; LibreOffice uses its selected executable --version. Layout review is explicitly separate and not implied by content checks. This proves only the selected candidate on that host and does not generalize to other Office versions, machines, rendering paths, or unselected routes.
