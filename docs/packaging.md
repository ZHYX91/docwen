...
DocWen 0.10 publishes one Windows x64 package and two Ubuntu 24.04 x64 packages built from the bundle composition root. The Windows archive contains both GUI and CLI; Ubuntu has GUI+CLI and CLI-only archives. Package verification runs against the produced directory, not the source tree. macOS has no 0.10 release asset, and its current capability contract reports conversion, validation, numbering, merge, and split as unavailable.

DocWen 0.10 正式发布一个 Windows x64 完整包和两个 Ubuntu 24.04 x64 包；Windows 包同时包含 GUI 与 CLI，Ubuntu 分为 GUI+CLI 和仅 CLI 两个压缩包。打包验证面向实际产物目录执行，而不是用源码态结果代替。macOS 没有 0.10 正式附件，当前 capability 契约明确把转换、校对、编号、合并和拆分报告为不可用。

| Platform / 平台 | 0.10 distribution status / 发行状态 | Required validation / 所需验证 |
| --- | --- | --- |
...
| macOS x64/arm64 | No 0.10 release asset / 无 0.10 正式附件 | Source CI and opt-in packaging experiment only; primary document operations are unavailable / 仅源码 CI 与手动打包实验，主要文档操作不可用 |
...
5. Manifest-bound deterministic Ubuntu archive construction with fixed `DocWen-0.10.0-linux-x64.tar.gz` and `DocWenCLI-0.10.0-linux-x64.tar.gz` names.
...
5. 按受版本控制的生产清单确定性构造 Ubuntu 压缩包，并固定为 `DocWen-0.10.0-linux-x64.tar.gz` 与 `DocWenCLI-0.10.0-linux-x64.tar.gz`。
