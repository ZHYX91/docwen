...
class TestNoOldImports:
    def test_main_window_no_old_imports(self) -> None:
        ...
        assert source is not None, "required main_window module source is missing"
        assert Path(source).is_file(), f"main_window module source is not a file: {source}"
    ...
    def test_app_module_no_old_imports(self) -> None:
        ...
        assert source is not None, "required app module source is missing"
        assert Path(source).is_file(), f"app module source is not a file: {source}"
