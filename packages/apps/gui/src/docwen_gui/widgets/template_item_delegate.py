...
from ..i18n import t
from ..styles.ui_scale import dp, scale_percent
...
class TemplateItemDelegate(QStyledItemDelegate):
    ...
    def sizeHint(self, option, index) -> QSize:
        return QSize(0, max(dp(32), option.fontMetrics.height() + dp(12)))
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex) -> None:
        ...
        painter.save()
        if self.draggable:
            row.rect.adjust(dp(16), 0, 0, 0)
            painter.setPen(Qt.PenStyle.NoPen)
            painter.setBrush(row.palette.color(QPalette.ColorRole.Mid))
            for x in (6, 10):
                for y in (-4, 0, 4):
                    painter.drawEllipse(option.rect.left() + dp(x), option.rect.center().y() + dp(y), dp(2), dp(2))
        style.drawControl(QStyle.ControlElement.CE_ItemViewItem, row, painter, row.widget)
        content = style.subElementRect(QStyle.SubElement.SE_ItemViewItemText, row, row.widget).adjusted(
            dp(4), 0, -dp(8), 0
        )
        badge_font = QFont(row.font)
        badge_font.setPointSizeF(max(8 * scale_percent() / 100, badge_font.pointSizeF() * 0.82))
        ...
        content.setLeft(badge.right() + dp(9))
        ...
        if index.data(DEFAULT_ROLE):
            ...
            marker_width = metrics.horizontalAdvance(label) + dp(8)
            ...
            content.setRight(marker.left() - dp(8))
    ...
    def editorEvent(self, event, model, option, index) -> bool:
        ...
        if self.draggable:
            adjusted.rect.adjust(dp(16), 0, 0, 0)
