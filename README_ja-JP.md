...
- 完全な国際化サポート（GUI と CLI が 11 言語をサポート）
- 互換性向上のため PaddleOCR を RapidOCR に置き換え
- 多言語 Word/Excel テンプレートを追加
- テンプレートスタイルの自動検出と注入
- その他の最適化と修正

### v0.5.1 (2025-01-01)

- 双方向の数式変換（Word OMML ↔ Markdown LaTeX）を追加しました。
- 双方向の脚注/文末脚注変換を追加しました。
- コード、引用などの文字スタイルと段落スタイルを追加しました。
- リスト処理（多層ネスト、自動番号付け）を強化しました。
- テーブル機能（スタイル検出/注入、三線表など）を強化しました。
- 小見出し番号のクリーニングと追加を最適化しました。
- インターフェースの相互作用と設定の連携を改善しました。

### v0.4.1 (2025-12-05)

- ユーザーエクスペリエンスを向上させるためにCLIをリファクタリングしました。
- より多くの文書タイプのサポートを追加しました。
- より多くの構成可能なオプションを実装しました。

## 🚀 クイックスタート

### プログラムの起動

`DocWen.exe` をダブルクリックしてグラフィカルインターフェースを起動します。
...
1.  **プログラムの起動**：`DocWen.exe` をダブルクリックします。
2.  **ファイルのインポート**：
...
-   **インタラクティブモード**：ファイルを渡した後にメニューガイドを表示します。GUI操作と同様です。
-   **ヘッドレスモード**：`--action` パラメータを追加して直接実行します。スクリプト呼び出しに適しています。
...
DocWen.exe document.md --action convert --target docx --template "テンプレート名"
...
| 引数 | 説明 |
| :--- | :--- |
| `--action` | 操作タイプ：`export_md`, `convert`, `validate`, `merge_pdfs`, `split_pdf` |
| `--target` | ターゲット形式：`pdf`, `docx`, `xlsx`, `md` |
| `--template` | テンプレート名（例：`テンプレート名`） |
| `--extract-img` | エクスポート中に画像を抽出 |
| `--ocr` | OCR認識を有効にする |
| `--batch` | バッチ処理モード |
| `--yes` / `-y` | 確認プロンプトをスキップ |
| `--continue-on-error` | エラー時に次の項目の処理を続行 |
| `--json` | JSON形式で結果を出力 |
